...
<!-- Latest compiled and minified CSS -->
    ...
    <script src="{{ asset('/js/libs/axios/0.21.1/axios.js') }}"></script>
    ...
    <script>
                ...
                $.ajax({
                    ...
                    success: function(data, textStatus, jqXHR){
                        ...
                        if(jqXHR.status === 203){
                            ...
                            window.location.href = redirectUrl?redirectUrl:(data.hasOwnProperty('redirectUrl')?data.redirectUrl:'{{rtrim(url("/"))}}/');
        ...
        $(document).ready( function(){
            var url = new Array($(location).attr('href'));
            var post_url = "{{ Config::get('app.api_host_domain','http://localhost/') }}";
            var post_url = post_url + '/api/before_login';
            axios.post(post_url,url)
                .then(response => {
                    if(response.data == 1){
                        $('#code_btn').removeClass('disnone'); // コード入力画面遷移ボタン表示
                    }else{
                        $('#pswd_btn').removeClass('disnone'); // パスワードリセット画面遷移ボタン表示
                    }
                })
                .catch(error => { return []; });
        });
...
<div class="page">
    <div class="content">
        <div class="content-left row-panel">
            ...
            <div class="panel-login">
                ...
                <form id="loginform">
                    ...
                    <a id='pswd_btn' class='disnone' href="{{url('/reentry')}}" style="color:black;"><u>パスワードをお忘れですか?</u></a>
                    <a id='code_btn' class='disnone' href="{{url('/password-code')}}" style="color:black;"><u>パスワード設定</u></a> <br />
                </form>
            </div>
        </div>
        <div class="content-right row-panel">
            <div>
                <img class="logo" src="{{ asset('images/logo_shachihata.png') }}">
                <div id="notice-text1" class="font1"><b></b></div>
                <div class="font2"><b>シヤチハタブランドをデジタルでも</b></div>
            </div>
            <div style="height:50%;">
                <img class="logo" src="{{ asset('images/show.png') }}">
                <div style="width:300px;height:40px;line-height:40px;color:#0984e3;background-color:#fff;border:1px solid #0984e3;border-radius: 5px 5px 5px 5px;margin:20px auto;">
