...
<!--          <feather-icon
          ...
          <vs-icon v-if="!featherIcon" :icon-pack="iconPack" :icon="icon" />
            <img v-else-if="customIcon" style="width:18px;height:18px;" :src="bindIcon(customIcon, isGroupActive)">
<!--          <feather-icon v-else :class="{'w-3 h-3': iconSmall}" :icon="icon" />-->
          <slot />
          <!-- Group Name -->
          <span :style="customIcon?'margin-left:14px;':''" v-show="!verticalNavMenuItemsMin" class="truncate mr-3 select-none">{{ group.name}}</span>
...
export default {
  ...
  methods: {
    ...
    bindIcon(icon, active) {
        if(icon === "timesheet"){ // Timesheet
            if(active){
                return require("@assets/images/pages/home/timesheet.svg");
            }else{
                return require("@assets/images/pages/home/timesheet.svg");
            }
        }else if (icon === "form-issuance"){ // Timesheet
          if(active){
            return require("@assets/images/pages/home/form_invoice.svg");
          }else{
            return require("@assets/images/pages/home/form_invoice.svg");
          }
        }
