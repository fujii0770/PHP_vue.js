    ...
    <vs-card>
      <vs-row >
        <vs-col vs-type="flex" vs-justify="space-between">
          <div class="vs-con-input flex-1">
            <datepicker  :format="DatePickerFormat" :use-utc="true" :language="ja" minimum-view="month" v-model="workMonth" id="workMonth"></datepicker>
            <div class="total-work-time-header flex flex-1 items-start">
              <div class="mr-6">
                <span>{{this.dispTotalWorkingTime}}</span>
              </div>
              <div class="flex-1 total-text" vs-type="flex" vs-align="center">
                <div class=""> 
                  <div class="inline-block mr-3 total-text">
                    {{this.dispTotalOverTime}}
                  </div>
                  <div class="inline-block total-text">
                    {{this.dispTotalHolidayWorkingTime}}
                  </div>
                </div>
                <vs-row class="inline-block">
                  <vs-col class="total-text" vs-type="block">
                    {{this.dispTotalMidNightWorkingTime}}
                  </vs-col>
                </vs-row>
              </div>
            </div>
          ...
          <div style="padding-right: 15px">
            <vs-button @click="onSelectAdjustWorkHours" class="square" color="success"> 勤務表回覧</vs-button>
            <vs-button @click="onSelectExport" class="square" color="success"> CSV出力</vs-button>
            <vs-button v-if="buttonSendStatus == 0" class="square" color="success" v-on:click="updateSubmissionState()"> 提出</vs-button>
            <vs-button v-else class="square" color="success" disabled> 提出</vs-button>
          </div>
        </vs-col>
      </vs-row>
      <vs-popup name="export-work-list"
      ...
      <vs-popup name="adjust-work-hours"
            :active.sync="showAdjustWorkHoursPopup"
            title="勤務時間の端数調整"
      >
        <vs-row class="">
          <vs-col vs-w="12" vs-type="block">
            <span class="title-bolder"><p>端数の調整方法を指定してください。</p></span>
          </vs-col>
        </vs-row>
        <div class="m-3 border-cell p-5">
          <form>
            <vs-row>
              <vs-col vs-w="3" class="text-left pr-3 pt-3">調整単位：</vs-col>
              <vs-col vs-type="" vs-w="4">
                <v-select
                  class="ajust-type-label"
                  name="adjustUnit" 
                  :value="selAdjustUnit"
                  :options="adjustUnitList" 
                  :clearable="false" 
                  :searchable ="false"
                  :disabled="disableAdjustUnit"
                  @input="onChangeAdjustUnit"></v-select>
              </vs-col>
              <vs-col class="mt-3 minutes-label" vs-w="5"> 分</vs-col>
            </vs-row>
            <vs-row class="mt-3">
              <vs-col vs-w="3" class="text-left pr-3 pt-3">調整方法：</vs-col>
              <vs-col vs-type="" vs-w="9" class="ajust-mode-label">
                <vs-row>
                  <label class="ajust-type-radio">
                    <vs-radio class="mb-2 mt-2" vs-value="1" vs-name="adjustType" v-on:change="onChangeAdjustType" v-model="adjustType">切上げ</vs-radio>
                  </label>
                </vs-row>
                <vs-row>
                  <label class="ajust-type-radio">
                    <vs-radio class="mb-2 mt-2" vs-value="2" vs-name="adjustType" v-on:change="onChangeAdjustType" v-model="adjustType">切捨て</vs-radio>
                  </label>
                </vs-row>
                <vs-row>
                  <label class="ajust-type-radio">
                    <vs-radio class="mb-2 mt-2" vs-value="0" vs-name="adjustType" v-on:change="onChangeAdjustType" v-model="adjustType">調整なし</vs-radio>
                  </label>
                </vs-row>
              </vs-col>
            </vs-row>
        ...
        <vs-row class="pt-6 pr-3" vs-type="flex" vs-justify="flex-end" vs-align="center">
          <vs-button class="square mr-2 " color="primary" type="filled" v-on:click="exportToNewEdition">作成</vs-button>
          <vs-button class="square mr-0" color="#bdc3c7" type="filled" v-on:click="cancelAjustWorkHours">閉じる</vs-button>
        </vs-row>
      </vs-popup>
      ...
      <vs-table class="mt-3" noDataText="データがありません。" :data="listSave"  stripe sst>
        ...
        <template slot-scope="{data}">
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data" >
            ...
            <td v-if="tr.work_start_time != ''">{{tr.work_start_time | moment("HH:mm")}}</td>
            <td v-else></td>
            <td v-if="tr.work_end_time != ''">{{tr.work_end_time}}</td>
            <td v-else></td>
...
export default {
  ...
  data() {
    return {
      ...
      totalWorkingTime: 0,
      totalOverTime: 0,
      totalHolidayWorkingTime: 0,
      totalMidNightWorkingTime: 0,
      dispTotalWorkingTime: '',
      dispTotalOverTime: '',
      dispTotalHolidayWorkingTime: '',
      dispTotalMidNightWorkingTime: '',
      adjustUnit:15,
      selAdjustUnit:15,
      disableAdjustUnit: false,
      adjustUnitList: [5, 10, 15, 30, 60],
      adjustType: "1",
      showExportPopup: false,
      showAdjustWorkHoursPopup: false,
      ...
      workMonth: this.$moment(this.$route.params.param +"02", "YYYYMMDD").toDate(),
  ...
  created() {
    ...
    this.addLogOperation({action: 'hr-work-detail-access-screen', result: 0});
  },
  computed: {
    hasMidnight(){
  ...
  methods: {
     ...mapActions({
          ...
          getHrTimeCardDetailByWorkingMonth: "hr/getHrTimeCardDetailByWorkingMonth",
          exportWorkListToCSV: "timeCardDetail/exportWorkListToCSV",
          exportToPDF: "hr/exportToPDF",
          addLogOperation: "logOperation/addLog",
          getWorkDetailHrInfo: "hr/getWorkDetailHrInfo" 
      }),
      ...
      async editWorkListDetail(tr) {
        ...
        this.workingDaySelected = this.$moment(tr.work_date, "MM月DD日").format("YYYYMMDD");
        ...
        if(!tr.id){
          ...
          let hrTimeCardDetail = await this.getWorkDetailByTimecard(this.detailIdDialog);
        ...
        this.$validator.validate().then(async valid => {
          if(valid){
            if(id){
              ...
              await this.createWorkDetailByTimecard(data);
              this.modalEditWorkListDetail = false;
            }
      ...
      onChangeAdjustType()
      {
        if (this.adjustType == "0"){
          this.selAdjustUnit = "";
          this.disableAdjustUnit = true;
        } else {
          this.selAdjustUnit = this.adjustUnit;
          this.disableAdjustUnit = false;
        }
      },
      onChangeAdjustUnit(val) {
        this.adjustUnit = val;
        this.selAdjustUnit = val;
      },
      createWorkListDetail(){
        var working_month = this.$route.params.param;
    ...
    async getWorkListDetail(){
        var working_month = this.$route.params.param;
        this.workDetail = await this.getWorkDetailByWorkingMonth(working_month);
        this.timeCard = await this.getHrTimeCardDetailByWorkingMonth(working_month);
        // シフト勤務情報を取得（シフトフラグ、規定業務開始時刻～シフト3終了時刻、シフト出勤区分）
        this.workUserHrInfo = await this.getWorkDetailHrInfo({id:0});  // id: Reserve param
        const myDate = this.$moment(working_month, "YYYYMM").daysInMonth();
        ...
        if(countApprovalState == myDate){
          this.buttonSendStatus = 1;
        }
        this.totalWorkingTime = 0;
        this.totalOverTime = 0;
        this.totalHolidayWorkingTime = 0;
        this.totalMidNightWorkingTime = 0;
        for (var i = 1; i <= myDate; i++){
          ...
          if(Object.prototype.hasOwnProperty.call(arrFullWorkDetail, date)){
            ...
            if(arrFullWorkDetail[date].working_time != null){
              if(arrFullWorkDetail[date].working_time != 0){
                ...
                this.totalOverTime                = this.totalOverTime + arrFullWorkDetail[date].overtime;
                this.totalHolidayWorkingTime      = this.totalHolidayWorkingTime + arrFullWorkDetail[date].holiday_working_time;
                this.totalMidNightWorkingTime     = this.totalMidNightWorkingTime + arrFullWorkDetail[date].midnight_time;
            ...
            if(arrFullWorkDetail[date].work_start_time == null && arrFullWorkDetail[date].work_end_time == null
                && arrFullWorkDetail[date].working_time == 0 && arrFullWorkDetail[date].break_time == 0
                && arrFullWorkDetail[date].paid_vacation_flg == 0 && arrFullWorkDetail[date].sp_vacation_flg ==0 && arrFullWorkDetail[date].day_off_flg == 0){
              this.listSave[i-1].working_time = '';
          ...
          //check button Edit condition
          if(this.timeCard){
            if(this.timeCard.submission_state == 1){
              this.listSave[i-1].buttonStatus = 1;
              this.buttonSendStatus = 1;
            }
          }
        }
        this.getDispTotalWorkingTime();
    },
    getDispTotalWorkingTime() {
      let workingTime = this.convertHoursMinutesFromMinutes(this.totalWorkingTime);
      this.dispTotalWorkingTime = "総勤務時間：" + workingTime.hours + " 時間 " + workingTime.minutes + " 分";
      let overTime = this.convertHoursMinutesFromMinutes(this.totalOverTime);
      this.dispTotalOverTime = "残業時間：" + overTime.hours + " 時間 " + overTime.minutes + " 分";
      let holidayWorkingTime = this.convertHoursMinutesFromMinutes(this.totalHolidayWorkingTime);
      this.dispTotalHolidayWorkingTime = "休日稼働時間：" + holidayWorkingTime.hours + " 時間 " + holidayWorkingTime.minutes + " 分";
      let midNightWorkingTime = this.convertHoursMinutesFromMinutes(this.totalMidNightWorkingTime);
      this.dispTotalMidNightWorkingTime = "深夜時間：" + midNightWorkingTime.hours + " 時間 " + midNightWorkingTime.minutes + " 分";
    },

    convertHoursMinutesFromMinutes(minute){
      let trhours = 0;
      let trminutes = 0;
      if(minute > 0){
        let wkTimeHour = (minute / 60);
        trhours = Math.floor(wkTimeHour);
        let tminutes = (wkTimeHour - trhours) * 60;
        trminutes = Math.round(tminutes);
      }
      if (trhours < 10){
        trhours = '0' + trhours;
      }
      if (trminutes < 10){
        trminutes = '0' + trminutes;
      }
      return {
        'hours' : trhours,
        'minutes' : trminutes,
    ...
    async getWorkMonth () {
      ...
      let params = currentUrl.split('work-detail/');
      this.workMonthExport = params[1].replace('-', '');
    },
    onSelectAdjustWorkHours: async function() {
      this.addLogOperation({action: 'hr-work-detail-open-ajust-hours', result: 0});
      this.showAdjustWorkHoursPopup = true;
    },
    onSelectExport: async function() {
      this.addLogOperation({action: 'hr-work-detail-open-export-csv', result: 0});
    ...
    exportHrWorkListToCSV: async function() {
      ...
      let workListExport =  await this.exportWorkListToCSV(params);
    ...
    async exportToNewEdition() {
      this.showAdjustWorkHoursPopup = false;    
      this.$store.dispatch('updateLoading', true);
      let circular_id = await this.exportToPDF(
        {
          "selectMonth": this.$moment(this.workMonth).format("YYYYMM"),
          "selectAdjustUnit": this.adjustUnit,
          "selectAdjustType": this.adjustType,
        }
      );
      this.$store.dispatch('updateLoading', false);
      if(circular_id && typeof(circular_id) === 'number'){
        this.$router.push('/saves/'+circular_id);
      }
    },
    ...
    cancelAjustWorkHours: async function() {
        this.showAdjustWorkHoursPopup = false;
    },
  ...
  watch:{
    workMonth: async function(val) {
      if (val) {
        ...
        var param = this.$moment(curMonth).format("YYYYMM");
          this.addLogOperation({action: 'hr-work-detail-change-work-date', result: 0});
        await router.push({path: `/hr/work-detail/${param}`});
      }
    },
