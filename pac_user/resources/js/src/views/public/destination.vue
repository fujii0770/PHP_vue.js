                    ...
                    <vx-card :hideLoading="true" class="h-full">
                        ...
                        <div class="mail-steps">
                            <div :class="[allowAddDestination && (!$store.state.home.usingPublicHash || ($store.state.home.usingPublicHash && userHashInfo && !userHashInfo.is_external)) ? 'mail-list' : 'mailListViewOnly', (!selectUsersDisplay || selectUsersDisplay.length == 1 || !selectUsers[0].return_flg)?'not-return':'']">
                                ...
                                <vs-row v-if="selectUsersDisplay && selectUsers.length > 0" vs-type="flex"
                                    ...
                                    <vs-col vs-w="6" class="child1st-block">
                                        <vs-row vs-justify="center" vs-type="flex"
                                                v-for="(user, index) in selectShowUsersDisplay(selectUsersDisplay[0])"
                                ...
                                <vs-row
                                        v-for="(group, idx) in (selectShowUsersDisplay(selectUsersDisplay))"
                                        vs-type="flex"
                                    ...
                                    <vs-col vs-w="6" class="child-block">
                                        <vs-row
                                                v-for="(user, index) in selectShowUsersDisplay(selectUsersDisplay[idx + 1])"
                                                vs-type="flex"
                                                vs-justify="center"
                                                :key="user.email + index + changeTimes">
                                            <vs-col vs-w="10"
                                                    ...
                                                    vs-align="flex-start"
                                                    v-if="!user.plan_users || user.plan_users.length<=0"
                                                    :class="['item child-order item-draggable', (idx + 1) % 3 == 1 ? 'bg-orange': ((idx + 1) % 3 == 2 ? 'bg-pinkle' : 'bg-green'), index + 1 === 1 ? 'first' : '', (index + 1 === selectUsersDisplay[idx + 1].length-1 && index + 1 >1) ? 'last': '', (index + 1 === 1 && selectUsersDisplay[idx + 1].length > 2) ? 'return-flg': '']">
                                                <span>{{user.name || '社員'}} - {{ user.mst_company_name}}</span>
                                                <span>【{{user.email}}】</span>
                                                <span v-if="selectUsersDisplay.length === (idx + 2) && selectUsersDisplay[idx + 1].length === (index + 2)"
                                                      class="final"> 最終</span>
                                            </vs-col>
                                          <!--PAC_5-1698 S-->
                                          <vs-col
                                            ...
                                            <span
                                                v-if="selectUsersDisplay.length === 1 && selectUsersDisplay[0].length === (index + 2)"
                                                class="final"> 最終</span>
                                            <template v-for="(user, index) in user.plan_users">
                                              <a class="currentUser-flg" :key="index + user.name"
                                                 v-if="circularUserLastSend && circularUserLastSend.id === user.id"
                                                 href="#"><i class="far fa-flag"></i></a>
                                              <!--                                                    <a v-if="circular && allowAddDestination && user.id != circularUserLastSend.id && circularUserLastSend.parent_send_order === user.parent_send_order && user.child_send_order > circularUserLastSend.child_send_order  && user.child_send_order > 1 && !isTemplateCircular"-->
                                              <!--                                                       href.prevent v-on:click="onRemoveCircularUser(user.id)"-->
                ...
                <div :class="'vx-col mb-4 lg:pr-0 ' + (allowAddDestination && (!$store.state.home.usingPublicHash || ($store.state.home.usingPublicHash && userHashInfo && !userHashInfo.is_external)) ? 'w-full lg:w-1/2 ': 'w-full lg:pr-4')">
                    ...
                    <vx-card :hideLoading="true" id="mail-steps-card">
                        <div class="mail-steps">
                            <div :class="[allowAddDestination && (!$store.state.home.usingPublicHash || ($store.state.home.usingPublicHash && userHashInfo && !userHashInfo.is_external)) ? 'mail-list' : 'mailListViewOnly', (!selectUsersDisplay || selectUsersDisplay.length == 1 || !selectUsers[0].return_flg)?'not-return':'']">
                                <vs-row v-if="selectUsersDisplay && selectUsers.length > 0" vs-type="flex"
                                        :class="['group applicant', (selectUsersDisplay && selectUsersDisplay[0] && selectUsersDisplay[0].length > 2) ? 'return-flg': '']">
                                    <vs-col vs-w="12">
                                        <vs-row vs-type="flex" vs-justify="center">
                                            <vs-col vs-w="12" class="item me" vs-type="flex" vs-align="flex-start">
                                                <div class="pl-2 item-div" style="width:25%;">{{selectUsers[0].name}}
                                                </div>
                                                <div class="pl-2 item-div" style="width:60%;">
                                                    {{selectUsers[0].email}}
                                                </div>
                                            </vs-col>
                                        </vs-row>
                                    </vs-col>
                                    <vs-col vs-w="12" class="child1st-block">
                                        <vs-row vs-justify="center" vs-type="flex"
                                                v-for="(user, index) in selectShowUsersDisplay(selectUsersDisplay[0])"
                                                :key="user.email + index + changeTimes"
                                            ...
                                            <vs-col
                                                    ...
                                                    vs-type="flex"
                                                    vs-align="flex-start"
                                                    :class="['item child-order item-draggable mt-6', index + 1 === 1 ? 'first' : '', (index + 1 === selectUsersDisplay[0].length-1 && index + 1 >1)  ? 'last': '',  (index + 1 === 1 && selectUsersDisplay[0].length > 2) ? 'return-flg': '']">
                                                <div class="pl-2 item-div" style="width:25%;">{{user.name ||
                                                    '社員'}}
                                                </div>
                                                <div class="pl-2 item-div" style="width:60%;"> {{user.email}}</div>
                                                <span v-if="selectUsersDisplay.length === 1 && selectUsersDisplay[0].length === (index + 2)"
                                                      class="final"> 最終</span>
                                            </vs-col>
                                        </vs-row>
                                ...
                                <vs-row
                                        ...
                                        vs-type="flex"
                                        vs-align="space-around"
                                        :class="['group parent-block', (selectUsersDisplay[idx + 1].length > 2) ? 'return-flg': '', (selectUsersDisplay[idx + 1][0].id == circularUserLastSend.id && isNotReturnCircular)?'not-return':'', (selectUsersDisplay[idx + 1][0].id != circularUserLastSend.id && !selectUsersDisplay[idx + 1][0].return_flg)?'not-return':'']"
                                        :key="group+''+idx"
                                        >
                                    <vs-col vs-w="12">
                                        <vs-row
                                            ...
                                            <vs-col vs-w="12"
                                                    ...
                                                    vs-align="flex-start"
                                                    class="item mt-6">
                                                <div class="pl-2 item-div" style="width:25%;">
                                                    {{selectUsersDisplay[idx + 1][0].name}}
                                                </div>
                                                <div class="pl-2 item-div" style="width:60%;">
                                                    {{selectUsersDisplay[idx + 1][0].email}}
                                                </div>
                                                <span v-if="selectUsersDisplay.length === (idx + 2) && selectUsersDisplay[idx + 1].length === 1"
                                                      class="final"> 最終</span>
                                            </vs-col>
                                    ...
                                    <vs-col vs-w="12" class="child-block">
                                        <vs-row
                                                v-for="(user, index) in selectShowUsersDisplay(selectUsersDisplay[idx + 1])"
                                                
                                                vs-type="flex"
                                                vs-justify="center"
                                                :key="user.email + index + changeTimes" style="position: relative;">
                                            <vs-col vs-w="12"
                                                    vs-type="flex"
                                                    vs-align="flex-start"
                                                    :class="['item item-draggable', (idx + 1) % 3 == 1 ? 'bg-orange': ((idx + 1) % 3 == 2 ? 'bg-pinkle' : 'bg-green'), index + 1 === 1 ? 'first' : '', (index + 1 === selectUsersDisplay[idx + 1].length-1 && index + 1 >1) ? 'last': '', (index + 1 === 1 && selectUsersDisplay[idx + 1].length > 2) ? 'return-flg': '']">
                                        ...
                                        <div :class="['full-width', 'child-range-'+(idx + 1)]"
                                            ...
                                            <vs-row                         
                                                    ...
                                                    vs-type="flex"
                                                    vs-justify="space-around"
                                                    :key="user.email + (index + 1) + changeTimes" style="position: relative;">
                                                <vs-col vs-w="12"
                                                        vs-type="flex"
                                                        vs-align="flex-start"
                                                        :class="['item item-draggable', (idx + 1) % 3 == 1 ? 'bg-orange': ((idx + 1) % 3 == 2 ? 'bg-pinkle' : 'bg-green'), index + 1 === 1 ? 'first' : '', (index + 1 === selectUsersDisplay[idx + 1].length-1 && index + 1 >1) ? 'last': '', (index + 1 === 1 && selectUsersDisplay[idx + 1].length > 2) ? 'return-flg': '']">
