...
<div id="main-home" :class="isMobile?'mobile':''">
  <div style="margin-bottom: 15px">
    <vs-row vs-type="flex" vs-align="center">
            ...
            <div style="display: flex;flex-wrap: nowrap;" class="buttonMain" v-else-if=" !showLeftToolbar">
                  <vs-row class="mb-3 buttonMain" vs-align="center" vs-type="flex" vs-justify="center">
    ...
    <vs-col vs-type="flex" vs-align="flex-end" vs-justify="flex-end"  vs-w="6" width="100%">
                    <vs-button id="button8" class="square" style="color:#000;border:1px solid #dcdcdc;" color="#fff" type="filled" v-if="display_temp_edit==1 && noPlaceholder!=1" v-on:click="onTemplateEditConfirmModal"> テンプレート編集</vs-button>
              ...
              <vs-button id="button6" v-if="!$store.state.home.fileSelected && settingLimit && (settingLimit.storage_local || settingLimit.storage_box||settingLimit.storage_onedrive||settingLimit.storage_google||settingLimit.storage_dropbox) && getNamePath != 'create_circular'"
              ...
              <vs-dropdown :vs-trigger-click="is_ipad" v-if="$store.state.home.fileSelected && settingLimit && (settingLimit.storage_local || settingLimit.storage_box||settingLimit.storage_onedrive||settingLimit.storage_google||settingLimit.storage_dropbox)">
                  ...
                  <vs-dropdown-menu v-show="showDownloadBtn">
                      ...
                      <!--  受信時のみ捺印履歴メニューを表示する PAC_5-1467  -->
                        ...
                        <li v-if="isEditScreen" class="vx-dropdown--item"><a class="vx-dropdown--item-link"><vs-radio class="mb-2 mt-2" vs-value="addStampHistory" @click.native.stop="changeStampHistory($event)"   vs-name="radioVal"    v-model="radioVal"  >回覧履歴を付ける</vs-radio></a></li>
                        <li v-if="isEditScreen" v-show="settingLimit.is_show_current_company_stamp" class="vx-dropdown--item"><a class="vx-dropdown--item-link"><vs-radio class="mb-2 mt-2" vs-value="addTextHistory" @click.native.stop="changeTextHistory($event)"   vs-name="radioVal"    v-model="radioVal"  :disabled="countAllTabNum">自社のみの回覧履歴を付ける</vs-radio></a></li>
              ...
              <vs-button id="button3" :style="isEditScreen ? 'display:none':'color:#000;border:1px solid #dcdcdc;'" color="#fff" v-on:click="onSaveFileClick" :disabled="files.length <= 0 || isProcessing" class="square" type="filled"><span><img :src="require('@assets/images/pages/home/draft.svg')" style="width: 17.5px;height: 14px;margin-right: 5px;"></span> 一時保存</vs-button>
    ...
    <template v-if="!showLeftToolbar && isShowButton">
      ...
      <vs-card :class="'work-content ' + (isEditScreen ? 'isEdit':'')">
          <vs-row>
              ...
              <vs-col vs-type="flex" oncontextmenu="return false" :vs-w="stampToolbarActive ? '2.5':'.2'" :class="stampToolbarActive?'right-toolbar vs-con-loading__container':'right-toolbar hide'">
                  <div class="tools stamp-tool-bar" :style="!stampToolbarActive ? 'display:none':'' ">
                        <div v-if="!isEditScreen && !specialButtonDisableFlg" style="height:100%">
                          <vs-tabs class="tab-parent comment-height" v-model="tab_home_info">
                            ...
                            <vs-tab :label="stickyNoteFlg?'コメント/付箋':'コメント'" class="comment-height" >
                              <div class="comments comment-height comment-position">
                                ...
                                <!-- 付箋を選択の場合、コメント記入欄表示不要 -->
                                <div style="position: absolute;bottom:0;width:100%;" v-show="tab_comment_info < 2">
                                <vs-row>
                                    <vs-col vs-w="10">
                        ...
                        <div v-if="isEditScreen" class ="tab-home" >
                                  ...
                                  <div class="mail-list">
                                      ...
                                      <template v-if="!isTemplateCircular && hasPlanCircularUsers.length <= 0">
                                        <vs-row :class="'item sended ' + (index === 0 ? ' maker ':'') + ((circularUserSendBack && user.id === circularUserSendBack.id) ? ' sendback': '')" vs-type="flex"
                                                v-for="(user, index) in showSelectUsers" v-bind:key="user.email + index" :index="index"
                                                >
                                            <vs-col vs-w="10">
                                      ...
                                      <!-- 特設サイト受取側組織名表示 end -->
                                      <!-- 申請者 -->
                                        <vs-row v-if="selectUsers && selectUsers.length > 0 && !specialCircularFlg" class="item unsend" vs-type="flex">
                                          <vs-col vs-w="10">
  ...
  <modal name="upload-modal"
    ...
    <vs-row class="mt-2 upload-button" vs-type="flex" vs-justify="flex-end" vs-align="center" :style="{flexDirection:'row',alignItems:'center'}">
      ...
      <vs-dropdown vs-trigger-click style="font-family: inherit !important;" v-show="!isMobile">
  ...
  <vs-popup classContent="popup-example"  title="確認" :active.sync="confirmTemplateEdit">
    <div>テンプレート編集機能を使用すると現在の画面の捺印はされません。よろしいですか？</div>
    <div>
      <vs-row class="mt-3">
        <vs-col vs-type="flex" vs-align="flex-end" vs-justify="flex-end"  vs-w="12">
          <vs-button @click="onTemplateEdit" color="primary">はい</vs-button>
          <vs-button @click="confirmTemplateEdit=false" color="dark" type="border">キャンセル</vs-button>
        </vs-col>
      </vs-row>
    </div>
  </vs-popup>
  ...
  <modal name="special-site-modal"
    ...
    <vs-row class="mt-6" vs-type="flex" vs-justify="flex-end" vs-align="center">
      <vs-button class="square mr-0" color="primary" type="filled" v-on:click="goBack"> 戻る</vs-button>
    </vs-row>
  </modal>
  ...
  <vs-popup
    ...
    />
      ...
      <!-- 5-277 mobile html -->
      <div id="main-home-mobile" v-if="isMobile" :class="(isMobile?' mobile ':' ')+(showEdit?' edit ':' ')">
          <div style="width:100%;" v-if="selectUsers && selectUsers.length > 0">
          ...
          <div style="width:100%;height:60px;background-color: #f2f2f2;padding-top:20px;" v-if="selectUsers && selectUsers.length > 0">From : {{selectUsers && selectUsers.length > 0 ? selectUsers[0].name : ''}}<span style="float: right;">{{selectUsers[0].create_at | moment("YYYY/MM/DD")}}</span></div>
          <div v-if="!showEdit">
              <div class="tools" :style="!stampToolbarActive ? 'display:none':'' ">
                  <div>
                      <div style="padding: 0">
                          <div v-if="isEditScreen || isMobile">
                              <div>
                              ...
                              <div>
                                  <div class="mail-list">
                                      <vs-row :class="'item sended ' + (index === 0 ? ' maker ':'') + ((circularUserSendBack && user.id === circularUserSendBack.id) ? ' sendback': '')" vs-type="flex" v-for="(user, index) in selectUsers" v-bind:key="user.email + index" :index="index">
                                          <vs-col vs-w="1" style="text-align:center;">
                                              <a v-if="circularUserLastSend && circularUserLastSend.id === user.id" class="ml-1" href="#"> <i class="far fa-flag"></i></a>
                                          </vs-col>
                                          <vs-col vs-w="5">
      ...
      <div v-if="selectUsers && selectUsers.length > 0 && !showEdit" class="btn_dialog">
        <div v-on:click="goToDestination"><div><img :src="require('@assets/images/mobile/approval.svg')"></div><div class="label">承認</div></div>
      ...
      <div v-if="showEdit && file_is_open" :class="'btn_dialog edit' + ( isShowTextFunction?' full':' nofull' ) " @touchstart="handleDialogStart" @touchend="handleDialogEnd" @touchmove="handleDialogMove">

        <div class="btn_dialog_status" v-on:click="handleClickDialogStatus" @touchstart="dragStartDialogStatus" @touchmove="dragMoveDialogStatus" @touchend="dragEndDialogStatus" v-if="isShowTextFunction">
          <span :class="'status_name '+(isTextSelected?'text':'')">{{ isTextSelected ? 'T' : '印' }}</span> <span class="status_icon"><i style="color: #0984e3;" class="fa fa-cloud"></i></span> </div>
          <div v-on:click="showStamps"><div class="icon"><img :src="require('@assets/images/mobile/stamp.svg')"></div><div class="label">押印</div>
        </div>

        <div type="input" v-on:click="selectTextCustom(currentPageNo)" v-if="isShowTextFunction">
          <div class="icon"><img :src="require('@assets/images/mobile/text.png')"></div><div class="label">テキスト</div>
        </div>

        <div v-on:click="goToDestination"><div class="icon"><i class="far fa-envelope" style="color:#107fcd;"></i></div><div class="label">次へ</div></div>
      ...
      <div class="work-content">
        ...
        <div v-show="showEdit">
          ...
          <div id="mail_list_box">
            ...
            <!-- Mail step -->
            <div class="mail-list">
                <div :class="'item sended ' + (index === 0 ? ' maker ':'') + ((circularUserSendBack && user.id === circularUserSendBack.id) ? ' sendback': '')" v-for="(user, index) in selectUsers" v-bind:key="user.email + index" :index="index">
                ...
                <div v-if="selectUsers && selectUsers.length > 0" class="item unsend">
                  <div class="mail_list_info">
                    <p class="name">{{selectUsers[0].name}}</p>
                    <p class="email">{{selectUsers[0].email}}</p>
                  </div>
                </div>
            </div>
        ...
        <!--
        <div v-show="showEdit" class="docName" style="text-align: center; margin:0 auto; line-height: 40px;">{{docName}}</div>
        -->
        
        <div v-show="showEdit" :class="'tabSelected ' + ( (( enableAdd || checkNoExistCircularUser) && isFileCreatedByOwn(fileSelected)) ? 'is-remove-icon' : '' )" >
          ...
          <a class="remove-document" v-if="( enableAdd || checkNoExistCircularUser) && isFileCreatedByOwn(fileSelected)" v-show="!specialButtonDisableFlg" v-on:click="onCloseDocumentClick(fileSelected, tabSelected)" ><i  class="close fas fa-times"></i></a>

          <a class="add-document" v-if=" enableAdd || checkNoExistCircularUser" v-show="!specialCircularFlg">
            <p v-tooltip.top-center="'Add File'" v-if="files.length > 0 && files.length < 5" v-on:click="onAddFileClick"> 
        ...
        <div class="confidential" v-if="fileSelected && fileSelected.create_user_id === loginUser.id && fileSelected.enableDelete && companyConfidentialFlg" v-show="!specialButtonDisableFlg">
        ...
        <vs-row vs-type="flex" vs-align="center" vs-justify="center" class="upload-wrapper" v-if="!fileSelected && !file_is_open">
            ...
            <div class="vx-col w-full md:w-1/2 upload-box" id="dropZone" @drop="handleFileSelect" @dragleave="handleDragLeave" @dragover="handleDragOver" v-if="settingLimit && settingLimit.storage_local === 1">
                <label class="wrapper" for="uploadFile">
                <input type="file" ref="uploadFile" multiple accept="application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/msword,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet,application/vnd.ms-excel,application/pdf" id="uploadFile" v-on:change="onUploadFile" />
                <vs-row vs-w="12" vs-type="flex" vs-align="center" vs-justify="center" class="label-choose-file">
                    <label for="uploadFile" class="pb-5"><strong>クリックしてファイルを選択してください</strong></label>
                </vs-row>
                <vs-row vs-type="flex" vs-align="center" vs-justify="center" class="mb-20">
                    <img class="file" :src="require('@assets/images/pdf.png')" alt="pdf">
        ...
        <vs-row v-if="mobilePages.length > 0">
          ...
          <vs-col v-if="showEdit" vs-type="flex">
...
<script>
  ...
  export default {
      ...
      beforeRouteLeave (to, form, next) {
          this.$store.commit('template/setDisplayTempEdit', 0);
          next();
        },
      data() {
          return {
      ...
      computed: {
          ...mapState({
              ...
              noPlaceholder: state => state.template.no_placeHolder,
              isStickySelected: state => state.home.StickySelected,
              noteHomeSelected: state => state.home.noteHomeSelected,
          ...
          showSelectUsers () {
            return this.selectUsers.filter((item) => (!this.specialCircularFlg || (this.specialCircularReceiveFlg || !item.special_site_receive_flg)))
          },
          stamps: {
          ...
          isHasOtherUser: {
              get() {
                  return this.circular && this.circular.users && this.circular.users.find(item => {
                      return item.child_send_order != 0 &&  this.loginUser.email == item.email;
                  });
              }
          },
          isCreateScreen: {
          ...
          companyLimit() {
              return {
                text_append_flg: (this.circular && this.circular.limit_text_append_flg) ? this.circular.limit_text_append_flg : 0,
              };
          },
          printedStampCount(){
      ...
      methods: {
          ...mapActions({
              ...
              getTemplatesEdit: "template/getTemplatesEdit",
              setHomeFiles: "template/setHomeFiles",
              saveTemplateEditStamp: "home/saveTemplateEditStamp",
              ...
              removeStickyAll: "home/removeStickyAll",
          }),
          ...
          onTemplateEditConfirmModal: function() {
            this.confirmTemplateEdit = false;
            this.confirmTemplateEdit = true;
          },
          ...
          goToApplication: async function () {
               if (this.settingLimit.require_print===1 && this.printedStampCount==0 && !this.specialButtonDisableFlg){
                  this.$vs.dialog({
                      type: 'alert',
          ...
          goToDestination: function () {
            ...
            setTimeout(()=>{
              ...
              var namePath = this.$route.name;
                if(namePath == 'expense_received_detail'){
                    this.$router.push('/expense/destination');
                }else
                {
          ...
          goBack: function() {
             ...
             }else if(namePath == 'expense_received_detail'){
               this.$route.meta.isKeep=true;
               this.$route.meta.keepReading=true
          ...
          onTemplateEdit: async function () {
              this.confirmEdit = false;
              this.confirmTemplateEdit = false;
              this.$store.commit('template/setCirularTemplateEditFlg', true);
              this.$store.commit('template/setStampUsed', this.stampUsed);
              this.$store.commit('template/templateCurrentParentSendOrder', this.$store.state.home.parent_send_order);
              let queries = {
                circularId: this.$route.params.id
              };
              const templateInfo = await this.getTemplatesEdit(queries);
              setTimeout(() => {
                this.setFiles(templateInfo);
                this.$router.push('/template/update/'+this.$route.params.id);
              }, 300);
            },
          ...
          async onDownloadFile(){
             const downloadFile = async ()=>{
                ...
                if(action[this.$route.name]){
                  if(ret){
                    this.addLogOperation({ action: action[this.$route.name], result: 0, params:{filename: this.fileSelected.name}});
                    ...
                    this.addLogOperation({ action: action[this.$route.name], result: 1, params:{filename: this.fileSelected.name}});
                  }
                }
              }
            await downloadFile();
          ...
          acceptConfirmDelete: async function() {
            ...
            if(action[this.$route.name]){
              this.addLogOperation({ action: action[this.$route.name], result: ret ? 0 : 1, params:{filename: this.deleteItem.name,circular_id: this.circular? this.circular.id:this.$router.currentRoute.params.id}});
            }
            ...
            if (!this.circular && this.$router.currentRoute.params.id) {
              this.$router.push('/')
            }
          ...
          beforeClose: function (event) {
            // モーダルの範囲外クリック時にダイアログをキャンセルできるように修正
          ...
          onPageBreaksPreview: function () {
            this.$vs.dialog({
              ...
              accept: () => {
                ...
                const currentPath = this.$router.currentRoute.path;
                const createNew = !this.circular || (!currentPath.startsWith("/received/") && !currentPath.startsWith("/saves/"));
                const queryObj = { create_new: createNew };
                if (createNew && this.circular && this.circular.id) {
                  queryObj.circular_id = this.circular.id;
                }
          ...
          storageListenerMethod: function(e) {
            console.log(e);
          },
          onConfidentialFlgLabelClick: function() {
            ...
            setTimeout(()=>{
              ...
              if (editor) editor.loadTexts(true);
            },10);
          },
        ...
        confirmSave (event) {
          ...
          if( false == this.$store.state.home.closeCheck ) {
            return;
          }
          if( 0 == this.$store.state.home.files.length ) {
        ...
        haveFiles: function() {
          ...
          addAttachment: async function () {
              ...
              if (this.attachmentAfterUploads.length <= 0 || this.getNamePath == 'create_circular') {
                  ...
                  const ret = await this.getAttachment(this.circular.id);
        ...
        // PAC_5-2150 印面リスト初期デフォルト選択 Start
        ...
        async isPrintStamp(id){
              let history = await this.getStampInfos(id)
        ...
        // PAC_5-1576 E
        MobileOpenFlatpickr: function(selectedDates, dateStr, instance){
          $('.flatpickr-calendar').addClass('isMobile');
        },

        MobileCloseFlatpickr: function(selectedDates, dateStr, instance){
        ...
        onDownloadReserve: async function(){
          ...
          const downloadFile = async ()=>{
            this.editFileAndSignature({stampDisplays: this.stampUsed});
            let ret = await this.downloadPreviewFile(info);
      ...
      watch: {
          ...
          "$store.state.home.fileSelected": async function(newVal) {
            ...
            if (newVal) {
              ...
              this.disabledUndo = this.$store.state.home.fileSelected.actions && this.$store.state.home.fileSelected.actions.length <= 0;
          ...
          "isEditScreen":function (newIndex){
            if(newIndex === true){
        ...
        // "$router.currentRoute.params": function(newVal, oldVal) {
        //   if (!this.circular.id) {
        //     this.$router.push('/')
        //   }
        // },
      ...
      async created() {
        ...
        if(this.$route.path.indexOf('received') >= 0 && ( (path_param.length > 1 && path_param[1] !== 'back=true' && path_param[1] !== 'back=false') || (path_param.length > 12))){
          let data = {
            hash  : path_param[1],
        ...
        this.opacity = this.userInfo.default_opacity;
            ...
            if (cir_info === "印鑑") {
                this.tab_cir_info = 0;
            } else if (cir_info === "回覧先") {
                this.tab_cir_info = 1;
            } else if (cir_info === "コメント/付箋") {
                this.tab_cir_info = 2;
        ...
        if (this.$store.state.home.circular){
            ...
            this.groupName = this.circular.special_site_group_name;
              if (this.specialCircularFlg && !this.files[0].server_file_path) {
        ...
        if(action[this.$route.name])
        this.addLogOperation({ action: action[this.$route.name], result: 0});
        ...
        if(this.circularUserLastSend && this.isEditScreen && (this.circularUserLastSend.circular_status === this.CIRCULAR_USER.NOTIFIED_UNREAD_STATUS)) {
          ...
          (async () => {
            ...
            if(this.settingLimit && this.settingLimit.default_stamp_history_flg == 1 && ('/' != this.$route.path  && -1 == this.$route.path.lastIndexOf('saves') )){
                this.addStampHistory = true
                this.radioVal = "addStampHistory";
            }
          ...
          (async () => {
              let company = null;
              // PAC_5-842　対応　▼
              company = this.$store.state.groupware.myCompany && this.$store.state.groupware.myCompany ? this.$store.state.groupware.myCompany : []
              if(!company || ((new Date().getTime()) -  company.currentTime > 600000 )){
                  company = await Axios.get(`${config.BASE_API_URL}/setting/getMyCompany`)
                      .then(response => {
                          return response.data ? response.data.data: [];
                      })
                      .catch(() => { return []; });
              ...
              // 所属会社情報取得
              //テンプレート編集フラグ取得
              if(company.template_flg == 1 && company.template_edit_flg == 1) {
                this.display_temp_edit = await this.getCircularTempEdit({circular_id:this.$route.params.id});
                this.$store.commit('home/setTemplateEditFlg', this.display_temp_edit);
          ...
          (async () => {
              this.$nextTick(() => {
                  if (this.isMobile || (this.getNamePath !== 'create_circular' && this.window.width >= 1200)) this.stampToolbarActive = true;
                  if (this.getNamePath !== 'create_circular') {
                    const $this = this;
                    $this.stamp_load_flg = false;
                    $this.$vs.loading({
        ...
        this.$store.commit('home/homeUnSelectSticky');
