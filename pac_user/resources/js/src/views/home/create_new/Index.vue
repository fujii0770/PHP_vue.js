<template>
    <div>
        ...
        <!-- 5-277 mobile html -->
        <div id="main-home-mobile" class="create_new" v-if="files.length">
        ...
        <div v-if="files.length && showEdit && file_is_open" :class="'btn_dialog tool edit ' + (!isShowTextFunction?'no-text':'')" @touchstart="handleDialogStart" @touchend="handleDialogEnd"> <!--  @touchstart="handleDialogStart" @touchend="handleDialogEnd" -->
          <div class="btn_dialog_status" v-on:click="handleClickDialogStatus" @touchstart="dragStartDialogStatus" @touchmove="dragMoveDialogStatus" @touchend="dragEndDialogStatus" v-if="isShowTextFunction"> <span :class="'status_name '+(isTextSelected?'text':'')">{{ isTextSelected ? 'T' : '印' }}</span> <span class="status_icon"><i style="color: #0984e3;" class="fa fa-cloud"></i></span> </div>
          <div v-on:click="showStamps"><div class="icon"><img :src="require('@assets/images/mobile/stamp.svg')"></div><div class="label">押印</div></div>
          
          <div type="input" v-on:click="selectTextCustom(currentPageNo)" v-if="isShowTextFunction"><div class="icon"><img :src="require('@assets/images/mobile/text.png')"></div><div class="label">テキスト</div></div>
        ...
        <div class="work-content">
          ...
          <div :class="'tabSelected ' + ( (( enableAdd || checkNoExistCircularUser) && isFileCreatedByOwn(fileSelected)) ? 'is-remove-icon' : '' )" v-if="files.length && showEdit">
            <div>
              <select v-model="tabSelected" @change="changeSelectedFile(tabSelected)" ref="selectBoxFile" >
      ...
      <div id="main-home" :class="'mobile create_new ' + (!files.length?'':'is_file')" v-if="!file_is_open || files.length == 0">
              <vs-card :class="'work-content ' + (isEditScreen ? 'isEdit':'')" v-show="!$route.params.id || !file_is_open">
                  <vs-row>
                              ...
                              <vs-col vs-type="flex" vs-w="12" vs-align="flex-start" vs-justify="flex-start">
                                  <vs-navbar v-model="tabSelected"
                                    ...
                                    <vs-navbar-item index="99999" class="add-document" v-if=" enableAdd || checkNoExistCircularUser" v-show="!specialCircularFlg">
                                          <p v-if="files.length > 0 && files.length < 5" v-on:click="onAddFileClick"> <i class="fas fa-plus"></i></p>
                                      </vs-navbar-item>
                                      <vs-spacer></vs-spacer>
                                  </vs-navbar>
                              </vs-col>
                                ...
                                <vs-tabs class="tab-parent comment-height" v-model="tab_home_info">
                                  <vs-tab label="印鑑">
                                    <div class="stamps-processing" style="position:relative;z-index:100;border:1px solid #dcdcdc;border-radius:30px;padding-left:15px;padding-right:15px;background-color:#000000;width:max-content;margin:auto;text-align:center;user-select:none;-moz-user-select:none;-webkit-user-select:none;-ms-user-select:none;-khtml-user-select:none;">
                                      ...
                                      <!--<vs-button  id="button09" class="square" style="background-color:#000000;" color="#ffffff" type="filled" @click="onStampsConfirmation" :disabled="disabledConfirmation"><i class="fas fa-check" style="color:#ffffff;margin-right: 5px;"></i> 決定</vs-button>-->
                                    </div>
                                    ...
                                    <vs-row vs-type="flex" vs-align="center" vs-justify="center" vs-w="12" style="padding: 10px;position: relative">
                                      ...
                                      <!--                                    <vs-button class="square" color="primary" type="filled" style="width: 100%;margin-right: 0;z-index: 2" :disabled="!$store.state.home.isUpdateStampOrder" v-on:click="onSaveStampsOrder">並び順変更</vs-button>&ndash;&gt;-->
                                              ...
                                              <vs-row :class="'item sended ' + (index === 0 ? ' maker ':'') + ((circularUserSendBack && user.id === circularUserSendBack.id) ? ' sendback': '')" vs-type="flex" v-for="(user, index) in hasPlanCircularUsers" v-bind:key="user.email + index" :index="index">
                                                  <template v-if="!user.plan_users">
                                                      <vs-col vs-w="10">
                                                          <p>{{user.name}}<span class="final" v-if="user.circular_status === CIRCULAR_USER.SEND_BACK_STATUS">差戻し</span></p>
                                                          <p>{{user.email}}</p>
                                                          <!--   <p>{{ arrBtnTitleAccount[user.env_flg][user.edition_flg] }}</p>-->
                                                      </vs-col>
                                                      <vs-col  vs-type="flex" vs-w="2"  vs-justify="flex-end" vs-align="center">
    ...
    export default {
        ...
        computed: {
            ...
            printedStampCount(){
              ...
              if (this.$route.name=='create_new_detail'){
        ...
        methods: {
            ...
            acceptConfirmDelete: async function() {
              ...
              if( this.files.length == 0 ) this.$router.push('/create'); 
          ...
          onUploadAttachmentFromCloud: async  function (fileId, filename) {
            ...
            async validatHasAttachment(circular_id) {
                ...
                if(ret.length > 0){
                  const dialogText = "添付されたファイルを確認される際はPC版でご確認ください";
                  this.$vs.dialog({
                    type: 'alert',
                    color: 'danger pre-msg',
                    title: `メッセージ`,
        ...
        async created() {
          ...
          this.isShowTextFunction = this.settingLimit?this.settingLimit.text_append_flg==1:false;
        },
        beforeDestroy() {
          // 取得を止めるため
          this.visiblePageRange = [-1, -1];
