    ...
    <div>
        <div id="sends-page" style="position: relative;" :class="isMobile?'mobile':''">
            <vs-row>
                <vs-col :vs-w="showReading?9:11.5" vs-xs="12" :vs-sm="showReading?7:11.5"
                    ...
                    <vs-card style="margin-bottom: 0">
                        <vs-row>
                            ...
                            <vs-col vs-type="flex" vs-lg="2" vs-sm="6" vs-xs="12" class="mb-3 pr-2">
                                <vs-select class="selectExample w-full" v-model="filter.state" label="回覧状態">
                                    <vs-select-item :key="index" :value="index" :text="date" v-for="(date, index) in circular_state"/>
                                </vs-select>
                            </vs-col>
                            <vs-col vs-type="flex" vs-lg="2" vs-sm="6" vs-xs="12" class="mb-3 pr-2">
                                <vs-select class="selectExample w-full" id="finishedDate" v-model="filter.finishedDate" label="完了日時">
                                    <vs-select-item class="finishedDate" :key="index" :value="index" :text="date" v-for="(date, index) in finishedTimeList"/>
                                </vs-select>
                            </vs-col>
                            <vs-col vs-type="flex" vs-lg="2" vs-sm="6" vs-xs="12" class="mb-3 pr-2">
                            </vs-col>
                    ...
                    <vs-card>
                        <vs-dropdown>
                            <vs-button id="button5" class="square mb-2 sm:mb-2 md:mb-0 lg:mb-0 xl:mb-0" color="primary" type="filled" v-bind:disabled="selected.length === 0 || !this.circular_type"><i class="fas fa-download"></i> ダウンロード予約</vs-button>
                            <vs-dropdown-menu v-show="selected.length > 0 && this.circular_type" class="download-complete">
                        ...
                        <vs-button v-if="canStoreCircular" class="square mb-2 sm:mb-2 md:mb-0 lg:mb-0 xl:mb-0"
                                   color="primary" v-on:click="onSaveLongtermModal"
                                   v-bind:disabled="selected.length === 0 || !this.circular_type">長期保管
                        </vs-button>
                        ...
                        <vs-table class="mt-3 custome-event" :data="compListData" noDataText="データがありません。"
                            ...
                            <template slot-scope="{data}">
                                <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
                                    ...
                                    <td @click="onShowReading(tr)">{{ tr.state }}</td>
                                    <td class="max-width-200" @click="onShowReading(tr)">{{ tr.file_names }}</td>
                                    <td @click="onShowReading(tr)">
                                        <div v-html="tr.sender"></div>
                                    </td>
                                    <td @click="onShowReading(tr)"> 社内 {{ tr.access_code }}<br/> 社外
                                        {{ tr.outside_access_code }}
                                    </td>
                        ...
                        <div>
                            <div class="mt-3">{{ pagination.totalItem }} 件中 {{ pagination.from }} 件から {{
                                    pagination.to
                                }} 件までを表示
                            </div>
                        </div>
                ...
                <vs-col class="preview-wrapper" :vs-w="showReading?3:0.5" :vs-xs="showReading?12:0"
                        :vs-sm="showReading?5:0.5">
                    <div v-if="showReading" style="">
                        <div style="height: calc(100vh - 60px);  flex-direction: column;background: #fff"
                             class="show-flex">

                            <div class="button2 flex-item ml-3" @click="showReading=false"
                                 style="cursor: pointer; position: relative;">
                                <i class="fas fa-caret-right"
                                   style="font-size: 40px; color: rgba(var(--vs-primary),1);"></i>
                                <div class="text" style="position: absolute; top: 10px; left: 20px;">閉じる</div>
                            </div>

                            <template>
                                <vs-card v-if="itemReadingDetail.circular.first_page_data" class="main-flex-item"
                                         style="position: relative; height: 100%;">
                                    <div class="preview" style="width: 100%; position: absolute; overflow: hidden; height: 100%;">
                                        <img :src="'data:image/jpeg;base64,' + itemReadingDetail.circular.first_page_data"
                                ...
                                <vs-card class="detail flex-item mb-2">
                                    ...
                                    <vs-row class="mt-3">
                                        ...
                                        <vs-col vs-w="8" class="info max-width-360">{{
                                                itemReading.file_names
                                            }}
                                        </vs-col>
                                    </vs-row>
                                    <vs-row>
                                        ...
                                        <vs-col vs-w="8" class="info">{{
                                                options_status[itemReading.circular_status]
                                            }}
                                        </vs-col>
                                    </vs-row>
                                    <vs-row>
                                        <vs-col vs-w="4" class="label">依頼者名</vs-col>
                                        <vs-col vs-w="8" class="info">{{ itemReadingDetail.userSend.family_name }}
                                            {{ itemReadingDetail.userSend.given_name }}
                                        </vs-col>
                                    </vs-row>
                                    <vs-row>
                                        <vs-col vs-w="4" class="label">依頼日時</vs-col>
                                        <vs-col vs-w="8" class="info">
                                            {{ itemReadingDetail.circular.applied_date | moment("YYYY/MM/DD HH:mm") }}
                                        </vs-col>
                                    </vs-row>
                                    ...
                                    <vs-row>
                                        ...
                                        <vs-col vs-w="8" class="info">{{
                                                itemReadingDetail.circular.access_code
                                            }}
                                        </vs-col>
                                    </vs-row>
                                    <vs-row>
                                        ...
                                        <vs-col vs-w="8" class="info">{{
                                                itemReadingDetail.circular.outside_access_code
                                            }}
                                        </vs-col>
                                    </vs-row>
                                    <vs-row class="mt-1">
                                    ...
                                    <div style="max-height: 130px; overflow: auto;">
                                        <vs-row v-for="(userReceive,index) in itemReadingDetail.userReceives"
                                    ...
                                    <div v-if="itemReadingDetail.viewingUser[0]"
                                         style="max-height: 130px; overflow: auto;">
                                        <vs-row v-for="(viewer,index) in itemReadingDetail.viewingUser" :key="index"
                                                class="mt-1">
                                            <vs-col vs-w="8" class="info">
                                            <span>
                                ...
                                <div class="flex-item text-right mb-6">
                                    <vs-button v-if="!itemReadingDetail.circular.origin_circular_url" class="square"
                                               color="primary" @click="toView(itemReading.id)"
                                               :disabled="isEmtyItemReading"> 表示
                                    </vs-button>
                                    <vs-button v-if="itemReadingDetail.circular.origin_circular_url" class="square"
                                               color="primary"
                                               @click="openWindow(itemReadingDetail.circular.origin_circular_url)"
                                               :disabled="isEmtyItemReading"> 表示
                                    </vs-button>
                                </div>
                            </template>
            ...
            <vs-popup classContent="popup-example" title="確認" :active.sync="confirmSaveLongTerm">
                ...
                <div v-if="selected.length===1">
                    <div class="mb-0">選択した文書の長期保管を行います。よろしいですか？</div>
                    <br>
                    ...
                    <div v-if="showTree" style="border: 1px solid rgba(0, 0, 0, 0.1);border-radius: 5px;overflow: auto;height: 250px;">
                      <FolderTree ref="tree" v-show="showTree" :treeId="selectViewingIndexTree" @onNodeClick="setFolderId"></FolderTree>
                    </div>
                </div>
                <vs-row class="mt-3">
                    <vs-col vs-type="flex" vs-align="flex-end" vs-justify="flex-end" vs-w="12">
            ...
            <vs-popup classContent="popup-example" title="選択文書ダウンロード予約" :active.sync="confirmDownload">
                <vs-row>
                    <vs-input class="inputx w-full" label="ファイル名" value="input.filename" v-model="input.filename"
                              :maxlength="inputMaxLength" placeholder="ファイル名(拡張子含め50文字まで。拡張子は自動付与されます。)"/>
                </vs-row>
        ...
        <!-- PAC_5-2384 閲覧一覧をスマホブラウザで表示できるようにしたい 追加　start -->
        <div id="sends-page-mobile" style="position: relative;" :class="isMobile?'mobile':''">
            <vs-row>
                <vs-col :vs-w="showReading?9:(isMobile?12:11.5)" vs-xs="12" :vs-sm="showReading?7:(isMobile?12:11.5)" style="transition: width .2s;">
                    <div class="sends-mobile-title"><h3>閲覧一覧</h3></div>

                    <vs-card style="margin-bottom: 0" class="select-panel">
                        <vs-row>
                            <div style="width:100%" @click="searchAreaFlgMobile=!searchAreaFlgMobile">
                                <vs-col vs-type="flex" :vs-lg="isMobile?12:2" :vs-sm="isMobile?12:6" vs-xs="12" class="mt-3 pr-2">
                        ...
                        <vs-row class="mt-3 search_button" v-show="searchAreaFlgMobile">
                            <vs-col vs-type="flex" vs>
        ...
        </div><!-- PAC_5-2384 閲覧一覧をスマホブラウザで表示できるようにしたい 追加　END -->
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            showReading: false,
            options_status: ['', '回覧中', '回覧完了', '回覧完了(保存済)', '', '', '', '', '', ''],
            received_status: ['未通知', '通知済/未読', '既読', '承認(捺印あり)', '承認(捺印なし)', '差戻し', '差戻し(未読)'],
            circular_state: ['回覧中', '回覧完了'],
            out_status: ['-', '未読', '処理済'],
            itemReading: {},
            ...
            folderId: '',
            folderSelect: false,
            selectViewingIndexTree: 'selectViewingIndexTree',
            showFolderFlg:false,
            radioVal: "default",
    ...
    methods: {
        ...mapActions({
            ...
            search: "circulars/getListViewing",
            getDetailCircularUser: "circulars/getDetailCircularUserForCompleted",
            getOriginCircularUrl: "circulars/getOriginCircularUrlForCompleted",
        ...
        onSearch: async function (resetPaging) {
            ...
            this.circular_type = this.filter.state;
            if(this.isMobile){
        ...
        clearRadioStatus(e){
          ...
          if (e.target.tagName === 'INPUT' || this.input.stampHistory == false) return
        ...
        changeRadioStatus(e) {
            // Because the native click event will be executed twice, the first time on the label tag 
            // and the second time on the input tag, this processing is required
            if (e.target.tagName === 'INPUT' || this.input.stampHistory == true || this.countAllTabNum == true) {
                return
            }
        ...
        showDialogDownload() {
            ...
            if (this.selected.length === 1 && this.selected[0].file_names.split(',').length === 1) {
                // .pdf, .docx, .xlsx
                let pos = this.selected[0].file_names.lastIndexOf('.');
        ...
        onDownloadReserve: async function () {
            ...
            this.input.download_type = this.DOWNLOAD_TYPE.VIEWING_CIRCULAR_DOWNLOAD_RESERVE;//閲覧一覧で予約
        ...
        async onShowReading(tr) {
            ...
            if (this.click === 1) {
                ...
                var time = setTimeout(async function () {
                    ...
                    root.itemReadingDetail = await root.getDetailCircularUser({
                        id: tr.id,
                        finishedDate: root.month
                    });
                ...
                clearTimeout(time);
                this.click = 0;
                var getOrigin = await this.getOriginCircularUrl({id: tr.id, finishedDate: this.month});
                if (getOrigin.originCircularUrl == null) {
                    localStorage.setItem('finishedDate', this.month);
                    this.$router.push('/viewing/' + tr.id);
                } else {
        ...
        },//PAC_5-2384 閲覧一覧をスマホブラウザで表示できるようにしたい 追加
        toView(id) {
            localStorage.setItem('finishedDate', this.month);
            this.$router.push('/viewing/' + id);
        },
        setFolderId(id){
          this.folderId = id;
        },
    ...
    computed: {
        compListData: function () {
            ...
            this.listData.forEach((item) => {
                item.status = this.circular_state[item.circular_kind];
                data.push(item);
            });
            return data;
        },
    ...
    watch: {
        ...
        'filter.state': function () {
            ...
            if (this.filter.state === 0) {
                $('input#finishedDate').attr("disabled", "disabled");
                this.filter.finishedDate = '';
            } else {
                $('input#finishedDate').attr('disabled', false);
                this.filter.finishedDate = 0;
            }
        },
