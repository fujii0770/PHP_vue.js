            ...
            <vs-row class="top-bar">
                ...
                <vs-col vs-type="flex" vs-align="flex-end" vs-justify="flex-end" vs-w="3" vs-xs="12" vs-sm="6">
                    <vs-button class="square w-1/2 sm:w-auto md:w-auto lg:w-auto xl:w-auto" color="primary"
                               type="filled" v-on:click="onBackClick"> 戻る
                    </vs-button>
                    <vs-button class="square w-1/2 sm:w-auto md:w-auto lg:w-auto xl:w-auto" color="danger" type="filled"
                               v-on:click="onSaveMemoClick"><i class="fas fa-check"></i> 保存
                    </vs-button>
                </vs-col>
            </vs-row>
        </vs-card>
                    ...
                    <div class="mail-steps">
                        <div class="mail-list">
                            ...
                            <template v-if="!isTemplateCircular">
                                <vs-row vs-type="flex" v-for="(formatUser, index) in selectUsers"
                                        v-bind:key="formatUser.user.email + index" :index="index"
                                        >
                                    <vs-col vs-w="12" :class="'item sended ' + (index === 0 ? 'maker ':' ')">
                                        <vs-col vs-w="10">
                                    ...
                                    <vs-col vs-w="12"
                                            v-if="currentViewingUser && currentViewingUser.parent_send_order !== 0 && currentViewingUser.parent_send_order == formatUser.user.parent_send_order">
                                        <vs-row :class="'item sended is-child'" vs-type="flex"
                                                v-for="(childUser, childIndex) in formatUser.children"
                                                v-bind:key="childUser.email + childIndex" :index="childIndex">
                                            <vs-col vs-w="10">
        ...
        selectUsers: {
            get() {
                ...
                formatCircularUsers.push(...circularUsers.filter(item => {
                    return (this.currentViewingUser && this.currentViewingUser.parent_send_order === 0) ? item.parent_send_order === 0 : item.child_send_order === 0
                }).map(item => {
