  ...
  <div class="main-article">
    <div v-if="isDispWide">
      ...
      <vs-card class="mx-auto" max-width="100vw">
        ...
        <div class="headline title font-weight-bold">全ての下書き</div>
        <vs-row>
        ...
        <vs-table class="mt-3 table-topic-width"
          ...
          <template slot-scope="{data}">
            <vs-tr :data="item" :key="index" v-for="(item, index) in data">
              ...
              <vs-td style="width:90%" @click="onEditTopicShow(item)">
                <div @click="onEditTopicShow(item)">
                    ...
                    <div>作成日：{{item.created_at ? item.created_at : ''}}</div>
                    <div>更新日：{{item.updated_at ? item.updated_at : ''}}</div>
                </div>
              </vs-td>
            </vs-tr>
          </template>
      ...
      <vs-card class="mx-auto" max-width="100vw">
        <div  v-if="!showSearchBox">
          ...
          <div class="topic_menubar">
            ...
            <category-menu @changeDisp="changeDisp"></category-menu>
          </div>
        ...
        <vs-table class="mt-3 table-topic-width"
          ...
          <template slot-scope="{data}">
            <vs-tr :data="item" :key="index" v-for="(item, index) in data">
              <vs-td vs-w="11" @click="onEditTopicShow(item)">
                <div @click="onEditTopicShow(item)">
                <div>{{ item.title ? item.title:''}}</div>
                <div>掲示期間：{{item.disp_start_date ? item.disp_start_date : ''}}～{{item.disp_end_date ? item.disp_end_date : ''}}</div>                            
                </div>
  ...
  computed: {
    ...
    checkAll: {
      get() {
        ...
        const postFilter = this.bbsTopicList;
...
　    this.pagination.totalItem   = data.total;
    ...
    onEditTopicShow: async function (data) {
        let infoTopic  = {
            procKbn    : '1',
            bbsId      : data.id,
            RecalcFlg  : 0,
        };
        let detailData = await this.getTopicList(infoTopic);
        if (!detailData) {
        ...
        let editData = {
            addFlg:false,
            value: detailData.topic,
            bbsComment : detailData.commentList,
            attachmentsNames:detailData.attachmentsNames
        }
        this.$emit('onAddTopicShow',  editData)
    },
    onAddTopicShow() {
      this.editData = {
          addFlg:true, value:null
      }
