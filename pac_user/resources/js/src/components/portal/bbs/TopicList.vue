<template>
  ...
  <div class="topic-list" v-if="!isMypage">
    <template v-if="isDispWide">
      ...
      <vs-card class="mx-auto" max-width="100vw">
        ...
        <vs-table class="mt-3 table-topic-width" style="height: auto!important;"
          :data="bbsTopicList"
          ...
          <template slot-scope="{data}">
            <vs-tr :data="item" :key="index" v-for="(item, index) in data">
              ...
              <vs-td style="width:90%" @click="onDetailTopicShow(item)">
                ...
                <div>掲示期間：{{item.disp_start_date ? item.disp_start_date : ''}}～{{item.disp_end_date ? item.disp_end_date : ''}}</div>                            
                </div>
              </vs-td>
            </vs-tr>
          </template>
      ...
      <vs-card class="mx-auto" max-width="100vw">
        <div  v-if="!showSearchBox">
          ...
          <div class="topic_menubar">
            ...
            <vs-button color="primary" @click="onAddTopicShow" id="btnt" v-if="boardPermission.topics_append">投稿</vs-button>
  ...
  <div class="topic-list-mobile" v-if="isMypage">
    ...
    <vs-table class="mt-3 table-topic-width"
      ...
      sst stripe>
      <template slot-scope="{data}">
        <vs-tr :data="item" :key="index" v-for="(item, index) in mobileCustomData(data)">
          <vs-td vs-w="12">
            <div class="group_name">{{ item.name }}</div>
            <div @click="onDetailTopic(item)">
              <div class="topic-title">{{ item.title ? item.title:''}}</div>                            
              <div class="topic-created">掲示期間：{{item.disp_start_date ? item.disp_start_date.replaceAll('-', '/') : ''}}</div>                            
            </div>
          </vs-td>
        </vs-tr>
      </template>
...
export default {
  components: { 
    SearchBox: () => import('@/components/portal/bbs/SearchBox'),
  ...
  watch: {
   ...
   created() {
    ...
    // From mypage
    if( this.$route.query && this.$route.query.data && typeof this.$route.query.data == 'object' )  this.onDetailTopicShow(this.$route.query.data);
  ...
  methods: {
    ...
    onSearch: async function (addLog = false) {
      ...
      if (addLog) {
          if (data) {
              this.addLogOperation({action: 'portal-bulletin-search-topic', result: 0})
          } else {
              this.addLogOperation({action: 'portal-bulletin-search-topic', result: 1})
          }
      }
      ...
      this.pagination.to          = data.to;
        ...
        const initPageFlg = this.initPageFlg;
      if (this.bbsTopicList) {
          this.initPageFlg = false;
          if (initPageFlg && data.current_page > 1 && this.bbsTopicList.length == 0 && data.total > 0) {
    ...
    onDetailTopic: function(data){
      this.$router.push({path: '/groupware/bulletin', query: {data: data}});
    },
    ...
    mobileCustomData: function(data){
      return data && data.length ? data.filter((item,index)=> index <= 4 ) : []
    }
  }
}
</script>
