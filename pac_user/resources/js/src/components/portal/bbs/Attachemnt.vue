<template>
  <div style="width: 100%; padding-top: 20px">
    <vs-col cols="12" vs-w="2" vs-xs="12" style="margin-top:40px;">
      <input type="file" ref="fileInput" class="hidden" multiple  id="uploadFile"
             v-on:change="onUploadFile"/>
      <vs-button id="button9" class="square fileselectbtn" :style="(isMobile?'margin-left:12px;':'')" color="#fff" type="filled"
    ...
    <vs-col cols="12" vs-w="10" vs-xs="12">
      <p v-for="(msg , index) in uploadFileError" :key="index" class="text-danger text-sm" style="padding-left:12px;">{{ msg }}</p>
...
export default {
  ...
  methods: {
    ...
    validateFile: function (file) {
      ...
      const target = this.dispAttachments.some(value => value.name === file.name && (value.type == 'add' || value.type == 'history')) || oldInitFiles.some(_ => _ == file.name);
      if (target) {
    ...
    validateCount(){
      ...
      if ((attachmentsCount + this.initFiles.length) > this.companyInfo.bbs_max_attachment_count) {
        this.uploadFileError.push(this.error.uploadFile.count);
        setTimeout(()=>{
          this.uploadFileError.shift();
        },3000)
        return false;
