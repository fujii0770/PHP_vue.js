<template>
  <div ref="wrap" style="position: relative">
      <img v-if="!useKonva"
       :src="imageUrl" alt="a4" style="width: 100%">
      <v-stage v-else
               @touchstart="touchEditorClick" @touchend="confirmTouchEditor" @touchmove="moveToChangePage"
               ref="stage"
               :config="config"
               :draggable="isDraggable"
               @dragstart="handleDragstart"
               @dragend="handleDragend"
               :key="page.no + isConfidential">
          <v-layer ref="layer">
              <v-image :config="{
                  image: createImageFromUrl(imageUrl),
                  id: 1,
                  draggable: false,
                  width: config.width,
                  height: config.height,
                  type: 'Image',
                }">
              </v-image>
          </v-layer>
      </v-stage>
      <!--<div class="text-options" v-if="text" :id="'text-' + text.index" :style="`top:${(text.y * ($store.state.home.fileSelected.zoom / 100) * realScale) - 35}px;left:${((text.x + text.width) * ($store.state.home.fileSelected.zoom / 100) * realScale)  - 45}px`" :key="textIdSelected">-->
          <!--<vs-row vs-type="flex" vs-align="center" vs-justify="center">-->
              <!--&lt;!&ndash;<v-popover class="button config" placement="top"&ndash;&gt;-->
                         <!--&lt;!&ndash;trigger="manual"&ndash;&gt;-->
                         <!--&lt;!&ndash;:open="openPopover"&ndash;&gt;-->
                         <!--&lt;!&ndash;offset="16"&ndash;&gt;-->
                         <!--&lt;!&ndash;:auto-hide="false" >&ndash;&gt;-->
                  <!--&lt;!&ndash;<div class="tooltip-target" v-on:click="openPopover = !openPopover">&ndash;&gt;-->
                      <!--&lt;!&ndash;<i class="fas fa-cog"> </i>&ndash;&gt;-->
                  <!--&lt;!&ndash;</div>&ndash;&gt;-->
                  <!--&lt;!&ndash;<vs-row style="position: relative" slot="popover">&ndash;&gt;-->
                      <!--&lt;!&ndash;<vs-col vs-type="flex" vs-w="8.5" style="border: 1px solid #0984e3">&ndash;&gt;-->
                          <!--&lt;!&ndash;<vs-dropdown class="dropdown" style="display: flex;width: 100%" vs-custom-content vs-trigger-click>&ndash;&gt;-->
                              <!--&lt;!&ndash;<a class="a-icon" href.prevent>&ndash;&gt;-->
                                  <!--&lt;!&ndash;{{getFontName(text.fontFamily)}}&ndash;&gt;-->
                                  <!--&lt;!&ndash;<vs-icon class="" icon="expand_more"></vs-icon>&ndash;&gt;-->
                              <!--&lt;!&ndash;</a>&ndash;&gt;-->

                              <!--&lt;!&ndash;<vs-dropdown-menu >&ndash;&gt;-->
                                  <!--&lt;!&ndash;<vs-dropdown-item v-on:click="onTextChangeFont(text,'MS Mincho')">&ndash;&gt;-->
                                      <!--&lt;!&ndash;ＭＳ 明朝&ndash;&gt;-->
                                  <!--&lt;!&ndash;</vs-dropdown-item>&ndash;&gt;-->
                                  <!--&lt;!&ndash;<vs-dropdown-item v-on:click="onTextChangeFont(text,'MS Gothic')">&ndash;&gt;-->
                                      <!--&lt;!&ndash;ＭＳ ゴシック&ndash;&gt;-->
                                  <!--&lt;!&ndash;</vs-dropdown-item>&ndash;&gt;-->
                                  <!--&lt;!&ndash;<vs-dropdown-item v-on:click="onTextChangeFont(text,'Meiryo')">&ndash;&gt;-->
                                      <!--&lt;!&ndash;メイリオ&ndash;&gt;-->
                                  <!--&lt;!&ndash;</vs-dropdown-item>&ndash;&gt;-->
                              <!--&lt;!&ndash;</vs-dropdown-menu>&ndash;&gt;-->
                          <!--&lt;!&ndash;</vs-dropdown>&ndash;&gt;-->
                      <!--&lt;!&ndash;</vs-col>&ndash;&gt;-->
                      <!--&lt;!&ndash;<vs-col vs-type="flex" vs-w="3" style="margin-left: 1px ; border: 1px solid #0984e3">&ndash;&gt;-->
                          <!--&lt;!&ndash;<vs-dropdown class="dropdown" style="display: flex;width: 100%" vs-custom-content vs-trigger-click>&ndash;&gt;-->
                              <!--&lt;!&ndash;<a class="a-icon" href.prevent>&ndash;&gt;-->
                                  <!--&lt;!&ndash;{{Math.round(text.fontSize/(1.333333333*($store.state.home.fileSelected.zoom / 100)))}}&ndash;&gt;-->
                                  <!--&lt;!&ndash;<vs-icon class="" icon="expand_more"></vs-icon>&ndash;&gt;-->
                              <!--&lt;!&ndash;</a>&ndash;&gt;-->
                              <!--&lt;!&ndash;<vs-dropdown-menu>&ndash;&gt;-->
                                  <!--&lt;!&ndash;<vs-dropdown-item v-on:click="onTextChangeFontSize(text, fontSize)" v-for="(fontSize,index) in fontSizeOptions" :key="index">&ndash;&gt;-->
                                      <!--&lt;!&ndash;{{fontSize}}&ndash;&gt;-->
                                  <!--&lt;!&ndash;</vs-dropdown-item>&ndash;&gt;-->
                              <!--&lt;!&ndash;</vs-dropdown-menu>&ndash;&gt;-->
                          <!--&lt;!&ndash;</vs-dropdown>&ndash;&gt;-->
                      <!--&lt;!&ndash;</vs-col>&ndash;&gt;-->
                  <!--&lt;!&ndash;</vs-row>&ndash;&gt;-->
              <!--&lt;!&ndash;</v-popover>&ndash;&gt;-->
              <!--<a class="button delete delete-text" v-on:click="onDeleteTextClick(text)"><i class="fas fa-times"> </i></a>-->
          <!--</vs-row>-->
      <!--</div>-->

      <modal name="text-option-modal"
             id="text-option-modal"
             :pivot-y="0.2"
             width="350"
             :classes="'v--modal'"
             height="auto"
             @before-close="beforeClose">

        
        <div class="text-option">
            <div class="font-family">
                <vs-select v-model="fontFamily">
                    <vs-select-item value="shnmin" text="明朝体"></vs-select-item>
                    <vs-select-item value="shnkgo" text="角ゴシック体"></vs-select-item>
                    <vs-select-item value="shgyo" text="行書体"></vs-select-item>
                </vs-select>
            </div>
            <div class="font-size">
                <vs-select v-model="fontSize">
                    <vs-select-item :value="fontSize" :text="fontSize" v-for="fontSize in fontSizeOptions" :key="fontSize"></vs-select-item>
                </vs-select>
            </div>
            <div class="font-color">
                <div class="color-box" @click="clickCurrentEvent">
                    <colorPicker  v-model="color" ref="colorPicker" ></colorPicker>
                </div>
            </div>
        </div>
      
        <div class="text-option-button">
          <div class="btn_save" @click="saveTextOption">
            <i class="fas fa-save"></i>
            <span>更新</span>
          </div>
          <div class="btn_cancel" @click="closeTextOption">
            <i class="fas fa-times-circle"></i>
            <span>閉じる</span>
          </div>
        </div>

      </modal>

      <div class="stamps-confirm-modal" style="display: none;">
          <vs-row vs-type="flex" vs-align="center" vs-justify="center">
              <vs-button class="square mr-0 bg-primary" type="filled" v-on:click="cancelStamp"><div><img :src="require('@assets/images/mobile/cancle_white.svg')"></div> キャンセル</vs-button>
              <vs-button class="square mr-0 bg-primary" type="filled" v-on:click="confirmStamp"><div><img :src="require('@assets/images/mobile/confirm_white.svg')"></div> 決定</vs-button>
          </vs-row>
      </div>
  </div>
</template>

<script>
  import { mapState, mapActions } from "vuex";
  import colorPicker  from '@/components/color-picker/color-picker.vue'
  //import Konva from "konva/konva";

export default {
name: 'pdf-page-editor-mobile',
components : {colorPicker},
props: {
  config              : { type: Object},
  page                : { type: Object},
  imageUrl            : { type: String},
  selected            : { type: Boolean },
  isPublic            : { type: Boolean, default: false},
  enable              : { type: Boolean, default: true},
  deleteFlg           : { type: Boolean, default: false},
  deleteWatermark     : { type: String, default: ''},
  isExternal  : { type: Boolean, default: false},
  showEdit  : { type: Boolean, default: true},
  stamps              : { type: Array},
  isOptionText : { type: Boolean, default: false}
},
data() {
  return {
    openPopover: false,
    selectedId: 0,
    stampIndex: 0,
    textIndex: 2,
    font: '0',
    textIdSelected: 0,
    fontSizeOptions: [8,9,10,11,12,14,16,18,20,22,24,26,28,36,42,72],
    realScale: 0,
    maskBase64: '',
    maskBase64Landscape: '',
    confirmIndex: 0,
    confirmFlag: false,
    startX: 0,
    moveEndX: 0,
    dragStartPonitX: 0,
    dragStartPonitY: 0,
    pinchInOutX0 : 0,
    pinchInOutX1 : 0,
    pinchInOutY0 : 0,
    pinchInOutY1 : 0,
    isDraggable: true,            // draggableの値
    pdfShiftDistanceX: 0,     // draggableで移動したPDFズレ X軸
    pdfShiftDistanceY: 0,     // draggableで移動したPDFズレ Y軸
    selectedStamp: {},
    defaultFontColor:"#000000",
    isMove: false,
    touchData: null,
    stepZoom: 5,
    textDataDrag: null,
    color: "#000000",
    changeColor:{},
    textOptionData: {new:{fontFamily: 'shnmin', fontSize: 8, fontColor: '#000000'}, old:{fontFamily: 'shnmin', fontSize: 8, fontColor: '#000000'}, current:{fontFamily: 'shnmin', fontSize: 8, fontColor: '#000000'}},
    is_tablet: false,
    fontSizeValue: 8,
    fontFamilyValue: 'shnmin',
    lastDist: 0,
    lastCenter: null,
  }
},
computed: {
  ...mapState({
    // stamps: state => state.home.stamps,
    stampSelected: state => state.home.stampSelected,
    isTextSelected: state => state.home.textSelected,
    fileSelected: state => state.home.fileSelected,
  }),
  zoomChanged: function () {
    if(this.$store.fileSelected == null) {
      return -1;
    }
    return this.$store.fileSelected.zoom;
  },
  isConfidential: function() {
    if(this.$store.state.home.fileSelected == null) {
      return false;
    }
    return this.$store.state.home.fileSelected.confidential_flg;
  },
  isOverlayHidden:function(){
    if(this.$store.state.home.fileSelected == null) {
      return false;
    }
    return this.$store.state.home.fileSelected.overlay_hidden_flg;

  },
  text: function() {
    return this.page.texts ? this.page.texts.find(item => item.index === this.textIdSelected) : null;
  },
  fontFamily:{
      get: function (newValue) {
          return this.text ? this.text.fontFamily : this.fontFamilyValue;
      },
      set: function (newValue) {
          this.textOptionData.new.fontFamily = newValue;
          this.fontFamilyValue = newValue;
          // this.onTextChangeFont(this.text, newValue);
      }
  },
  fontSize:{
    get: function () {
        return this.text && this.text.fontSize ? this.text.fontSize : this.fontSizeValue;
    },
    set: function (newValue) {
        this.textOptionData.new.fontSize = newValue;
        this.fontSizeValue = newValue;
        // this.onTextChangeFontSize(this.text, newValue);
    }
  },
  useKonva() {
    const page = this.page;
    return this.selected || page.stamps.length > 0 || page.texts.length > 0;
  }
},
methods: {
  ...mapActions({
    addFileStamp: "home/addFileStamp",
    updateFileStamp: "home/updateFileStamp",
    deleteFileStamp: "home/deleteFileStamp",
    addFileText: "home/addFileText",
    updateFileText: "home/updateFileText",
    deleteFileText: "home/deleteFileText",
    undoAction: "home/undoAction",
    selectText: "home/selectText",
    updateCurrentFileZoom: "home/updateCurrentFileZoom",
  }),
  createImageFromUrl: function(url) {
    const image = new Image();
    image.src = url;
    return image;
  },
  createImage: function(base64) { 
    if(!base64) {
      return new Image();
    }
    return this.createImageFromUrl(`data:image/png;base64,${base64}`);
  },
  createStampImage: function(stampId) {
    if(!stampId) {
      return new Image();
    }
    const stamp = this.stamps.find(item=> item.id === stampId);
    let image = new Image();
    if(stamp) image.src = `data:image/png;base64,${stamp.url}`;
    return image;
  },
  touchEditorClick: function(e){
      if(!this.enable) return;

      this.touchData = e.evt.touches;

      this.isMove = false;
      if(e.target.attrs.type == "Stamp"){
          this.selectedStamp = this.page.stamps.find(s => s.index == e.target.attrs.id);
      }
      this.confirmFlag = false;
      this.startX = 0;
      this.moveEndX = 0;

      // 二本指タッチ時、ドラッグフラグをfalseにする
      if (e.evt.touches.length === 2 || this.isDraggable === false){
          this.isDraggable = false;
          
          return;
      } 
      this.isDraggable = true;
      // 一本指タッチ時、ドラッグ距離を計算する
      if (e.evt.touches.length === 1){
          this.dragStartPonitX = e.evt.touches[0].clientX;
          this.dragStartPonitY = e.evt.touches[0].clientY;
          localStorage.setItem('pagex', e.evt.touches[0].pageX);
          localStorage.setItem('pagey', e.evt.touches[0].pageY);
      }

      if(!this.showEdit){
          // get start x
          this.startX = e.evt.touches[0].pageX;
          return;
      }

      let stage = this.$refs.stage.getStage();
      let layer = stage.children[0];

      if(this.isTextSelected) {
          const targetId = e.target.attrs.id;
          if (e.target.attrs.type === 'Text') {
              this.textIdSelected = targetId;
              return;
          }
          
          if( !document.getElementById('textareaBox') ) {
            if(this.textIdSelected > 0) {
                this.textIdSelected = 0;
                return false;
            }
          } else {
            this.textIdSelected = 100;
          }
      }
  },
  confirmTouchEditor: function(e) {
      const stage = this.$refs.stage.getStage();
      const layer = stage.children[0];
      const scale = stage.scaleX();

      if(this.isPublic && this.selectedId === 0 && e.evt.changedTouches.length === 1 && !this.isTextSelected && !this.isMove && !this.stamps.length) {
          this.stampIndex++;
          const callback = () => {
              this.selectedId = this.stampIndex;
              this.loadStamps();
          }
          this.$emit('generateStamp',{event: e, page: this.page, realScale: this.realScale, stampIndex: this.stampIndex, selectedId: this.selectedId, callback:callback});
          return;
      }

      if ( e.target.attrs.type === undefined || e.evt.changedTouches.length === 2 || ( !this.isTextSelected && !this.stampSelected ) ) {
        this.isDraggable = true;
        return false;
      }

      // ドラッグ距離の計算
      const distance = this.getDistance(e.evt.changedTouches[0].clientX, e.evt.changedTouches[0].clientY, this.dragStartPonitX, this.dragStartPonitY)

      // 初期化
      this.dragStartPonitX = null;
      this.dragStartPonitY = null;
      this.pinchInOutX0 = null;
      this.pinchInOutX1 = null;
      this.pinchInOutY0 = null;
      this.pinchInOutY1 = null;

      // 編集時でタッチ終了時にドラッグフラグを立てる
      /*if (this.showEdit){
          var ele = document.getElementsByClassName('stamps-confirm-modal');
          // 決定、キャンセルモーダルが表示されていない場合のみドラッグフラグを立てる
          if (ele[0].outerHTML.indexOf('display: none;') !== -1){
              this.isDraggable = true;
          }
      }*/

      /*if(!this.isTextSelected && !this.isExternal && this.showEdit) {
          if(e.evt.changedTouches.length === 1 && distance < 5){
              if (typeof this.pdfShiftDistanceX === "undefined"){
                  this.pdfShiftDistanceX = 0;
                  this.pdfShiftDistanceY = 0;
              }

              $(".stamps-confirm-modal").show();
              this.isDraggable = false;
              this.confirmIndex = e.target.id();
          }
      }*/
      var ele = document.getElementsByClassName('stamps-confirm-modal');
      // 決定、キャンセルモーダルが表示されていない場合のみドラッグフラグを立てる
      if (ele[0].outerHTML.indexOf('display: none;') !== -1){
          this.isDraggable = true;
      }

      if(!this.isTextSelected) { // && !this.isExternal
          if(e.evt.changedTouches.length === 1 && distance < 5 && this.showEdit){
              if (typeof this.pdfShiftDistanceX === "undefined"){
                  this.pdfShiftDistanceX = 0;
                  this.pdfShiftDistanceY = 0;
              }

              $(".stamps-confirm-modal").show();
              this.isDraggable = false;
              this.confirmIndex = e.target.id();
          }
      }
      if(!this.isTextSelected){
          // 再度タップでも捺印
          if (this.confirmFlag) {
              this.confirmStamp();
          }
      }
      if(!this.showEdit){
          // 1 not touchmove 2 android startX not equals moveEndX
          if(this.moveEndX !== 0 && this.moveEndX !== this.startX){
              if(this.moveEndX > this.startX){
                  this.$emit('prev');
              }else if(this.moveEndX < this.startX){
                  this.$emit('next');
              }
          }else{
              this.$emit('func');
              return;
          }
      }

      if( e.evt.changedTouches.length === 1 ) {
        if( this.selectedId > 0 && !this.isMove ) {
          if( e.target.id() === this.selectedId ) {
            this.confirmStamp()
            return 
          }
          else {
            this.cancelStamp()
            return
          }
        }
      }

      // if( !this.isTextSelected ) { this.isDraggable = true;  return false; }

      // ドラッグしていない場合
      if (distance < 5){
          // スタンプ選択時、かつドラッグしていない場合、スタンプを押下する
          var topDistance = document.getElementsByClassName('sp-navi')[0].offsetHeight + 70;
          const $mailList = $('#handleMailList');
          const $confidential = $('#main-home-mobile .confidential');
          const $mailListInfo = $('#mail_list_box');
          var headerInfo = 0;

          if( $mailList && $mailList.length ) {
            headerInfo = headerInfo + $mailList.height();
          }
          if( $confidential && $confidential.length && $confidential.is(':visible') ) {
            headerInfo = headerInfo + $confidential.height() + 5;
          }
          if( $mailListInfo && $mailListInfo.length && $mailListInfo.is(':visible') ) {
            headerInfo = headerInfo + $mailListInfo.height();
          }

          topDistance = $('#main-home-mobile .content').offset().top;
          
          if(!this.isTextSelected) {
              let canvas = this.$el.querySelector(`canvas`);
              let canvasClientWidth = canvas.clientWidth;
              let canvasClientHeight = canvas.clientHeight;
              let navHeight = document.getElementsByClassName('sp-navi')[0].clientHeight;
              let mobileWidth = document.getElementsByClassName('router-view')[0].clientWidth;
              let stampWidth = this.stampSelected.width * scale;
              if( scale < 1 && (e.evt.changedTouches[0].pageX-7 > canvasClientWidth) || (e.evt.changedTouches[0].pageY-navHeight - topDistance > canvasClientHeight) ) {
                  //|| e.evt.changedTouches[0].pageY > canvasClientHeight * scale + navHeight + 150){
                  this.isTextSelected = true;
                  $(".stamps-confirm-modal").hide();
                  return false;
              }
              let y = e.evt.changedTouches[0].clientY;
              // set stamps-confirm-modal position
              if(window.innerHeight - y < 190){
                  $(".stamps-confirm-modal").css("bottom",190 + this.stampSelected.width * this.realScale);
              }else{
                  $(".stamps-confirm-modal").css("bottom",120);
              }
              if(this.selectedId > 0) {
                  if(e.target.id() === this.selectedId) {
                      // 再度タップでも捺印
                      this.confirmFlag = true;
                  }
                  return;
              }
              this.stampIndex++;
              
              //let topDistance = document.getElementsByClassName('sp-navi')[0].offsetHeight + 140;
              //const stamp = {id: this.stampSelected.id,url:this.stampSelected.url, width: this.stampSelected.width, height: this.stampSelected.height, x:e.evt.offsetX, y:e.evt.offsetY};

              let wx = ( ((e.evt.changedTouches[0].pageX-15) / scale) / this.realScale ) - ( this.stampSelected.width / 2 );
              let wy = 0;
              
              if( scale+this.realScale > 2 )
                wy = ( ((e.evt.changedTouches[0].pageY-topDistance+15) / scale) / this.realScale ) - this.stampSelected.height;
              else 
                wy = ( ((e.evt.changedTouches[0].pageY-topDistance) / scale) / this.realScale ) - this.stampSelected.height;

              wx = wx - ( (this.pdfShiftDistanceX / scale) / this.realScale );
              wy = wy - ( (this.pdfShiftDistanceY / scale) / this.realScale );

              const stampWithoutUrl = {
                  index: this.stampIndex,
                  id: this.stampSelected.id,
                  width: this.stampSelected.width,
                  height: this.stampSelected.height,
                  // x: ((e.evt.changedTouches[0].pageX - (this.stampSelected.width * scale) / 2) / scale) / this.realScale - (this.pdfShiftDistanceX - handle_position_X),
                  x: wx, 
                  //y: ((e.evt.changedTouches[0].pageY - (this.stampSelected.height * scale) / 2 - 220) / scale) / this.realScale - this.pdfShiftDistanceY,
                  // y: ((e.evt.changedTouches[0].pageY - (this.stampSelected.height * scale) / 2 - topDistance) / scale) / this.realScale - handle_position_Y,
                  y: wy+20,
                  scaleX: 1,
                  scaleY: 1,
                  rotation: 0,
                  selected: true,
                  clientY: e.evt.changedTouches[0].clientY,
              };
              //thisq.page.stamps.push(stamp);
              this.addFileStamp({stamp: stampWithoutUrl, pageno: this.page.no});
              this.selectedId = this.stampIndex;

              //Draw new stamp with selected
              let groups = layer.find('Group').toArray();
              let group = groups[0];

              let stampNode = new Konva.Image({
                  image: this.createStampImage(stampWithoutUrl.id),
                  id: stampWithoutUrl.index,
                  draggable: stampWithoutUrl.selected,
                  width: stampWithoutUrl.width * this.realScale,
                  height: stampWithoutUrl.height * this.realScale,
                  x: stampWithoutUrl.x,
                  y: stampWithoutUrl.y,
                  scaleX: stampWithoutUrl.scaleX,
                  scaleY: stampWithoutUrl.scaleY,
                  rotation: stampWithoutUrl.rotation,
                  stroke: '#0984e3',
                  strokeWidth: (stampWithoutUrl.selected ? 2: 0),
                  type: 'Stamp',
                  clientY: stampWithoutUrl.clientY,
              });

              group.add(stampNode);
              layer.draw();
              this.isDraggable = true;
          } else if (this.isTextSelected){

              const targetId = e.target.attrs.id;
              if (e.target.attrs.type === 'Text') {
                  this.textIdSelected = targetId;
                  return;
              }
              if(this.textIdSelected > 0) {
                  this.textIdSelected = 0;
                  return;
              }
              this.textIndex++;
              this.textIdSelected = this.textIndex;

              let wx = e.evt.changedTouches[0].pageX - 26;
              let wy = e.evt.changedTouches[0].pageY - topDistance + 6.5;

              let touchData = null;
              if( this.is_tablet ) touchData = {pageX: e.evt.changedTouches[0].pageX-5, pageY: e.evt.changedTouches[0].pageY - 22};
              else {
                let posY = e.evt.changedTouches[0].pageY;
                if( scale > 1.7 ){
                  posY = posY + ( this.textOptionData.current.fontSize <=20 ? 4.5 : 5.5 );
                } else {
                  // posY = posY + 4.5;
                  let temp = topDistance + (this.config.height*50)/100;
                  posY = posY + ( posY > temp ? 5.8 : 4.8 );
                }
                
                touchData = {pageX: e.evt.changedTouches[0].pageX-5.3, pageY: posY+3};
              }

              let textNode = new Konva.Text({
                  id: this.textIndex,
                  text: '',
                  x: wx,
                  y: wy,
                  // x: e.evt.changedTouches[0].pageX / scale - 20 - this.pdfShiftDistanceX,
                  // y: e.evt.changedTouches[0].pageY / scale - 90 - this.pdfShiftDistanceY,
                  //y: e.evt.changedTouches[0].pageY / scale - 220 - this.pdfShiftDistanceY,PAC_5-1922テキストエリア
                  // fontSize: Math.round(20*1.333333333*scale* this.realScale),
                  fontSize: this.textOptionData.current.fontSize,
                  fontFamily: this.textOptionData.current.fontFamily,
                  fontColor: this.textOptionData.current.fontColor,
                  fill: this.textOptionData.current.fontColor,
                  draggable: true,
                  width: 50,
                  type: 'Text',
                  touchData: touchData
              });
              textNode.on('touchmove', function () {
                  stage.container().style.cursor = 'move';
              });

              textNode.on('touchend', function () {
                  stage.container().style.cursor = 'default';
              });

              textNode.on('transform', function () {
                  // reset scale, so only with is changing by transformer
                  textNode.setAttrs({
                      width: textNode.width() * textNode.scaleX(),
                      scaleX: 1
                  });
              });

              const text = {
                  index: textNode.id(),
                  text: textNode.text(),
                  x: textNode.x(),
                  y: textNode.y(),
                  fontSize: textNode.fontSize() / this.realScale,
                  fontFamily: textNode.fontFamily(),
                  fontColor: textNode.fill(),
                  width: textNode.width() / this.realScale,
                  scaleX: scale,
                  scaleY: scale,
                  rotation: textNode.rotation(),
                  touchData: touchData
              };
              this.textDataDrag = null;
              this.addFileText({text: text, pageno: this.page.no});
              this.onTextDblClick(stage, textNode, layer, true);
              this.$emit('hideBtn');
          }
      }
  },
  moveToChangePage:function(e){
    e.evt.preventDefault();
  if(e.evt.touches.length == 2) {
    let stage = this.$refs.stage.getStage();
    var touch1 = e.evt.touches[0];
    var touch2 = e.evt.touches[1];
    var dist = null;
    var scale = 0;

    var topDistance = document.getElementsByClassName('sp-navi')[0].offsetHeight + 70;
    const $mailList = $('#handleMailList');
    const $confidential = $('#main-home-mobile .confidential');
    const $mailListInfo = $('#mail_list_box');
    var headerInfo = 0, scroll_top = 0;

    if( $mailList && $mailList.length ) {
      headerInfo = headerInfo + $mailList.height();
    }
    if( $confidential && $confidential.length && $confidential.is(':visible') ) {
      headerInfo = headerInfo + $confidential.height() + 5;
    }
    if( $mailListInfo && $mailListInfo.length && $mailListInfo.is(':visible') ) {
      headerInfo = headerInfo + $mailListInfo.height();
    }

    topDistance = topDistance + headerInfo;

    scroll_top = $(window).scrollTop();
    
    var p1 = {
      x: touch1.clientX - 18,
      y: touch1.clientY - topDistance + scroll_top - 15, 
    };
    var p2 = {
      x: touch2.clientX - 18,
      y: touch2.clientY - topDistance + scroll_top - 15,
    };

    dist = this.getDistanceKonva(p1, p2);
    if (!this.lastDist) this.lastDist = dist;

    scale = stage.scaleX() * (dist / this.lastDist);

    if( scale == 0 || scale < 0.5 || scale > 2 ) return false;

    if (touch1 && touch2) {

      if (!this.lastCenter) {
        this.lastCenter = this.getCenter(p1, p2);
        return;
      }
      var newCenter = this.getCenter(p1, p2);

      // local coordinates of center point
      var pointTo = {
        x: (newCenter.x - stage.x()) / stage.scaleX(),
        y: (newCenter.y - stage.y()) / stage.scaleX(),
      };

      var scale = stage.scaleX() * (dist / this.lastDist);

      stage.scaleX(scale);
      stage.scaleY(scale);

      // calculate new position of the stage
      var dx = newCenter.x - this.lastCenter.x;
      var dy = newCenter.y - this.lastCenter.y;

      var newPos = {
        x: newCenter.x - pointTo.x * scale + dx,
        y: newCenter.y - pointTo.y * scale + dy,
      };

      stage.position(newPos);
      stage.batchDraw();

      this.pdfShiftDistanceX = newPos.x;
      this.pdfShiftDistanceY = newPos.y;

      this.lastDist = dist;
      this.lastCenter = newCenter;
    }
    }

  },
  // 距離計算
  getDistance: function(p1x, p1y, p2x, p2y) {
      var x = p2x - p1x,
          y = p2y - p1y;

      return Math.sqrt((x * x) + (y * y));
  },
  cancelStamp: function(){
      $(".stamps-confirm-modal").hide();
      this.isDraggable = true;
      this.undoAction();
      this.updateFileStamp({stamp: {index:this.selectedId, selected: false}, pageno: this.page.no});
      this.selectedId = 0;
      this.loadStamps();
      return;
  },
  confirmStamp: function(){
      $(".stamps-confirm-modal").hide();
      this.isDraggable = true;
      this.updateFileStamp({stamp: {index:this.selectedId, selected: false}, pageno: this.page.no});
      this.selectedId = 0;
      this.loadStamps();
      return;
  },
  onStampClick: function(e) {
      //this.selectedId = e.currentTarget.attrs.id;
      //this.$refs.stage.draw();

  },
  handleDragstart(e) {
      this.isMove = true;
      this.confirmFlag = false;
  },
  handleDragend(e) {
      this.lastDist = 0;
      this.lastCenter = null;

      let stage = this.$refs.stage.getStage();
      const layer = stage.children[0];
      const scale = stage.scaleX();

      if( e.evt.touches.length === 2 ) {
        this.$store.state.home.fileSelected.zoom = scale*100;
        this.reloadStage();
      }

      // ドラッグした距離を格納
      this.pdfShiftDistanceX = e.currentTarget.attrs.x;
      this.pdfShiftDistanceY = e.currentTarget.attrs.y;
      localStorage.setItem('pdfShiftDistanceX', e.currentTarget.attrs.x);
      localStorage.setItem('pdfShiftDistanceY', e.currentTarget.attrs.y);


      // Handle max drag 2/3
      var max_drag_x = ( this.config.width * 2) / 3;
      var max_drag_y = ( this.config.height * 2) / 3;

      if( this.pdfShiftDistanceX > 0 ) this.pdfShiftDistanceX = this.pdfShiftDistanceX > max_drag_x ? max_drag_x : this.pdfShiftDistanceX;
      else this.pdfShiftDistanceX = -this.pdfShiftDistanceX > ( scale * this.config.width - (this.config.width-max_drag_x) ) ? -( scale * this.config.width - (this.config.width-max_drag_x) ) : this.pdfShiftDistanceX;

      if( this.pdfShiftDistanceY > 0 ) this.pdfShiftDistanceY = this.pdfShiftDistanceY > max_drag_y ? max_drag_y : this.pdfShiftDistanceY;
      else this.pdfShiftDistanceY = -this.pdfShiftDistanceY > ( scale * this.config.height - (this.config.height-max_drag_y) ) ? -( scale * this.config.height - (this.config.height-max_drag_y) ) : this.pdfShiftDistanceY;
      
      if(e.target.attrs.type === 'Stamp' && e.evt) {
        if(
          e.target.attrs.x < 0 || 
          e.target.attrs.y < 0 ||
          e.target.attrs.x > stage.width()*scale || 
          e.target.attrs.y > stage.height()*scale
        ) return false;
        
        this.doUpdateStamp(e);
      }
      // else if(e.target.attrs.type === 'Text') {
      //     this.moveToChangePage(e)
          // this.textDataDrag = e.evt.changedTouches[0]
      //     this.doUpdateText(e.target);
      //     let stage = this.$refs.stage.getStage();
      //     let layer = stage.children[0];
      //     setTimeout(()=> {
      //         // this.onTextDblClick(stage,e.target, layer, false);
      //         // this.$emit('hideBtn');
      //           stage.find(node => node.attrs.type === 'Text' && node.attrs.id === e.target.id()).show();
      //           layer.draw();
      //     },100);

      // }
      // 以下でPDFドラッグ時の位置調整を行う　(draggble=trueの場合の処理)
      else{

          // X軸の位置調整
          // if (this.pdfShiftDistanceX >= 0 || scale < 1){
          //     this.pdfShiftDistanceX = 0;
          // }
          // else 
          // if (scale >= 1 && this.config.width * scale + this.pdfShiftDistanceX < this.config.width){
          //     this.pdfShiftDistanceX = this.config.width - this.config.width * scale;
          // }

          // Y軸の位置調整
          // if (this.pdfShiftDistanceY >= 0 || scale < 1){
          //     this.pdfShiftDistanceY  = 0;
          // }
          // else 
          // if (scale >= 1 && this.config.height * scale + this.pdfShiftDistanceY < this.config.height){
          //     this.pdfShiftDistanceY = this.config.height - this.config.height * scale;
          // }

          stage.x(this.pdfShiftDistanceX);
          stage.y(this.pdfShiftDistanceY);
          layer.draw();
      }
  },
  doUpdateText: function(textNode) {

      if( this.textDataDrag === 'out' ) return false;

      let stage = this.$refs.stage.getStage();
      const scale = stage.scaleX();
      const $mailList = $('#handleMailList');
      const $confidential = $('#main-home-mobile .confidential');
      const $mailListInfo = $('#mail_list_box');
      var topDistance = document.getElementsByClassName('sp-navi')[0].offsetHeight;
      var headerInfo = 0;

      if( $mailList && $mailList.length ) {
        headerInfo = headerInfo + $mailList.height();
      }
      if( $confidential && $confidential.length && $confidential.is(':visible') ) {
        headerInfo = headerInfo + $confidential.height();
      }
      if( $mailListInfo && $mailListInfo.length && $mailListInfo.is(':visible') ) {
        headerInfo = headerInfo + $mailListInfo.height();
      }

      topDistance = $('#main-home-mobile .content').offset().top;
      
      var firstTouch = null;
      if(this.textDataDrag) {
        firstTouch = this.textDataDrag;
      } else if(textNode.attrs.touchData) {
        firstTouch = textNode.attrs.touchData;
      } else {
        if( !this.is_tablet ){
          firstTouch = {
                          pageX: document.getElementById('textareaBox').getBoundingClientRect().left + 9 ,
                          pageY: document.getElementById('textareaBox').getBoundingClientRect().top + 4 + $(window).scrollTop(),
                        }
        } else {
          firstTouch = {
                        pageX: document.getElementById('textareaBox').getBoundingClientRect().left + 9 ,
                        pageY: document.getElementById('textareaBox').getBoundingClientRect().top + 18 + $(window).scrollTop(),
                      }
        }
      }
      if( !firstTouch ) return false;
      this.pdfShiftDistanceX = this.pdfShiftDistanceX ? this.pdfShiftDistanceX : 0
      this.pdfShiftDistanceY = this.pdfShiftDistanceY ? this.pdfShiftDistanceY : 0

      let wx = ( ((firstTouch.pageX - 21) / scale) / this.realScale ) - (this.pdfShiftDistanceX / scale) / this.realScale;
      let wy = ( ((firstTouch.pageY - topDistance) / scale) / this.realScale ) - ( (this.pdfShiftDistanceY / scale) / this.realScale );
      
      let newWidthVal = this.createNewNodeTextAttr(textNode.attrs)
      const text = {
          index: textNode.id(),
          text: textNode.text(),
          x: wx,
          y: wy,
          fontSize: textNode.attrs.fontSize / this.realScale,
          fontFamily: textNode.fontFamily(),
          fontColor: textNode.attrs.fontColor,
          width: newWidthVal / this.realScale,
          scaleX: textNode.scaleX(),
          scaleY: textNode.scaleY(),
          rotation: textNode.rotation()
      };
      this.updateFileText({text: text, pageno: this.page.no});
      this.textDataDrag = null;
  },
  doUpdateStamp: function(e) {
      const stamp = {
          index: e.target.attrs.id,
          x: e.target.x() / this.realScale,
          y: e.target.y() / this.realScale,
          width: e.target.width() / this.realScale,
          height: e.target.height() / this.realScale,
          scaleX: e.target.scaleX(),
          scaleY: e.target.scaleY(),
          rotation: e.target.rotation(),
          selected: true,
      };
      this.updateFileStamp({stamp:stamp, pageno: this.page.no});
  },
  onStageClick: function (e) {

  },
  clickCurrentEvent: function(){
      this.$refs.colorPicker.openPanel();
  },
  loadTexts: function (draggable) {
      if(!this.$refs.stage) {
          return;
      }

      const $this = this;
      let stage = this.$refs.stage.getStage();
      let layer = stage.children[0];
      this.textIndex = 2;
      const scale = stage.scaleX();
      layer.find('Text').destroy();
      for(let text of this.page.texts) {
          if(!this.changeColor.hasOwnProperty('id_' + text.index)) this.changeColor['id_' + text.index] = text.fontColor
        
          this.textIndex = text.index;
          let textNode = new Konva.Text({
              id: this.textIndex,
              text: text.text,
              x: text.x * this.realScale,
              y: text.y * this.realScale,
              fontSize: text.fontSize* this.realScale,
              fontFamily: text.fontFamily,
              fontColor: this.changeColor['id_' + text.index],
              fill: this.changeColor['id_' + text.index],
              draggable: draggable,
              width: text.width * this.realScale,
              scaleX: text.scaleX,
              scaleY: text.scaleY,
              rotation: text.rotation,
              type: 'Text',
          });

          //textNode.zIndex(-1);
          if(textNode.attrs.id === this.textIdSelected && $(this.$refs.wrap).has('textarea:visible').length) {
              textNode.hide();
          }

          layer.add(textNode);

          textNode.on('dragstart', (e) => {
              this.textIdSelected = 0;
          });

          textNode.on('dragend', (e) => {
              this.textIdSelected = e.target.id();
          });

          textNode.on('mouseover', function () {
              stage.container().style.cursor = 'move';
          });

          // textNode.on('touchstart', function (e) {
          //   let position_x = textNode.x() / $this.realScale - $this.pdfShiftDistanceX;
          //   let position_y = textNode.y() / $this.realScale - $this.pdfShiftDistanceY;

          //   if( $this.is_tablet ) position_y = position_y - 30;

          //   $this.textDataDrag = {pageX: position_x, pageY: position_y};
          // });

          textNode.on('touchend', function (e) {
              stage.container().style.cursor = 'default';
          });

          textNode.on('transform', function () {
              // reset scale, so only with is changing by transformer
              textNode.setAttrs({
                  width: textNode.width() * textNode.scaleX(),
                  scaleX: 1
              });
          });

          layer.draw();

          textNode.on('touchstart', (e) => {
              this.onTextDblClick(stage, textNode, layer, false);
          });
          const $this = this;
          textNode.on('transformend', function (e) {
              $this.doUpdateText(e.target);
          });
      }
  },
  loadStamps: function () {
      if(!this.$refs.stage) {
          return;
      }
      let stage = this.$refs.stage.getStage();
      let layer = stage.children[0];
      layer.find('Group').destroy();
      const group = new Konva.Group();
      layer.add(group);

      if (typeof this.pdfShiftDistanceX === "undefined"){
          this.pdfShiftDistanceX = 0;
          this.pdfShiftDistanceY = 0;
      }

      ///const mask = group.find('Image').toArray().find(item => item.attrs.type === 'Mask');
      //if(mask) mask.destroy();
      
      if(this.isConfidential && !this.isOverlayHidden) {
          const image1 = new Konva.Image({
              image: this.createImage((this.config.width < this.config.height ? this.maskBase64 : this.maskBase64Landscape)),
              id: 0,
              draggable: false,
              width: this.config.width,
              height: this.config.height,
              type: 'Mask',
          });

          group.add(image1);
      }
      if(this.deleteFlg && this.deleteWatermark) {
          const image1 = new Konva.Image({
              image: this.createImage(this.deleteWatermark),
              id: 0,
              draggable: false,
              width: this.config.width,
              height: this.config.height,
              type: 'Mask',
          });

          group.add(image1);
      }
      
      for(let stamp of this.page.stamps) {
          let stampNode = new Konva.Image({
              image: this.createStampImage(stamp.id),
              id: stamp.index,
              draggable: stamp.selected,
              width: stamp.width * this.realScale,
              height: stamp.height * this.realScale,
              x: stamp.x * this.realScale,
              y: stamp.y * this.realScale,
              scaleX: stamp.scaleX,
              scaleY: stamp.scaleY,
              rotation: stamp.rotation,
              stroke: '#0984e3',
              strokeWidth: (stamp.selected ? 2: 0),
              type: 'Stamp'
          });

          group.add(stampNode);
      }

      //layer.add(group);
      layer.draw();
      setTimeout(()=> {
          //layer.draw();
      },100);

  },
  onStampTransform: function (e) {
      this.doUpdateStamp(e);

  },
  getCenter: function(p1, p2) {
    return {
      x: (p1.x + p2.x) / 2,
      y: (p1.y + p2.y) / 2,
    };
  },
  getDistanceKonva: function(p1, p2) {
    return Math.sqrt(Math.pow(p2.x - p1.x, 2) + Math.pow(p2.y - p1.y, 2));
  },
  reloadStage:function(newVal=0, oldVal=0) {

      if(newVal > 200 || newVal < 70) return false;

      let stage = this.$refs.stage ? this.$refs.stage.getStage() : null;
      const scale = stage ? stage.scaleX() : this.$store.state.home.fileSelected.zoom / 100;

      stage.width(this.config.width);
      stage.height(this.config.height);

      stage.draw();

      localStorage.setItem('pdfShiftDistanceX', stage.x());
      localStorage.setItem('pdfShiftDistanceY', stage.y());
  },
  onTextDblClick: function(stage,textNode,layer,isAdd) {
      if(!this.isTextSelected) {
          return;
      }

      const $this = this;
      const scale = stage.scaleX();
      // hide text node and transformer:
      stage.find(node => node.attrs.type === 'Text' && (node.attrs.id === textNode.attrs.id || node.attrs.id === this.textIdSelected)).hide();
      layer.draw();

      // create textarea over canvas with absolute position
      // first we need to find position for textarea
      // how to find it?

      // at first lets find position of text node relative to the stage:
      let textPosition = textNode.absolutePosition();

      // then lets find position of stage container on the page:
      let stageBox = stage.container().getBoundingClientRect();

      this.changeColor['id_'+this.textIdSelected] = textNode.fill();

      // so position of textarea will be the sum of positions above:
      let areaPosition = {
          /*x: stageBox.left + textPosition.x,
          y: stageBox.top + textPosition.y*/
          x: textPosition.x,
          y: textPosition.y
      };

      let newWidthVal = this.createNewNodeTextAttr({text:textNode.text(),fontFamily:textNode.fontFamily(),fontSize:textNode.fontSize()});

      // create textarea and style it
      let textareaBox = document.createElement('div');
      let textarea = document.createElement('textarea');
      let linetextarea = document.createElement('span');
      let bodertextarea = document.createElement('p');
      let iconTextarea = document.createElement('img');

      textareaBox.append(textarea);
      textareaBox.append(bodertextarea);
      textareaBox.append(linetextarea);
      textareaBox.append(iconTextarea);
      
      //document.body.appendChild(textarea);
      this.$refs.wrap.appendChild(textareaBox);
      document.getElementsByTagName('body')[0].classList.add('disabledScroll');

      // apply many styles to match text on canvas as close as possible
      // remember that text rendering on canvas and on the textarea can be different
      // and sometimes it is hard to make it 100% the same. But we will try...

      bodertextarea.style.position = 'absolute'
      bodertextarea.style.width = '100%';
      bodertextarea.style.height = 'calc(100% - 4px)';
      bodertextarea.style.top = '0px';
      bodertextarea.style.left = '0px';
      bodertextarea.style.border = '2px solid #0984e3';
      bodertextarea.style.zIndex  = '2';
      
      textareaBox.id = "textareaBox";
      textareaBox.style.position = 'absolute';
      textareaBox.style.top = (areaPosition.y - 4) + 'px';
      textareaBox.style.left = (areaPosition.x - 5) + 'px';
      
      textarea.value = textNode.text();
      textarea.placeholder = 'あa';
      textarea.rows = 1;
      textarea.wrap = 'off';
      textarea.style.position = 'relative';
      textarea.style.top = '0px';
      textarea.style.width = `${((newWidthVal * scale  - textNode.padding() * 2 )>=50) ? (newWidthVal * scale  - textNode.padding() * 2  + 25 ): 50}px`;
      textarea.style.height = textNode.height() * scale - textNode.padding() + 'px';
      textarea.style.fontSize = (textNode.fontSize() * scale) + 'px';
      textarea.style.border = 'none';
      textarea.style.margin = '0';
      textarea.style.overflow = 'hidden';
      textarea.style.background = 'none';
      textarea.style.outline = 'none';
      textarea.style.resize = 'none';
      textarea.style.lineHeight = textNode.lineHeight();
      textarea.style.fontFamily = textNode.fontFamily();
      textarea.style.transformOrigin = 'left top';
      textarea.style.textAlign = textNode.align();
      textarea.style.color = Object.prototype.hasOwnProperty.call(this.changeColor, "id_" + textNode.id()) ?this.changeColor['id_'+textNode.id()] : this.textInfo.last_text_color;
      textarea.style.padding = '5px';
      textarea.style.whiteSpace = 'pre';
      textarea.style.overflowWrap = 'anywhere';
      textarea.style.zIndex  = '3';
      let rotation = textNode.rotation();
      let transform = '';
      

      // Line
      linetextarea.style.position = 'absolute';
      linetextarea.style.left = 'calc( 50% - 1px )';
      linetextarea.style.top = 'calc(100% - 4px)';
      linetextarea.style.width = '2px';
      linetextarea.style.height = '10px';
      linetextarea.style.background = '#4472c4';
      linetextarea.style.opacity = '0.7';

      // Icon 
      iconTextarea.id = "textarea-icon";
      iconTextarea.src = `data:image/jpeg;base64, `;
      iconTextarea.style.width = '40px';
      iconTextarea.style.position = 'absolute';
      iconTextarea.style.left = 'calc( 50% - 20px )';
      iconTextarea.style.top = 'calc(100% + 0px)';
      iconTextarea.style.color = '#4472c4';
      iconTextarea.style.opacity = '0.7';
      iconTextarea.style.zIndex  = '2';
      iconTextarea.style.padding  = '5px';

      if (rotation) {
          transform += 'rotateZ(' + rotation + 'deg)';
      }
      let  px = Math.round(textNode.fontSize() / 20) + 1 + 1/scale ;
      transform += 'translateY(-' + px + 'px)';

      textarea.style.transform = transform;

      // reset height
      textarea.style.height = 'auto';
      // after browsers resized it we can set actual value
      textarea.style.height = textarea.scrollHeight + 3 + 'px';

      textarea.focus();

      iconTextarea.addEventListener('touchstart', e => {
        document.activeElement.blur();
      });

      iconTextarea.addEventListener('touchmove', e => {

        $('body').addClass('disabledScroll');

        var topDistance = document.getElementsByClassName('sp-navi')[0].offsetHeight + 70;
        const $mailList = $('#handleMailList');
        const $confidential = $('#main-home-mobile .confidential');
        const $mailListInfo = $('#mail_list_box');
        var headerInfo = 0;

        if( $mailList && $mailList.length ) {
          headerInfo = headerInfo + $mailList.height();
        }
        if( $confidential && $confidential.length && $confidential.is(':visible') ) {
          headerInfo = headerInfo + $confidential.height() + 5;
        }
        if( $mailListInfo && $mailListInfo.length && $mailListInfo.is(':visible') ) {
          headerInfo = headerInfo + $mailListInfo.height();
        }

        topDistance = $('#main-home-mobile .content').offset().top;

        textareaBox.style.left = (e.changedTouches[0].pageX - 15 - textareaBox.clientWidth/2) + 'px';
        textareaBox.style.top = ((e.changedTouches[0].pageY - topDistance - 50) - textareaBox.clientHeight) + 'px';

      });

      iconTextarea.addEventListener('touchend', e => {
        textarea.focus();
        let position_x = e.changedTouches[0].pageX;
        let position_y = e.changedTouches[0].pageY;
        const scale = stage.scaleX();

        var topDistance = document.getElementsByClassName('sp-navi')[0].offsetHeight + 70;
        const $mailList = $('#handleMailList');
        const $confidential = $('#main-home-mobile .confidential');
        const $mailListInfo = $('#mail_list_box');
        const $tabSelected = $('.tabSelected');
        var headerInfo = 0;

        if( $mailList && $mailList.length ) {
          headerInfo = headerInfo + $mailList.height();
        }
        if( $confidential && $confidential.length && $confidential.is(':visible') ) {
          headerInfo = headerInfo + $confidential.height() + 5;
        }
        if( $mailListInfo && $mailListInfo.length && $mailListInfo.is(':visible') ) {
          headerInfo = headerInfo + $mailListInfo.height();
        }
        if( $tabSelected && $tabSelected.length && $tabSelected.is(':visible') ) {
          headerInfo = headerInfo + $tabSelected.height();
        }
        topDistance = $('#main-home-mobile .content').offset().top;
        
        let canvas = this.$el.querySelector(`canvas`);
        if( 
          position_x - 20 < this.pdfShiftDistanceX || 
          (position_x - 5) > (canvas.clientWidth*scale + this.pdfShiftDistanceX) ||
          (position_y - topDistance) < this.pdfShiftDistanceY ||
          (position_y - topDistance) > this.pdfShiftDistanceY + canvas.clientHeight * scale
        ) {
          if( !isAdd ) $this.textDataDrag = 'out';
          return false;
        }

        position_x =  position_x - textareaBox.clientWidth/2 + 10;
        position_y =  position_y - textareaBox.clientHeight;

        if ( this.is_tablet ) position_y = position_y - 20;
        // else position_y = position_y + ( scale > 1.8 ? 0 : 5 );

        $this.textDataDrag = {pageX: position_x+0.4, pageY: position_y - 45 };
      });

      function removeTextarea() {
        if (textareaBox && textareaBox.parentNode){
          //$this.textIdSelected = 0;
          textareaBox.parentNode.removeChild(textareaBox);
          window.removeEventListener('touchstart', handleOutsideClick);
          stage.find(node => node.attrs.type === 'Text' && node.attrs.id === textNode.attrs.id).show();
          layer.draw();
          $this.$emit('showBtn');
        }
      }

      function setTextareaWidth(newWidth) {
          if (!newWidth) {
              // set width for placeholder
              newWidth = textNode.placeholder.length * textNode.fontSize();
          }
          // some extra fixes on different browsers
          let isSafari = /^((?!chrome|android).)*safari/i.test(
              navigator.userAgent
          );
          let isFirefox =
              navigator.userAgent.toLowerCase().indexOf('firefox') > -1;
          if (isSafari || isFirefox) {
              newWidth = Math.ceil(newWidth);
          }

          let isEdge =
              document.documentMode || /Edge/.test(navigator.userAgent);
          if (isEdge) {
              newWidth += 1;
          }
          textarea.style.width = newWidth + 'px';
      }

      textarea.addEventListener('focus', function (e) {
          $this.openPopover = false;
      });

      textarea.addEventListener('keydown', function (e) {
          let scale = textNode.getAbsoluteScale().x;
          $(this).width(10).height(10).width(this.scrollWidth).height(this.scrollHeight-3);
          let inputWidth = this.scrollWidth;
          textNode.width(inputWidth + 30);
          setTextareaWidth((inputWidth + 40) * scale);
          //const optionsElement = document.getElementById(`text-${textNode.attrs.id}`);
          //optionsElement.style.left = `${textNode.x() + ((inputWidth + 30) * scale) - 45}px`;
      });

      function handleOutsideClick(e) {
          if (e.target !== textarea && e.target.id != 'textarea-icon') {

              textNode.text(textarea.value);
              document.getElementsByTagName('body')[0].classList.remove('disabledScroll')
              // if(e.target.className === 'vs-con-dropdown parent-dropdown button config') {
              //     const text = stage.find(node => node.attrs.type === 'Text' && node.attrs.id === textNode.attrs.id);
              //     text.text(textarea.value);
              //     // text.width(textNode.width());
              //     layer.draw();
              // }
              if(isAdd) {
                  const text = $this.page.texts.find(item => item.index === textNode.id());
                  if (text){
                      textNode.fontFamily(text.fontFamily);
                      textNode.fontSize(text.fontSize * $this.realScale);
                      textNode.fill = text.fontColor;
                      $this.changeColor['id_'+textNode.id()] = $this.color;

                      $this.applyText(textNode);
                  }
                  if(!textarea.value) {
                      $this.deleteFileText({text: text, pageno: $this.page.no});
                  }
              } else {
                $this.doUpdateText(textNode);
              }
              if( textareaBox ) removeTextarea();
              // if(e.target.className !== 'vs-con-dropdown parent-dropdown dropdown' && e.target.className !== 'vs-dropdown--item-link'
              //     && $(e.target).closest('.config').length <= 0 ) {
              //     if($(e.target).closest('.vs-dropdown--item').length <= 0) $this.doUpdateText(textNode)
              //     removeTextarea();
              //     $this.openPopover = false;
              // }
          }
      }

      setTimeout(() => {
          window.addEventListener('touchstart', handleOutsideClick);
      });
  },
  onTextChangeFont(text, fontFamily) {
      const stage = this.$refs.stage.getStage();
      const layer = stage.children[0];
      if(!stage) {
          return;
      }

      const nodeText = layer.find('Text').toArray().find(item => item.attrs.id == text.index);
      if(!nodeText) {
          return;
      }
      nodeText.fontFamily(fontFamily);
      layer.draw();

      this.applyText(nodeText);
  },
  onTextChangeFontSize(text, fontSize) {
      const stage = this.$refs.stage.getStage();
      const layer = stage.children[0];
      if(!stage) {
          return;
      }
      const nodeText = layer.find('Text').toArray().find(item => item.attrs.id == text.index);
      if(!nodeText) {
          return;
      }
      nodeText.fontSize(fontSize);
      layer.draw();

      this.applyText(nodeText);
  },
  onTextChangeFontColor(fontColor){
      let text = this.text;
      const stage = this.$refs.stage.getStage();
      //document.getElementsByClassName("canvas-textarea")[0].style.color = (fontColor ? fontColor : this.defaultColor);
      var textarea = document.getElementsByClassName("canvas-textarea")[0];
      if(textarea){
          textarea.style.color = fontColor
          textarea.style.removeProperty("background");
      }
      const layer = stage.children[0];
      if(!stage) {
          return;
      }
      this.changeColor['id_'+ this.page.no] = fontColor
      const nodeText = layer.find('Text').toArray().find(item => item.attrs.id == text.index);
      if(!nodeText) {
          return;
      }
      if (text.fontColor != fontColor) {
        this.textInfo.last_text_color = fontColor;
        this.updateTextSetting();
      }
      nodeText.fontColor = fontColor;
      layer.draw();
      this.applyText(nodeText)
      setTimeout(() => {
          stage.find(node => node.attrs.type === 'Text' && node.attrs.id === nodeText.attrs.id).show();
          layer.draw();
      })
  },
  applyText(nodeText){
      const fakeEl = $('<span>').hide().appendTo(document.body);
      let html = nodeText.text();
      html = html.replace(/[\r\n]/g, "<br />");
      fakeEl.html(html).css('font-size', nodeText.fontSize());
      fakeEl.css('font-family', nodeText.fontFamily());
      const width = fakeEl.width();
      nodeText.width(width + 30);
      nodeText.height(fakeEl.height());

      this.doUpdateText(nodeText);
      fakeEl.remove();
  },
  getFontName(fontFamily) {
      let fontName = '';
      if(fontFamily === 'shnmin') {
          fontName = '明朝体';
      }else if(fontFamily === 'shnkgo') {
          fontName = '角ゴシック体';
      }else if(fontFamily === 'shgyo') {
          fontName = '行書体';
      }
      return fontName;
  },
  createNewNodeTextAttr: function(nodeText){
      const fakeEl = $('<pre>').hide().appendTo(document.body);
      let html = nodeText.text;
      html = html.replace(/[\r\n]/g, "<br />");
      fakeEl.html(html).css('font-size', nodeText.fontSize);
      fakeEl.css('font-family', nodeText.fontFamily);
      const width = fakeEl.width();
      fakeEl.remove();
      return width;
  },
  onDeleteTextClick: function (text) {
      const stage = this.$refs.stage.getStage();
      const layer = stage.children[0];
      if(!stage) {
          return;
      }
      const nodeText = layer.find('Text').toArray().find(item => item.attrs.id == text.index);
      if(!nodeText) {
          return;
      }
      nodeText.destroy();
      layer.draw();
      this.textIdSelected = 0;
      this.deleteFileText({text: text, pageno: this.page.no});
  },
  onWindowClick: function(e) {
      if(!this.isTextSelected && this.selectedId && $(e.target).prop("tagName").toUpperCase() !== 'CANVAS' && !this.isPublic) {
          if($(e.target).closest('.submit-circular').length > 0) return;
          if($(e.target).closest('.submit-circular2').length > 0) return;

          if( $(e.target).parents('.zoom-in-container').length == 0 && $(e.target).parents('.zoom-out-container').length == 0 ) {
            $(".stamps-confirm-modal").hide();
            this.isDraggable = true;
            this.undoAction();
            this.selectedId = 0;
            this.textIdSelected = 0;
          }
      }
  },
  exportToImage: function() {
      const stage = this.$refs.stage.getStage();
      if(stage) {
          return stage.toDataURL();
      }
      return null;
  },
  AddStampsConfirmation: function (e) {
      var i=this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo.length;
      if(i>0){
          if(this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo[i-1].text == null){
              this.addFileStamp({stamp: this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo[i-1], pageno: this.page.no});
              this.updateFileStamp({stamp: {index:this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo[i-1].index, selected: false}, pageno: this.page.no});
              this.loadStamps();
              this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo.pop();
          }else {
              this.addFileText({text: this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo[i-1], pageno: this.page.no});
              this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo.pop();
          }
      }

      if(this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo.length > 0){
          this.$store.state.home.disabledProceed = false;
      }else {
          this.$store.state.home.disabledProceed = true;
      }
  },

  openTextOption: function( data ){
    this.textIdSelected = 0;
    var $this = this;
    $('body').addClass('disabledScroll');
    $('header').append('<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=0">');

    this.fontFamilyValue = this.textOptionData.current.fontFamily;
    this.fontSizeValue = this.textOptionData.current.fontSize;
    this.color = this.textOptionData.current.fontColor;
    
    setTimeout(function(){
      window.scroll({top: 2,left: 0,behavior: 'smooth'});
      window.scroll({top: -2,left: 0,behavior: 'smooth'});
    }, 0);

    setTimeout(function(){
      $this.$modal.show('text-option-modal');
    }, 400);
  },
  saveTextOption: function() {
    this.textOptionData.current.fontFamily = this.textOptionData.new.fontFamily;
    this.textOptionData.current.fontSize = this.textOptionData.new.fontSize;
    this.textOptionData.current.fontColor = this.textOptionData.new.fontColor;

    if( !this.isTextSelected ) this.selectText();
    // Fix tabs minimal
    window.scroll({top: 1,left: 0,behavior: 'smooth'});
    window.scroll({top: -1,left: 0,behavior: 'smooth'});

    this.$modal.hide('text-option-modal');
  },
  closeTextOption: function() {
    if( !this.isTextSelected ) this.selectText();
    this.$modal.hide('text-option-modal');
  },
  beforeClose: function(){
    this.color = this.textOptionData.current.fontColor;
    $('body').removeClass('disabledScroll');
    $('header meta[name=viewport]').remove();
  }

  },
  mounted() {
      this.stampIndex = this.page.stamps.length + 1;
      this.loadTexts(this.isTextSelected);
      this.loadStamps();
      this.reloadStage();
  },
  created() {
      // Check Tablet
      if( 
        /(ipad|tablet|(android(?!.*mobile))|(windows(?!.*phone)(.*touch))|kindle|playbook|silk|(puffin(?!.*(IP|AP|WP))))/.test( navigator.userAgent.toLowerCase() )
       ) 
      {
        this.is_tablet = true;
      }

      this.realScale = this.config.width / this.page.width;
      window.addEventListener('click', this.onWindowClick);
  },
  updated() {
      this.reloadStage();
      this.loadStamps();
  },
  watch: {
      "$store.state.home.fileSelected.zoom": function (newVal, oldVal) {
          this.isMove = true;
      },
      "$store.state.home.fileSelected.confidential_flg": function (newVal, oldVal) {
          this.loadStamps();
      },
      "$store.state.home.fileSelected.overlay_hidden_flg": function (newVal, oldVal) {
          this.loadStamps();
      },
      "isTextSelected": function (newVal, oldVal) {
          if(this.selectedId > 0) {
              this.undoAction();
              this.selectedId = 0;
          }

          this.textIdSelected = 0;
          this.loadTexts(newVal);
      },
      "page.texts": {
          handler: function (val, oldVal) {
              this.loadTexts(this.isTextSelected);
          },
          deep: true
      },
      "page.stamps": function (val, oldVal) {
          //this.loadStamps();
          if(val.length === 0) {
              const stage = this.$refs.stage.getStage();
              const layer = stage.children[0];
              if(!stage) {
                  return;
              }
              layer.draw();
          }
      },
      "page.stamps.length": function (val, oldVal) {
          if(val < oldVal) {
              this.loadStamps();
          }
      },
      "config.width": function (newVal, oldVal) {
          this.realScale = this.config.width / this.page.width;
          this.loadTexts(this.isTextSelected);
          this.loadStamps();
      },
      "showEdit": function(newVal, oldVal) {
          // 編集モード終了時、PDFを初期化
          this.isDraggable = true;
          /*if (!this.showEdit){
              let stage = this.$refs.stage.getStage();
              stage.x(0);
              stage.y(0);
              this.$store.state.home.fileSelected.zoom = 100;
              this.isDraggable = false;
              this.pdfShiftDistanceX = 0;
              this.pdfShiftDistanceY = 0;
          }
          // 編集モード時、PDFをドラッグ可能に
          else{
              this.isDraggable = true;
          }*/
      },
      color(val,oldVal){
          this.textOptionData.new.fontColor = val;
      }
  }
}

</script>

<style lang="scss">
#text-option-modal {
  .v--modal-box.v--modal{
    top: 120px !important;
  }
}
</style>
