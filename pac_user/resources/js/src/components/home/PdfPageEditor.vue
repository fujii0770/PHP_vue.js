<template>
    <div ref="wrap" style="position: relative; background: #e8e8e8;"
        :style="{
            width: `${shownSize.width}px`,
            height: `${shownSize.height}px`,
            cursor: cursorStyle,
        }">
        <img v-if="!useKonva"
             :src="imageUrl" alt="a4" :style="`width: 100%`">
        <v-stage v-else
                 @click="onEditorClick" @tap="onEditorClick"
                 ref="stage"
                 :config="{
                    width: shownSize.width,
                    height: shownSize.height,
                    scale: {
                        x: zoomScale,
                        y: zoomScale,
                    }
                  }"
                 @dragstart="handleDragstart"
                 @dragmove="handleDragmove"
                 @dragend="handleDragend"

            @mousedown="handlersForAreaSelect.mousedown"
            @mousemove="handlersForAreaSelect.mousemove"
            @mouseup="handlersForAreaSelect.mouseup"
            @mouseleave="handlersForAreaSelect.mouseleave"

                 :key="page.no + isConfidential">
            <v-layer ref="layer">
                <v-image :config="{
                    image: createImageFromUrl(imageUrl),
                    id: 1,
                    draggable: false,
                    width: noZoomSize.width,
                    height: noZoomSize.height,
                    type: 'Image',
                  }">
                </v-image>
            </v-layer>
            <!-- areaSelectMode 用 -->
            <v-layer v-if="areaSelectMode">
                <v-rect v-if="selectingArea" :config="areaSelectRectConfig">
                </v-rect>
            </v-layer>
        </v-stage>

        <div class="text-options cursor-move" v-if="text" :id="'text-' + text.index" :style="`top:${((text.y) * zoomScale * realScale) - 30}px;
            left:${((text.x) * zoomScale * realScale - 7)}px;
            width: ${(text.width * zoomScale * realScale) >= 44 ? (text.width* zoomScale * realScale) : 44}px;`" :key="textIdSelected" draggable
            @dragend="handleDragend">
            <div>
                <div>
                    <vs-row vs-type="flex" vs-align="flex-end" vs-justify="flex-end" :style="`background: #ffcc00;`">
                <v-popover class="button config" placement="top"
                           trigger="manual"
                           :open="openPopover"
                                   offset="12"
                           :auto-hide="false" >
                    <div class="tooltip-target" v-on:click="openPopover = !openPopover">
                        <i class="fas fa-cog"> </i>
                    </div>
                            <vs-row style="position: relative;max-width:230px" slot="popover">
                                <vs-col vs-type="flex" vs-w="6.2" class="text-right">
                            <vs-select class="dropdown" width="100%" v-model="fontFamily">
                                <vs-select-item value="shnmin" text="明朝体"></vs-select-item>
                                <vs-select-item value="shnkgo" text="角ゴシック体"></vs-select-item>
                                <vs-select-item value="shgyo" text="行書体"></vs-select-item>
                            </vs-select>
                        </vs-col>
                                <vs-col vs-type="flex" vs-w="3" >
                            <vs-select class="dropdown" width="100%" style="display: flex;" v-model="fontSize">
                                <vs-select-item :value="fontSize" :text="fontSize" v-for="fontSize in fontSizeOptions" :key="fontSize"></vs-select-item>
                            </vs-select>
                        </vs-col>
                                <vs-col  vs-w="2.8" >
                                    <div  width="100%" heigth="100%" style="padding-top: 9px;padding-left: 16px" class="color-box" @click="clickCurrentEvent">
                                        <colorPicker  v-model="color" ref="colorPicker" ></colorPicker>
                                    </div>
                                </vs-col>
                    </vs-row>
                </v-popover>
                <a class="button delete delete-text" v-on:click="onDeleteTextClick(text)"><i class="fas fa-times"> </i></a>
            </vs-row>
        </div>
                <div :id="'text_ext_' + text.index"></div>
    </div>
        </div>
        <div id="stick-note-append"></div>
    </div>
</template>
<script>
import { mapState, mapActions } from "vuex";
import colorPicker  from '@/components/color-picker/color-picker.vue'
import config from "../../app.config";
import store from "../../store/store";
import Axios from "axios";
import StickyNotePreview from "../stick-note/StickyNotePreview.vue";
import Vue from "vue";
import StickyNotePopup from "../../components/stick-note/StickyNotePopup";
export default {
  name: 'pdf-page-editor',
  components : {colorPicker},
  props: {
    page                : { type: Object},
    imageUrl            : { required: true },
    expectedSize        : { type: Object, required: true },
    imageScale          : { type: Number, required: true },
    isPublic            : { type: Boolean, default: false},
    enable              : { type: Boolean, default: true},
    areaSelectMode      : { type: Boolean, default: false },
    deleteFlg           : { type: Boolean, default: false},
    deleteWatermark     : { type: String, default: ''},
    rotateAngle         : { type: Number, default: 0 },
    opacity             : { type: Number, default: 0.5 },
    isVisible           : { type: Boolean, required: true },
    stamps              : { type: Array},
  },
  data() {
    return {
      ignoreUpdateText: false,
      dragDeltaX: 0,
      dragDeltaY: 0,
      openPopover: false,
      selectedId: 0,
      stampIndex: 0,
      textIndex: 2,
      font: '0',
      textIdSelected: 0,
      fontSizeOptions: [8,9,10,11,12,14,16,18,20,22,24,26,28,36,42,72],
        dragendPosition:{
          x:0, y:0,
        },
      maskBase64: '',
      maskBase64Landscape: '',
      originalSize: null, // 画像ファイルの画素数 {width: 0, height: 0}
      color: "#000000",
      defaultFontColor: '#000000', //デフォルトテキストフォント色
      changeColor:{},
      defaultFontSize: 10, //デフォルトテキストサイズ
      defaultFont: 'shnmin', //デフォルトテキストフォント
      manualCursorStyle: null,
      selectingArea: null, // マウスで選択中の領域
      pageTextData: {},
      textInfo: {},
    }
  },
  computed: {
    ...mapState({
      // stamps: state => state.home.stamps,
      stampSelected: state => state.home.stampSelected,
      isTextSelected: state => state.home.textSelected,
      isStickySelected: state => state.home.StickySelected,
      fileSelected: state => state.home.fileSelected,
      disabledProceed: state => state.home.disabledProceed,// 決定ボタンチェック
      zoom: state => state.home.fileSelected.zoom,
    }),
    zoomScale() {
      return this.zoom / 100;
    },
    isConfidential: function() {
      if(this.$store.state.home.fileSelected == null) {
        return false;
      }
      return this.$store.state.home.fileSelected.confidential_flg;
    },
    isOverlayHidden:function(){
      if(this.$store.state.home.fileSelected == null) {
        return false;
      }
      return this.$store.state.home.fileSelected.overlay_hidden_flg;

    },
    text: function() {
      return this.page.texts.find(item => item.index === this.textIdSelected);
    },
    fontFamily:{
        get: function () {
            return this.text.fontFamily;
        },
        set: function (newValue) {
            this.onTextChangeFont(this.text, newValue);
        }
    },
    fontSize:{
      get: function () {
          return Math.round(this.text.fontSize/(1.333333333 * this.zoomScale))
      },
      set: function (newValue) {
          this.onTextChangeFontSize(this.text, newValue);
      }
    },
    useKonva() {
      const page = this.page;
      return this.isVisible || page.stamps.length > 0 || page.texts.length > 0;
    },
    noZoomSize() {
      // zoom 反映前サイズ
      const size = this.originalSize ?? this.expectedSize;
      const pageScale = this.imageScale;
      return {
        width: pageScale * size.width,
        height: pageScale * size.height
      };
    },
    shownSize() {
      // zoom を反映したサイズ
      return {
        width: this.noZoomSize.width * this.zoomScale,
        height: this.noZoomSize.height * this.zoomScale,
      };
    },
    realScale() {
      return this.noZoomSize.width / this.page.width;
    },
    cursorStyle() {
      const styleByAreaSelectMode = this.areaSelectMode ? 'crosshair' : null;
      return this.manualCursorStyle ?? styleByAreaSelectMode ?? 'default';
    },
    handlersForAreaSelect() {
      // areaSelectMode 用定義
      const handlersForAreaSelect = {
        mousedown: this.mousedownForAreaSelect,
        mousemove: this.mousemoveForAreaSelect,
        mouseup: this.mouseupForAreaSelect,
        mouseleave: this.clearAreaSelect,
      };

      if (!this.areaSelectMode) {
        // 必要なければ何もしないようにしたものを返す
        const handlers = handlersForAreaSelect;

        const nop = () => {};
        for (const key of Object.keys(handlers)) {
            handlers[key] = nop;
    }

        return handlers;
      }

      if (this.enable) {
        // this.enable が true の場合は想定していないため
        throw new Error("enable must be false");
      }

      return handlersForAreaSelect;
  },
    areaSelectRectConfig() {
      if (!this.selectingArea) {
        return;
      }

      const {down: p1, current: p2} = this.selectingArea;
      return {
        x: p1.x,
        y: p1.y,
        width: p2.x - p1.x,
        height: p2.y - p1.y,
        stroke: 'red',
      };
    },
  },
  methods: {
    ...mapActions({
      addFileStamp: "home/addFileStamp",
      updateFileStamp: "home/updateFileStamp",
      deleteFileStamp: "home/deleteFileStamp",
      addFileText: "home/addFileText",
      updateFileText: "home/updateFileText",
      deleteFileText: "home/deleteFileText",
      undoAction: "home/undoAction",
      setStickyPosition: "home/setStickyPosition",
      updateStickyPosition: "home/updateStickyPosition",
    }),
    // areaSelectMode 用
    getRelativePointerPosition() {
      const stage = this.$refs.stage.getStage();
      // https://konvajs.org/docs/sandbox/Relative_Pointer_Position.html
      const transform = stage.getAbsoluteTransform().copy();
      transform.invert();

      const absolutePosition = stage.getPointerPosition();
      return transform.point(absolutePosition);
    },
    mousedownForAreaSelect(e) {
      const isLeft = e.evt.button === 0;
      if (!isLeft) {
        return;
      }

      const pos = this.getRelativePointerPosition();
      this.selectingArea = {
        down: pos,
        current: pos,
      };
    },
    mousemoveForAreaSelect() {
      if (!this.selectingArea) {
        return;
      }

      const pos = this.getRelativePointerPosition();
      this.selectingArea.current = pos;
    },
    mouseupForAreaSelect() {
      if (!this.selectingArea) {
        return;
      }

      const {down: p1, current: p2} = this.selectingArea;
      this.clearAreaSelect();

      if (p1.x == p2.x || p1.y == p2.y) {
          // 面ではない
          return;
      }

      const values = [p1.x, p1.y, p2.x, p2.y]
        .map(x => x / this.imageScale);
      this.$emit("area-selected", {
        x1: values[0],
        y1: values[1],
        x2: values[2],
        y2: values[3],
      });
    },
    clearAreaSelect() {
      this.selectingArea = null;
    },
    //
    createImageFromUrl: function(url) {
      const image = new Image();
      image.src = url;
      return image;
    },
    createImage: function(base64) {
      if(!base64) {
        return new Image();
      }
      return this.createImageFromUrl(`data:image/png;base64,${base64}`);
    },
    getImageSize: function(url) {
      return new Promise((resolve, reject) => {
        const img = new Image()
        if(this.deviceType && this.deviceType.isTablet) {
        img.onload = () => resolve({width: img.width * 7/10, height:img.height * 7/10});
        img.onerror = reject
        img.src = url;
        }
      });
    },
    createStampImage: function(stampId) {
      if(!stampId) {
        return new Image();
      }
      const stamp = this.stamps.find(item=> item.id === stampId);
      let image = new Image();
      if(stamp) image.src = `data:image/png;base64,${stamp.url}`;
      return image;
    },
    showStickyNotePreview () {
      const stickys = this.$store.state.home.fileSelected.sticky_notes;
      $(this.$refs.wrap).closest('.pdf-content').find('.stickynote').remove()
      let can_operate_view = !['sent_detail', 'completed_detail', 'received_view'].includes(this.$route.name);
      if (stickys.length) {
        for (var i=0;i<stickys.length;i++) {
            const StickyNoteComponent = Vue.extend(StickyNotePreview)
            const ComponentInstance = new StickyNoteComponent({
              store,
              // 付箋コメントを渡す
              propsData: {
                id: stickys[i].id,
                comment: stickys[i].note_text,
                note_format: stickys[i].note_format,
                edit_id: i,
                removed_flg: stickys[i].removed_flg,
                deleted_flg: stickys[i].deleted_flg,
                can_operate: stickys[i].is_author && can_operate_view,
                callAddStickNoteDialog: this.callAddStickNoteDialog
              },
            })
            ComponentInstance.$mount()
            // 親コンポーネントを取得
            const parent = $(this.$refs.wrap).closest('.pdf-content').find('.page[data-index="' + (Number(stickys[i].page_num) - 1) + '"]')[0]
            // 座標を取得
            const parentClientRect = parent.getBoundingClientRect()
            // カーソル位置に付箋の中心を持ってくるようにstyleを変更
            // レスポンシブ対応のため、「px」ではなく「%」で設定

            $(this.$refs.wrap).closest('.pdf-content').find('.page[data-index="' + (Number(stickys[i].page_num) - 1) + '"]').append(ComponentInstance.$el)
            if (stickys[i].left > 1 || stickys[i].top > 1) {
              let x_offset = 28
              ComponentInstance.$el.style.left = `${((stickys[i].left + x_offset) / parentClientRect.width) * 100}%`
              ComponentInstance.$el.style.top = `${(stickys[i].top / parentClientRect.height) * 100}%`
            } else {
              ComponentInstance.$el.style.left = stickys[i].left
              ComponentInstance.$el.style.top = stickys[i].top
            }
            let data = {
              edit_id: i,
              left: ComponentInstance.$el.style.left,
              top: ComponentInstance.$el.style.top,
            }
            this.updateStickyPosition(data)
        }
      }
    },
    // 付箋イコン選択の場合、double click
    callAddStickNoteDialog: function (edit_id) {
      let stick = this.fileSelected.sticky_notes[edit_id];
      let stickyPosition = {
        page_num: stick.page_num,
        left: stick.left,
        top: stick.top,
      };
      this.setStickyPosition(stickyPosition)
      const StickyNoteComponent = Vue.extend(StickyNotePopup)
      const ComponentInstance = new StickyNoteComponent({
        store,
          propsData: {
          note_format: stick.note_format,
          note_text: stick.note_text,
          note_index: edit_id,
        },
      })
      ComponentInstance.$mount()
      $("#stick-note-append").append(ComponentInstance.$el);
      this.$modal.show('add-note-modal')
    },
    onEditorClick: function (e) {
      if(!this.enable) return;
      //  特設サイト申請作成時に捺印できない。
      if (this.$store.state.home.circular && this.$store.state.home.circular.special_site_flg &&
          ((this.$store.state.home.usingPublicHash == true && this.$store.state.user && this.$store.state.user.email == this.$store.state.home.circular.create_user &&
              this.$store.state.user.current_edition_flg == this.$store.state.home.circular.edition_flg && this.$store.state.user.current_env_flg == this.$store.state.home.circular.env_flg &&
              this.$store.state.user.current_server_flg == this.$store.state.home.circular.server_flg) ||
              (this.$store.state.home.usingPublicHash == false && this.$store.state.user && this.$store.state.user.mst_user_id == this.$store.state.home.circular.mst_user_id))) {
        return;
      }
      if (this.isPublic && (this.selectedId === 0 && !this.isTextSelected && !this.isStickySelected)) {
        this.stampIndex++;
        const callback = () => {
          this.selectedId = this.stampIndex;
          this.loadStamps();
        }
        this.$emit('generateStamp',{event: e, page: this.page, realScale: this.realScale, stampIndex: this.stampIndex, selectedId: this.selectedId, callback:callback});
        return;
      }

      if (!this.isPublic && !this.isTextSelected && !this.isStickySelected && !this.stampSelected) {
        return;
      }
      // 付箋イコン選択の場合
      if(this.isStickySelected){
        let stickyPosition = {
          page_num: this.page.no,
          left: e.evt.layerX - 105,
          top: e.evt.layerY
        }
        this.setStickyPosition(stickyPosition)
        const StickyNoteComponent = Vue.extend(StickyNotePopup)
        const ComponentInstance = new StickyNoteComponent({
          store,
          // // 付箋コメントを渡す
          propsData: {
            note_format: '',
            note_text: '',
            note_index: '',
          },
        })
        ComponentInstance.$mount()
        $("#stick-note-append").append(ComponentInstance.$el);
        this.$modal.show('add-note-modal')
        return
      }

      let stage = this.$refs.stage.getStage();
      let layer = stage.children[0];

      const scale = this.zoomScale;
      if(!this.isTextSelected) {
        if(this.selectedId > 0) {
            if(e && e.target.id() !== this.selectedId) {
                this.undoAction();
            }
            this.updateFileStamp({stamp: {index:this.selectedId, selected: false,opacity:this.opacity}, pageno: this.page.no});
            this.selectedId = 0;

            this.loadStamps();
            //layer.draw();
            return;
        }
          // this.disabledConfirmation = false;
        this.stampIndex++;
        //const stamp = {id: this.stampSelected.id,url:this.stampSelected.url, width: this.stampSelected.width, height: this.stampSelected.height, x:e.evt.offsetX, y:e.evt.offsetY};
        const stampWithoutUrl = {
            index: this.stampIndex,
            id: this.stampSelected.id,
            width: this.stampSelected.width,
            height: this.stampSelected.height,
            time_stamp_permission: this.stampSelected.time_stamp_permission,
            x: ((e.evt.layerX - (this.stampSelected.width * scale * this.realScale) / 2) / scale) / this.realScale,
            y: ((e.evt.layerY - (this.stampSelected.height * scale * this.realScale) / 2) / scale) / this.realScale,
            scaleX: 1,
            scaleY: 1,
            rotation: -this.rotateAngle,
            selected: true,
        };
        //this.page.stamps.push(stamp);
        this.addFileStamp({stamp: stampWithoutUrl, pageno: this.page.no});
        this.selectedId = this.stampIndex;

        //Draw new stamp with selected
        let stage = this.$refs.stage.getStage();
        let layer = stage.children[0];
        let groups = layer.find('Group').toArray();
        let group = groups[0];

        let stampNode = new Konva.Image({
            image: this.createStampImage(stampWithoutUrl.id),
            id: stampWithoutUrl.index,
            draggable: stampWithoutUrl.selected,
            width: stampWithoutUrl.width * this.realScale,
            height: stampWithoutUrl.height * this.realScale,
            x: stampWithoutUrl.x * this.realScale,
            y: stampWithoutUrl.y * this.realScale,
            scaleX: stampWithoutUrl.scaleX,
            scaleY: stampWithoutUrl.scaleY,
            rotation: stampWithoutUrl.rotation,
            stroke: '#0984e3',
            strokeWidth: (stampWithoutUrl.selected ? 2: 0),
            opacity: (stampWithoutUrl.selected ? 0.5: 0),
            dash: [5, 5],
            type: 'Stamp'
        });
          const rotatePoint = ({ x, y }, rad) => {
              const rcos = Math.cos(rad);
              const rsin = Math.sin(rad);
              return { x: x * rcos - y * rsin, y: y * rcos + x * rsin };
          };
          const topLeft = { x: -stampNode.width() / 2, y: -stampNode.height() / 2 };
          const current = rotatePoint(topLeft, Konva.getAngle(0));
          const rotated = rotatePoint(topLeft, Konva.getAngle(-this.rotateAngle));
          const dx = rotated.x - current.x,
                dy = rotated.y - current.y;
          stampNode.x(stampNode.x() + dx)
          stampNode.y(stampNode.y() + dy)

        group.add(stampNode);
        layer.draw();

      } else {
        const targetId = e.target.attrs.id;
        if (e.target.attrs.type === 'Text') {
            this.textIdSelected = targetId;
            this.color = Object.prototype.hasOwnProperty.call(this.changeColor, "id_" + this.textIdSelected) ? this.changeColor['id_'+this.textIdSelected] : this.textInfo.last_text_color
            var textarea = document.getElementsByClassName("canvas-textarea")[0];
            if(textarea){
                textarea.style.color = this.color
                textarea.style.removeProperty("background");
            }
            return;
        }
        if(this.textIdSelected > 0) {
            this.textIdSelected = 0;
            return;
        }
        this.color = this.textInfo.last_text_color
        this.textIndex++;
        this.textIdSelected = this.textIndex;
        let textNode = new Konva.Text({
            id: this.textIndex,
            text: '',
            x: e.evt.layerX / scale,
            y: e.evt.layerY / scale,
            //20210127 渡邉　デフォルトフォントサイズを20から10に変更
            //fontSize: Math.round(20*1.333333333*scale* this.realScale),
            fontSize: Math.round(this.textInfo.last_text_size*1.333333333*scale* this.realScale),
            fontFamily: this.textInfo.last_text_font,
            fontColor: Object.prototype.hasOwnProperty.call(this.changeColor, "id_" + this.textIdSelected) ?this.changeColor['id_'+this.textIdSelected] : this.textInfo.last_text_color ,
            fill: Object.prototype.hasOwnProperty.call(this.changeColor, "id_" + this.textIdSelected) ?this.changeColor['id_'+this.textIdSelected] : this.textInfo.last_text_color ,
            draggable: true,
            width: 50,
            type: 'Text'
        });
        if(!Object.prototype.hasOwnProperty.call(this.changeColor, "id_" + this.textIdSelected)){
            this.changeColor['id_'+this.textIdSelected] = this.textInfo.last_text_color
        }
          $(document).on('dragenter',".konvajs-content", (e) => {
            e.preventDefault();
        });
          $(document).on('dragover',".konvajs-content", this.handleDragover);
        let textPosition = {
            x: e.evt.layerX,
            y: e.evt.layerY,
        }
        textNode.on('mouseover', () => {
            this.manualCursorStyle = 'move';
        });

        textNode.on('mouseout', () => {
            this.manualCursorStyle = null;
        });

        textNode.on('transform', function () {
            // reset scale, so only with is changing by transformer
            textNode.setAttrs({
                width: textNode.width() * textNode.scaleX(),
                scaleX: 1
            });
        });
        const text = {
            index: textNode.id(),
            text: textNode.text(),
            x: textNode.x() / this.realScale,
            y: textNode.y() / this.realScale,
            fontSize: textNode.fontSize() / this.realScale,
            fontFamily: textNode.fontFamily(),
            fontColor: Object.prototype.hasOwnProperty.call(this.changeColor, "id_" + textNode.id()) ?this.changeColor['id_'+textNode.id()] : this.textInfo.last_text_color ,
            width: textNode.width() / this.realScale,
            scaleX: textNode.scaleX(),
            scaleY: textNode.scaleY(),
            rotation: textNode.rotation()
        };
        this.addFileText({text: text, pageno: this.page.no});
        this.onTextDblClick(stage,textNode,textPosition, layer, true,false);
      }
    },
    handleDragstart(e) {
        const targetId = e.target.attrs.id;
        if (e.target.attrs.type === 'Text') {
            this.textIdSelected = targetId;
            const optionsElement = document.getElementById('text-' + targetId);
            if (optionsElement) {
                optionsElement.style.left = (e.target.x() * this.zoomScale)  + 'px';
                optionsElement.style.top = (e.target.y() * this.zoomScale - 30) + 'px';
            }
        }
        this.openPopover = false;
    },
    handleDragmove(e) {
        if (e.target.attrs.type === 'Text') {
            var textId = e.target.id();
            const optionsElement = document.getElementById('text-' + textId);
            if (optionsElement) {
                optionsElement.style.left = (e.target.x() * this.zoomScale) + 'px';
                optionsElement.style.top = (e.target.y() * this.zoomScale - 30) + 'px';
            }
        }
    },
    handleDragover(e) {
        //if (this.text && this.text.text){
        if (!this.text) {
            e.preventDefault();
            return;
        }
        let $this = this;
            $('#text-' + this.textIdSelected).hide();
        this.$nextTick(()=>{
                try{
            let stage = this.$refs.stage.getStage();
                    let textNode = stage.findOne(node => node.attrs.type === 'Text' && (node.attrs.id === $this.textIdSelected));
            const layer = stage.children[0];
            let offset = $(e.target).offset();
            var textarea = document.getElementsByClassName("canvas-textarea")[0];

                    var x = (e.pageX - offset.left - $this.dragDeltaX) / this.zoomScale;
                    var y = (e.pageY - offset.top - $this.dragDeltaY + 30) / this.zoomScale;
                textNode.x(x);
                textNode.y(y);
                if(textarea){
                    textarea.style.left = textNode.x() + 'px';
                    textarea.style.top = textNode.y() + 'px';
                }
                    if(!textarea){
                        textNode.show();
                    }
                layer.batchDraw();
                }catch (e){

        }
        });
    },
    handleDragend(e) {
        if ($(e.target).hasClass('text-options')){
            const optionsElement = document.getElementById('text-' + this.textIdSelected);
            if (optionsElement) {
                this.$nextTick(()=>{
                let stage = this.$refs.stage.getStage();
                const layer = stage.children[0];
                let textNode = stage.findOne(node => node.attrs.type === 'Text' && (node.attrs.id === this.textIdSelected));
                textNode.show();
                optionsElement.style.left = textNode.x() + 'px';
                optionsElement.style.top = (textNode.y() - 30)+ 'px';
                this.ignoreUpdateText = true;
                $(optionsElement).show();
                var textarea = document.getElementsByClassName("canvas-textarea")[0];
                if(textarea){
                    textarea.style.left = textNode.x()*textNode.scaleX() + 'px';
                    textarea.style.top = textNode.y()*textNode.scaleY() + 'px';
                    this.dragendPosition = {
                        x:textNode.x(),
                        y:textNode.y(),
                    };
                }
                    this.doUpdateText(textNode);
                    layer.draw();
                });

            }
        }else {
            if(e.target.attrs.type === 'Stamp' && e.evt) {
                this.doUpdateStamp(e);
            }
            if(e.target.attrs.type === 'Text') {
                this.doUpdateText(e.target);
                let stage = this.$refs.stage.getStage();
                let layer = stage.children[0];
                setTimeout(()=> {
                    // this.onTextDblClick(stage,e.target,textPosition, layer, false);
                    stage.find(node => node.attrs.type === 'Text' && node.attrs.id === e.target.id()).show();
                    layer.batchDraw();
                });
            }
        }
    },
    //テキスト更新
    doUpdateText: function(textNode) {
        textNode.attrs.fill = Object.prototype.hasOwnProperty.call(this.changeColor, "id_" + textNode.id()) &&
        this.changeColor['id_'+textNode.id()] != undefined ?this.changeColor['id_'+textNode.id()] : this.textInfo.last_text_color
        textNode.fontColor = textNode.attrs.fill
        // PAC_5-1976   テキスト情報を更新する時、domが更新遅れたの原因で、取得失敗の問題を発生されます。
        try{
            const stage = this.$refs.stage.getStage();
            const layer = stage.children[0];
            if(!stage) {
                return;
            }
            const nodeText = layer.find('Text').toArray().find(item => item.attrs.id == textNode.attrs.id);
            if(!nodeText) {
                return;
            }
            let newWidthVal = this.createNewNodeTextAttr(textNode.attrs)
        const text = {
            index: textNode.id(),
            text: textNode.text(),
            x: textNode.x() / this.realScale,
            y: textNode.y() / this.realScale,
                fontSize: textNode.attrs.fontSize / this.realScale,
            fontFamily: textNode.fontFamily(),
                fontColor: textNode.attrs.fill,
                width: newWidthVal / this.realScale,
            scaleX: textNode.scaleX(),
            scaleY: textNode.scaleY(),
            rotation: textNode.rotation()
        };
        this.updateFileText({text: text, pageno: this.page.no});
        }catch (e){}
    },
    //テキスト設定保存
    async updateTextSetting() {
      await Axios.post(`${config.BASE_API_URL}${store.state.home.usingPublicHash ? '/public': ''}/updateTextSetting`,
          {info: this.textInfo, usingHash: store.state.home.usingPublicHash})
          .then(response => {
            return response.data ? response.data.data : [];
          })
          .catch(error => {
            return [];
          });
    },
    doUpdateStamp: function(e) {
        const rotatePoint = ({ x, y }, rad) => {
            const rcos = Math.cos(rad);
            const rsin = Math.sin(rad);
            return { x: x * rcos - y * rsin, y: y * rcos + x * rsin };
        };
        const topLeft = { x: -e.target.width() / 2, y: -e.target.height() / 2 };
        const current = rotatePoint(topLeft, Konva.getAngle(0));
        const rotated = rotatePoint(topLeft, Konva.getAngle(e.target.rotation()));
        const dx = rotated.x - current.x,
            dy = rotated.y - current.y;
        let stampPos = {
            x:e.target.x() - dx,
            y: e.target.y() - dy
        }
        const stamp = {
            index: e.target.attrs.id,
            x: stampPos.x / this.realScale,
            y: stampPos.y / this.realScale,
            width: e.target.width() / this.realScale,
            height: e.target.height() / this.realScale,
            scaleX: e.target.scaleX(),
            scaleY: e.target.scaleY(),
            rotation: e.target.rotation(),
            selected: true,
        };
        this.updateFileStamp({stamp:stamp, pageno: this.page.no});
    },
    // PAC_5-1715 add color and  change dragend
    drawingTem: function (textID) {
        let $this = this
        const stage = this.$refs.stage.getStage();
        const layer = stage.children[0];
        if(!stage) {
            return;
        }
        if($this.pageTextData[$this.page.no + '_'+textID]){
            return ;
        }
        let nodeText = layer.find('Text').toArray().find(item => item.attrs.id == textID);
        var textarea = document.getElementsByClassName("canvas-textarea")[0];
        if(textarea){
            var nodeOpenList = $("div[id^='text-']");
            if(nodeOpenList.eq(0).attr("id") != 'text-'+textID){
                return ;
            }
            if(textarea.getAttribute("textNodeID") == textID){
                this.color = nodeText.attrs.fontColor;
            }
        }else{
            this.color = nodeText.attrs.fontColor;
        }

        $this.textIdSelected = textID
        this.openPopover = false;
        let divSpace = document.createElement('div');
        let currentNode = document.getElementById('text_ext_' + textID)

        const fakeEl = $('<pre>').hide().appendTo(document.body);
        let html = nodeText.text();
        html = html.replace(/[\r\n]/g, "<br />");
        fakeEl.html(html).css('font-size', nodeText.fontSize());
        fakeEl.css('font-family', nodeText.fontFamily());
        const currentHeight  = fakeEl.height()
        fakeEl.remove();
        let newWidthVal = this.createNewNodeTextAttr({text:nodeText.text(),fontFamily:nodeText.fontFamily(),fontSize:nodeText.fontSize()});
        $('#text-' + this.textIdSelected).css('width', (newWidthVal+10)+"px");
        //this.applyText(nodeText);
        divSpace.style.width = (nodeText.attrs.width) +'px';
        divSpace.style.height = currentHeight+'px';
        //divSpace.style.backgroundColor = "rgba(0,0,0,0)";
        divSpace.style.opacity = "0";
        // PAC_5-1976  タブを切り替える時、画像レイアーの問題は発生します。
        // 下記のコードで該当問題を修正することはできます。
        // テキスト情報を更新し、domを更新することでデータ異常問題対応します。
        if(!Object.prototype.hasOwnProperty.call($this.pageTextData, $this.page.no)){
            $this.textIdSelected = textID
            const scale = $this.zoomScale;
            let textPosition = {
                x: nodeText.x()*scale,
                y: nodeText.y()*scale
            }
            $this.onTextDblClick(stage,nodeText,textPosition, layer, true,true);
            nodeText.show();
            layer.draw();
            $(".canvas-textarea").eq(0).hide();
            // PAC_5-1976 レンダリングされるかどうかのフラグを追加することで、複数回レンダリングことを回避する。
            $this.pageTextData[$this.page.no] = 1;
        }
        divSpace.addEventListener('click', function (e) {
            let nodeText = layer.find('Text').toArray().find(item => item.attrs.id == textID);
            $this.textIdSelected = textID
            const scale = $this.zoomScale;
            let textPosition = {
                x: nodeText.x()*scale,
                y: nodeText.y()*scale
            }
            $this.onTextDblClick(stage,nodeText,textPosition, layer, true,true);
        });
        divSpace.addEventListener('dragstart', function (e) {

        });
        divSpace.addEventListener('dragover', function (e) {
                var textId = textID;
                $('#text-' + textID).show();
                const optionsElement = $('text-' + textId);
                if (optionsElement) {
                    optionsElement.css("left",(e.target.x * $this.zoomScale) + 'px');
                    optionsElement.css("top",(e.target.y * $this.zoomScale - 30) + 'px');
                }
            $('#text-' + textID).hide();
        });
        divSpace.addEventListener('dragend', function (e) {

        });
        $('#text-' + textID).show();
        try{
            currentNode.appendChild(divSpace);
        }catch(e){
        }
    },
    clickCurrentEvent: function(){
        this.$refs.colorPicker.openPanel()
    },
    loadTexts: function (draggable) {
        if(!this.$refs.stage) {
            return;
        }
        let stage = this.$refs.stage.getStage();
        let layer = stage.children[0];
        this.textIndex = 2;
        layer.find('Text').destroy();
        for(let text of this.page.texts) {
            if(!Object.prototype.hasOwnProperty.call(this.changeColor, "id_" + text.index)){
                this.changeColor['id_' + text.index] = text.fontColor
            }
            let newWidthVal = this.createNewNodeTextAttr(text);
            this.textIndex = text.index;
            let textNode = new Konva.Text({
                id: this.textIndex,
                text: text.text,
                x: text.x * this.realScale,
                y: text.y * this.realScale,
                fontSize: text.fontSize* this.realScale,
                fontFamily: text.fontFamily,
                fontColor: this.changeColor['id_' + text.index],
                draggable: draggable,
                width: newWidthVal  * this.realScale  + 10,
                fill: this.changeColor['id_' + text.index],
            //    height: 48 * this.realScale,
                scaleX: text.scaleX,
                scaleY: text.scaleY,
                rotation: text.rotation,
                type: 'Text'
            });

            //textNode.zIndex(-1);

            if(textNode.attrs.id === this.textIdSelected && $(this.$refs.wrap).has('textarea:visible').length) {
                textNode.hide();
            }

            layer.add(textNode)
            textNode.hitStrokeWidth('auto');
            textNode.shadowForStrokeEnabled(false);
            this.$nextTick(() => {
                const scale = this.zoomScale;
                let textPosition = {
                    x: textNode.x()*scale,
                    y: textNode.y()*scale
                }

            textNode.on('dragstart', (e) => {
                this.textIdSelected = 0;
                    console.log(e)
            });

            textNode.on('dragend', (e) => {
                    this.openPopover = false;
                    if(this.isTextSelected) {
                      this.drawingTem(e.target.id());
                    }
            });

                textNode.on('mouseover', () => {
                  if(this.isTextSelected) {
                    this.manualCursorStyle = 'move';
                    this.drawingTem(textNode.attrs.id);
                    // @todo
                    this.pageTextData[this.page.no + '_' + textNode.attrs.id] = 1;
                  }
            });

                textNode.on('mouseout', () => {
                    $('#text-' + this.textIdSelected).css('heigth', `auto`);
                    this.manualCursorStyle = null;
                    // @todo
                    delete this.pageTextData[this.page.no + '_'+textNode.attrs.id]
                    //delete this.pageTextData[this.page.no]
            });

                textNode.on('transform', () => {
                // reset scale, so only with is changing by transformer
                textNode.setAttrs({
                    width: textNode.width() * textNode.scaleX(),
                    scaleX: 1
                });
            });

            textNode.on('click', (e) => {
                    this.onTextDblClick(stage,textNode,textPosition, layer, true,true);
            });
                textNode.on('transformend', (e) => {
                    this.doUpdateText(e.target);
            });
            });

            layer.batchDraw();
        }
    },
    loadStamps: function () {
        if(!this.$refs.stage) {
            return;
        }
        let stage = this.$refs.stage.getStage();
        let layer = stage.children[0];
        layer.find('Group').destroy();
        const group = new Konva.Group();
        layer.add(group);

        ///const mask = group.find('Image').toArray().find(item => item.attrs.type === 'Mask');
        //if(mask) mask.destroy();
        const rotatePoint = ({ x, y }, rad) => {
            const rcos = Math.cos(rad);
            const rsin = Math.sin(rad);
            return { x: x * rcos - y * rsin, y: y * rcos + x * rsin };
        };
        for(let stamp of this.page.stamps) {
            let stampNode = new Konva.Image({
                image: this.createStampImage(stamp.id),
                id: stamp.index,
                draggable: stamp.selected,
                width: stamp.width * this.realScale,
                height: stamp.height * this.realScale,
                x: stamp.x * this.realScale,
                y: stamp.y * this.realScale,
                scaleX: stamp.scaleX,
                scaleY: stamp.scaleY,
                rotation: stamp.rotation,
                stroke: '#0984e3',
                strokeWidth: (stamp.selected ? 2: 0),
                opacity: (stamp.selected ? 0.5 : stamp.opacity),
                dash: [5, 5],
                type: 'Stamp'
            });
            const topLeft = { x: -stampNode.width() / 2, y: -stampNode.height() / 2 };
            const current = rotatePoint(topLeft, Konva.getAngle(0));
            const rotated = rotatePoint(topLeft, Konva.getAngle(stamp.rotation));
            const dx = rotated.x - current.x,
                  dy = rotated.y - current.y;
            stampNode.x(stampNode.x() + dx)
            stampNode.y(stampNode.y() + dy)
            group.add(stampNode);
        }
        if(this.isConfidential && !this.isOverlayHidden) {
            const image1 = new Konva.Image({
                image: this.createImage((this.noZoomSize.width < this.noZoomSize.height ? this.maskBase64 : this.maskBase64Landscape)),
                id: 0,
                draggable: false,
                width: this.noZoomSize.width,
                height: this.noZoomSize.height,
                type: 'Mask',
            });

            group.add(image1);
        }
        if(this.deleteFlg && this.deleteWatermark) {
            const image1 = new Konva.Image({
                image: this.createImage(this.deleteWatermark),
                id: 0,
                draggable: false,
                width: this.noZoomSize.width,
                height: this.noZoomSize.height,
                type: 'Mask',
            });

            group.add(image1);
        }

        //layer.add(group);
        layer.batchDraw();
        setTimeout(()=> {
            //layer.draw();
        },100);

    },
    onStampTransform: function (e) {
        this.doUpdateStamp(e);

    },
    onTextDblClick: function(stage,textNode,textPosition,layer,isAdd,isChangeFlg) {
        if(!this.isTextSelected) {
            return;
        }
        const $this = this;
        const scale = this.zoomScale;
        // hide text node and transformer:
        stage.find(node => node.attrs.type === 'Text' && (node.attrs.id === textNode.attrs.id || node.attrs.id === this.textIdSelected)).hide();
        layer.draw();

        // create textarea over canvas with absolute position
        // first we need to find position for textarea
        // how to find it?

        // at first lets find position of text node relative to the stage:
        // let textPosition = textNode.absolutePosition();

        // then lets find position of stage container on the page:
        let stageBox = stage.container().getBoundingClientRect();

        // so position of textarea will be the sum of positions above:
        let areaPosition = {
            /*x: stageBox.left + textPosition.x,
            y: stageBox.top + textPosition.y*/
            x: textPosition.x,
            y: textPosition.y
        };

        // create textarea and style it
        let textarea = document.createElement('textarea');
        //document.body.appendChild(textarea);
        this.$refs.wrap.appendChild(textarea)
        let newWidthVal = this.createNewNodeTextAttr({text:textNode.text(),fontFamily:textNode.fontFamily(),fontSize:textNode.fontSize()});
        // apply many styles to match text on canvas as close as possible
        // remember that text rendering on canvas and on the textarea can be different
        // and sometimes it is hard to make it 100% the same. But we will try...
        textarea.value = textNode.text();
        textarea.className="canvas-textarea";
        textarea.spellcheck = false;
        textarea.placeholder = 'あa';
        textarea.setAttribute("textNodeID",textNode.attrs.id);
        textarea.wrap = 'off';
        textarea.style.position = 'absolute';
        textarea.style.top = (areaPosition.y) -3 + 'px';
        textarea.style.left = (areaPosition.x) -7+ 'px';
        textarea.style.width = `${((newWidthVal * scale  - textNode.padding() * 2 )>=44) ? (newWidthVal * scale  - textNode.padding() * 2  + 25 ): 44}px`;
        textarea.style.height = textNode.height() * scale - textNode.padding() + 5 + 'px';
        textarea.style.fontSize = (textNode.fontSize() * scale) + 'px';
        textarea.style.border = 'none';
        textarea.style.margin = '0px';
        textarea.style.overflow = 'hidden';
        //textarea.style.background = 'none';
        textarea.style.outline = 'none';
        textarea.style.resize = 'none';
        textarea.style.lineHeight = 1.3;//textNode.lineHeight();
        textarea.style.fontFamily = textNode.fontFamily();
        textarea.style.transformOrigin = 'left top';
        textarea.style.textAlign = textNode.align();
        textarea.style.color = Object.prototype.hasOwnProperty.call(this.changeColor, "id_" + textNode.id()) ?this.changeColor['id_'+textNode.id()] : this.textInfo.last_text_color;
        textarea.style.padding = '5px';
        textarea.style.border = '2px solid #0984e3';
        textarea.style.whiteSpace = 'pre';
        textarea.style.overflowWrap = 'anywhere';
        let rotation = textNode.rotation();
        let transform = '';
        if (rotation) {
            transform += 'rotateZ(' + rotation + 'deg)';
        }
        let  px = Math.round(textNode.fontSize() / 20) + 1 + 1/scale ;
        transform += 'translateY(-' + px + 'px)';

        textarea.style.transform = transform;
        // reset height
        textarea.style.height = 'auto';
        // after browsers resized it we can set actual value
        textarea.style.height = textarea.scrollHeight - 11 + 'px';

        textarea.focus();
        $('#text-' + this.textIdSelected).css('width', textarea.style.width);
        this.openPopover = false;
        function removeTextarea() {
            //$this.textIdSelected = 0;
            if (textarea && textarea.parentNode){
                textarea.parentNode.removeChild(textarea);
                window.removeEventListener('click', handleOutsideClick);
                stage.find(node => node.attrs.type === 'Text' && node.attrs.id === textNode.attrs.id).show();
                layer.draw();
            }
        }

        function setTextareaWidth(newWidth) {
            if (!newWidth) {
                // set width for placeholder
                newWidth = textNode.placeholder.length * textNode.fontSize();
            }
            // some extra fixes on different browsers
            let isSafari = /^((?!chrome|android).)*safari/i.test(
                navigator.userAgent
            );
            let isFirefox =
                navigator.userAgent.toLowerCase().indexOf('firefox') > -1;
            if (isSafari || isFirefox) {
                newWidth = Math.ceil(newWidth);
            }

            let isEdge =
                document.documentMode || /Edge/.test(navigator.userAgent);
            if (isEdge) {
                newWidth += 1;
            }
            textarea.style.width = newWidth >=44 ? newWidth : 44 + 'px';
        }

        textarea.addEventListener('focus', function (e) {
            $this.openPopover = false;
        });

        textarea.addEventListener('keydown', function (e) {
            let inputWidth = this.scrollWidth;
            let textAreaWidth = (inputWidth+10)>=44 ? (inputWidth+10) : 44;
            $(this).width(10).height(10).width(this.scrollWidth).height(this.scrollHeight -3);
            textNode.width(inputWidth+10);
            $('#text-' + $this.textIdSelected).css('width', `${textAreaWidth}px`);
            setTextareaWidth(textAreaWidth);
            // const optionsElement = document.getElementById(`text-${textNode.attrs.id}`);
            // optionsElement.style.left = `${textNode.x() + ((inputWidth + 10) * scale) }px`;
        });
        textarea.addEventListener('change', function(e) {
            textNode.text(e.target.value);
            $this.applyText(textNode);
        })

        function handleOutsideClick(e) {
            if (e.target !== textarea) {
                if($this.dragendPosition.x&&$this.dragendPosition.y) {
                    textNode.x($this.dragendPosition.x + 7);
                    textNode.y($this.dragendPosition.y + 3);
                    $this.dragendPosition.x = 0;
                    $this.dragendPosition.y = 0;
                }
                textNode.text(textarea.value);
                if(e.target.className === 'v-popover button config') {
                    const text = stage.find(node => node.attrs.type === 'Text' && node.attrs.id === textNode.attrs.id);
                    text.text(textarea.value);
                    text.width(textNode.width());
                    layer.draw();
                }
                if(isAdd) {
                    const text = $this.page.texts.find(item => item.index === textNode.id());
                    if (text){
                        textNode.fontFamily(text.fontFamily);
                        textNode.fontSize(text.fontSize * $this.realScale);
                        if(isChangeFlg == false){
                            textNode.fill = $this.color;
                            $this.changeColor['id_'+textNode.id()] = $this.color;
                        }
                        $this.applyText(textNode);
                    }
                    if(e.target.className !== 'con-select dropdown' &&
                        !textarea.value
                      //&&  !$(e.target).hasClass('vs-select--item')
                        && !$(e.target).hasClass('tooltip-target')&&
                        $(e.target).prop("tagName").toLowerCase() !== 'body' &&
                        $(e.target).closest('.config').length <= 0 &&
                        $(e.target).closest('.vs-select--options').length <= 0  &&
                        $(e.target).closest('.adjust-zoom').length <= 0 &&
                        $(e.target).closest('.text-options').length <= 0) {
                        if(e.target.className !== 'button delete delete-text') $this.deleteFileText({text: text, pageno: $this.page.no});
                    }
                }
                if(e.target.className !== 'con-select dropdown' &&
                 //   e.target.className !== 'vs-select--item' &&
                    $(e.target).prop("tagName").toLowerCase() !== 'body'
                    && $(e.target).closest('.config').length <= 0
                    // && $(e.target).closest('.vs-select--options').length <= 0
                    && $(e.target).closest('.adjust-zoom').length <= 0
                    ) {
                    if($(e.target).closest('.delete-text').length >0) {
                        $this.doUpdateText(textNode);
                        if(textarea) {
                            removeTextarea();
                            $this.openPopover = false;
                        }
                    }
                    if($(e.target).closest('.text-options').length <= 0) {
                        if(textNode.text().length<=400){
                            $this.doUpdateText(textNode);
                        }else{
                            alert("挿入するテキストは400文字が上限です");
                        }
                        if(textarea) {
                            removeTextarea();
                            //$this.openPopover = false;
                        }
                    }
                }
            }
        }

        function handleOutsideDragstart(e) {
            if ($(e.target).hasClass('text-options')) {
                let offset = $(e.target).offset();
                $this.dragDeltaX = e.pageX - offset.left;
                $this.dragDeltaY = e.pageY - offset.top;
                if (textarea.value){
                    textNode.text(textarea.value);
                    if(isAdd) {
                        const text = $this.page.texts.find(item => item.index === textNode.id());
                        if (text){
                            textNode.fontFamily(text.fontFamily);
                            textNode.fontSize(text.fontSize*$this.realScale);
                            $this.applyText(textNode);
                            isAdd = false;
                        }
                    }
                    if(textarea) {
                        removeTextarea();
                        $this.openPopover = false;
                    }
                    e.dataTransfer.effectAllowed = "move";
                }else{
                    e.dataTransfer.effectAllowed = "none";
                }
            }
        }
        function handleOutsideDragMove(e) {
            e.stopPropagation();
            setTimeout(() => {
                var textId = textNode.attrs.id;
                $('#text-' + textId).show();
                const optionsElement = $('text-' + textId);
                if (optionsElement) {
                    optionsElement.css("left",(e.target.x * $this.zoomScale) + 'px');
                    optionsElement.css("top",(e.target.y * $this.zoomScale - 30) + 'px');
                }
                $('#text-' + textId).hide();
            });
            window.removeEventListener('dragover', handleOutsideDragMove);
            window.addEventListener('dragover', handleOutsideDragMove);
            return false;
        }

        setTimeout(() => {
            window.addEventListener('click', handleOutsideClick);
            window.addEventListener('dragstart', handleOutsideDragstart);
            window.addEventListener('dragover', handleOutsideDragMove);
        });
    },
    onTextChangeFont(text, fontFamily) {
        const stage = this.$refs.stage.getStage();
        const layer = stage.children[0];
        if(!stage) {
            return;
        }

        const nodeText = layer.find('Text').toArray().find(item => item.attrs.id == text.index);
        if(!nodeText) {
            return;
        }
      if (nodeText.fontFamily() != fontFamily) {
        this.textInfo.last_text_font = fontFamily;
        this.updateTextSetting();
      }
        nodeText.fontFamily(fontFamily);
        this.applyText(nodeText);
        setTimeout(() => {
            stage.find(node => node.attrs.type === 'Text' && node.attrs.id === nodeText.attrs.id).show();
            layer.draw();
        })
    },
    onTextChangeFontSize(text, fontSize) {
        const stage = this.$refs.stage.getStage();
        const layer = stage.children[0];
        if(!stage) {
            return;
        }
        const nodeText = layer.find('Text').toArray().find(item => item.attrs.id == text.index);
        if(!nodeText) {
            return;
        }
      if (nodeText.fontSize() != Math.round(fontSize * 1.333333333 * this.zoomScale * this.realScale)) {
        this.textInfo.last_text_size = fontSize;
        this.updateTextSetting();
      }
        nodeText.fontSize(Math.round(fontSize*1.333333333* this.zoomScale * this.realScale));
        layer.draw();
        this.applyText(nodeText)
        setTimeout(() => {
            stage.find(node => node.attrs.type === 'Text' && node.attrs.id === nodeText.attrs.id).show();
            layer.draw();
        })
    },
    onTextChangeFontColor(fontColor){
        let text = this.text
        const stage = this.$refs.stage.getStage();
        //document.getElementsByClassName("canvas-textarea")[0].style.color = (fontColor ? fontColor : this.defaultColor);
        var textarea = document.getElementsByClassName("canvas-textarea")[0];
        if(textarea){
            textarea.style.color = fontColor
            textarea.style.removeProperty("background");
        }
        const layer = stage.children[0];
        if(!stage) {
            return;
        }
        this.changeColor['id_'+ text.index] = fontColor
        const nodeText = layer.find('Text').toArray().find(item => item.attrs.id == text.index);
        if(!nodeText) {
            return;
        }
        if (text.fontColor != fontColor) {
          this.textInfo.last_text_color = fontColor;
          this.updateTextSetting();
        }
        nodeText.fontColor = fontColor;
        layer.draw();
        this.applyText(nodeText)
        setTimeout(() => {
            stage.find(node => node.attrs.type === 'Text' && node.attrs.id === nodeText.attrs.id).show();
            layer.draw();
        })
    },
    applyText(nodeText){
        const fakeEl = $('<pre>').hide().appendTo(document.body);
        let html = nodeText.text();
        html = html.replace(/[\r\n]/g, "<br />");
        fakeEl.html(html).css('font-size', nodeText.fontSize());
        fakeEl.css('font-family', nodeText.fontFamily());
        const width = fakeEl.width();
        nodeText.width(width + 10);
        nodeText.height('auto');
        if(nodeText.text().length<=400){
            this.doUpdateText(nodeText);
        }
        fakeEl.remove();
    },
    //PAC_5-2241 テキスト追加で半角スペース2回入力を2回行うと確定時に改行される
    createNewNodeTextAttr: function(nodeText){
        const fakeEl = $('<pre>').hide().appendTo(document.body);
        let html = nodeText.text;
        html = html.replace(/[\r\n]/g, "<br />");
        fakeEl.html(html).css('font-size', nodeText.fontSize);
        fakeEl.css('font-family', nodeText.fontFamily);
        const width = fakeEl.width();
        fakeEl.remove();
        return width;
    },
    onDeleteTextClick: function (text) {
        const stage = this.$refs.stage.getStage();
        const layer = stage.children[0];
        if(!stage) {
            return;
        }
        const nodeText = layer.find('Text').toArray().find(item => item.attrs.id == text.index);
        if(!nodeText) {
            return;
        }
        nodeText.destroy();
        layer.draw();
        this.textIdSelected = 0;
        this.deleteFileText({text: text, pageno: this.page.no});
    },
    onWindowClick: function(e) {
        if(!this.isTextSelected && this.selectedId && $(e.target).prop("tagName").toUpperCase() !== 'CANVAS' && !this.isPublic) {
            if($(e.target).closest('.submit-circular').length > 0) return;
            if($(e.target).closest('.submit-circular2').length > 0){
                this.onEditorClick(null);
                return;
            }
            this.undoAction();
            this.selectedId = 0;
            this.textIdSelected = 0;
            // this.disabledConfirmation = true;
        }
    },
    exportToImage: function() {
        const stage = this.$refs.stage.getStage();
        if(stage) {
            return stage.toDataURL();
        }
        return null;
    },
      AddStampsConfirmation: function (e) {
          var i=this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo.length;
          if(i>0){
              if(this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo[i-1].text == null){
                  this.addFileStamp({stamp: this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo[i-1], pageno: this.page.no});
                  this.updateFileStamp({stamp: {index:this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo[i-1].index, selected: false}, pageno: this.page.no});
                  this.loadStamps();
                  this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo.pop();
              }else {
                  this.addFileText({text: this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo[i-1], pageno: this.page.no});
                  this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo.pop();
              }
          }

          if(this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo.length > 0){
              this.$store.state.home.disabledProceed = false;
          }else {
              this.$store.state.home.disabledProceed = true;
          }
      },
    },
    mounted() {
        this.stampIndex = this.page.stamps.length + 1;
        this.loadStamps();
        this.loadTexts(this.isTextSelected);
        this.$nextTick(() => {
            $(document).on('dragenter',".konvajs-content", (e) => {
                e.preventDefault();
            });
            $(document).on('dragover',".konvajs-content", this.handleDragover);
        });
    },
    created() {
        //すすむボタンチェック
        if(this.$store.state.home.fileSelected.pages[this.page.no-1].deleteInfo.length > 0){
            this.$store.state.home.disabledProceed = false;
        } else if(this.$store.state.home.fileSelected.pages.length <= 1){
            this.$store.state.home.disabledProceed = true;
        }

        window.addEventListener('click', this.onWindowClick);
        this.$once('hook:beforeDestroy', () => {
            window.removeEventListener('click', this.onWindowClick);
        });
        if(this.$store.state.user){
          this.textInfo = {
            last_text_font:this.$store.state.user.last_text_font ? this.$store.state.user.last_text_font : this.defaultFont,
            last_text_size:this.$store.state.user.last_text_size ? this.$store.state.user.last_text_size : this.defaultFontSize,
            last_text_color:this.$store.state.user.last_text_color ? this.$store.state.user.last_text_color : this.defaultFontColor
          };
        }
    },
    updated() {
        this.loadStamps();
        this.loadTexts(this.isTextSelected);
    },
    watch: {
        "$store.state.home.fileSelected.zoom": function (newVal, oldVal) {
            if (this.$refs && this.$refs.stage){
                let stage = this.$refs.stage.getStage();
                let scale = newVal/100;
                let textAreaAttrs = {};
                let nodeText = stage.find(node => node.attrs.type === 'Text' && node.attrs.id === this.textIdSelected);
                if($('textarea') && nodeText[0]) {
                    textAreaAttrs.left = nodeText[0].attrs.x * scale+ 'px';
                    textAreaAttrs.top =  nodeText[0].attrs.y * scale+ 'px';
                    textAreaAttrs.width = nodeText[0].attrs.width * scale+ 'px';
                    textAreaAttrs.height = nodeText[0].attrs.fontSize * scale +13 +'px';
                    textAreaAttrs.fontSize = nodeText[0].attrs.fontSize * scale+'px';
                    $('textarea').css(textAreaAttrs)
                }
                this.openpopover = false;
            }

        },
        "$store.state.home.fileSelected.confidential_flg": function (newVal, oldVal) {
            this.loadStamps();
        },
        "$store.state.home.fileSelected.overlay_hidden_flg": function (newVal, oldVal) {
            this.loadStamps();
        },
        "isTextSelected": function (newVal, oldVal) {
            if(this.selectedId > 0) {
                this.undoAction();
                this.selectedId = 0;
            }
            this.textIdSelected = 0;
            this.loadTexts(newVal);
        },
        "page.texts": {
            handler: function (val, oldVal) {
                if (this.ignoreUpdateText){
                    this.ignoreUpdateText = false;
                }else{
                    this.loadTexts(this.isTextSelected);
                }
            },
            deep: true
        },
        "page.stamps": function (val, oldVal) {
            //this.loadStamps();
            if(val.length === 0) {
                const stage = this.$refs.stage.getStage();
                const layer = stage.children[0];
                if(!stage) {
                    return;
                }
                layer.draw();
            }
        },
        "page.stamps.length": function (val, oldVal) {
            if(val < oldVal) {
                this.loadStamps();
            }
        },
        "noZoomSize.width": function (newVal, oldVal) {
            this.loadStamps();
            this.loadTexts(this.isTextSelected);
        },
        "imageUrl": {
          handler: async function(url) {
            if (url) {
              this.originalSize = await this.getImageSize(url);
            }
          },
          immediate: true,
        },
        color(val,oldVal){
            this.onTextChangeFontColor(val)
        }
    }
}

</script>
<style lang="scss">
.color-box{
    padding: 8px;
    font-size: 0.78rem;
    border: 1px solid rgba(0, 0, 0, 0.2);
    position: relative;
    border-radius: 5px;
    cursor: pointer;
    padding-right: 30px;
    background-color: #fff;
    &:after{
        font-family: "Material Icons";
        font-weight: normal;
        font-style: normal;
        display: inline-block;
        line-height: 1;
        text-transform: none;
        letter-spacing: normal;
        word-wrap: normal;
        white-space: nowrap;
        direction: ltr;
        -webkit-font-smoothing: antialiased;
        text-rendering: optimizeLegibility;
        -moz-osx-font-smoothing: grayscale;
        font-feature-settings: 'liga';
        position: absolute;
        right: 1px;
        font-size: 1.0625em;
        top: 50%;
        pointer-events: none;
        -webkit-transform: translateY(-50%);
        transform: translateY(-50%);
        color: inherit;
        text-align: center;
        content: "keyboard_arrow_down";
    }
}
</style>
