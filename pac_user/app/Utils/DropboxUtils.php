...
class DropboxUtils
{
    ...
    const DROPBOX_API_TOKEN = 'dropbox_api_token';

    public static function getApiClient(){
        Log::debug('--- base_url = '.Config::get('oauth.dropbox.base_url'));
        $client = new Client(['base_uri' => Config::get('oauth.dropbox.base_url'), 'timeout' => config('app.guzzle_timeout'), 'connect_timeout' => config('app.guzzle_connect_timeout'), 'http_errors' => false, 'verify' => false,]);
        return $client;
    }

    public static function getAuthorizedApiClient($isUpload = false,$customHeaders = null){
        $access_token = Session::get(DropboxUtils::DROPBOX_API_TOKEN);
        if(!$access_token) return false;

        $baseUrl = Config::get('oauth.dropbox.base_url');

        if($isUpload) {
            $baseUrl = Config::get('oauth.dropbox.upload_url');
        }
