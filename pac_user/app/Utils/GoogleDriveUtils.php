...
class GoogleDriveUtils
{
    ...
    const GOOGLE_API_TOKEN = 'google_drive_api_token';

    public static function getApiClient(){
        Log::debug('--- base_url = '.Config::get('oauth.google.base_url'));
        $client = new Client(['base_uri' => Config::get('oauth.google.base_url'), 'timeout' => config('app.guzzle_timeout'), 'connect_timeout' => config('app.guzzle_connect_timeout'), 'http_errors' => false, 'verify' => false,]);
        return $client;
    }

    public static function getAuthorizedApiClient($isUpload = false,$customHeaders = null){
        $access_token = Session::get(GoogleDriveUtils::GOOGLE_API_TOKEN);
        if(!$access_token) return false;

        $baseUrl = Config::get('oauth.google.base_url');

        if($isUpload) {
            $baseUrl = Config::get('oauth.google.upload_url');
        }
