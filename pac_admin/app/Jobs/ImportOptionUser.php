...
class ImportOptionUser implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        try {
            ...
            if ($total) {
                ...
                //domainリスト出力
                $domains = preg_split('/\r\n|\r|\n/', $company->domain);
                if($csv_import_list->import_type == AppUtils::STATE_IMPORT_CSV_WITHOUT_EMAIL_OPTION_USER){
                    foreach ($domains as $domain){
                        $domains[] = "$domain.gw.scs";
                    }
                }
                ...
                foreach ($csv_data as $i => $row) {
                    ...
                    if( preg_match("/(@.*)/u", $email, $importDomain) ){
                        ...
                        $email = OptionUserUtils::replaceEmail($email, 'gw');
                    }
                    if ($csv_import_list->import_type == AppUtils::STATE_IMPORT_CSV_WITHOUT_EMAIL_OPTION_USER) {
                    ...
                    // (8)電話番号,(9)FAX番号,
                    // (10)有効化(1:有効にする),
                    // (11)多要素認証(0:無効,1:メール,2:QRコード),(12)備考,
                    ...
                    $data_user = [
                        ...
                        'reference' => $row[12],
                        'option_flg' => AppUtils::USER_OPTION,
                    ];
                    if($csv_import_list->import_type == AppUtils::STATE_IMPORT_CSV_OPTION_USER){
                    ...
                    // 多要素認証の設定
                    if ($company->mfa_flg) {
                        $data_user_info['mfa_type'] = $row[11];
