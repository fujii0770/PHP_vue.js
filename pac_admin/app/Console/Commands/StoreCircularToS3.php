...
class StoreCircularToS3 extends Command
{
    ...
    public function handle()
    {
        ...
        $folderId = $this->option('folder_id');//長期保管フォルダのID
        // 当月
        if (!$finishedDateKey) {
            $finishedDate = '';
        } else {
        ...
        try{
            $path = config('filesystems.prefix_path'). '/' .config('app.s3_storage_root_folder') . '/' . config('app.pac_app_env') . '/' . config('app.pac_contract_app')
                . '/' . config('app.pac_contract_server').'/'.$mst_company_id.'/'.$circular_id;
            ...
            if ($windowCircularUser){
                ...
                $query_sub = DB::table("circular$finishedDate as C")
                    ...
                    AND edition_flg = ".config('app.pac_contract_app')." AND env_flg = ".config('app.pac_app_env')." AND server_flg = ".config('app.pac_contract_server')."
                    ...
                    ->where('U.edition_flg', config('app.pac_contract_app'))
                    ->where('U.env_flg', config('app.pac_app_env'))
                    ->where('U.server_flg', config('app.pac_contract_server'))
                ...
                $query_sub2 = DB::table("circular_user$finishedDate as E")
                    ->join("circular_user$finishedDate as M", function($join) use ($windowCircularUser){
                        ...
                        $join->where('M.edition_flg', '=',config('app.pac_contract_app'));
                        $join->where('M.env_flg', '=',config('app.pac_app_env'));
                        $join->where('M.server_flg', '=',config('app.pac_contract_server'));
                    })
                ...
                //本環境の文書データを取得する
                if ($sendCircularUser->edition_flg == config('app.pac_contract_app') && $sendCircularUser->env_flg == config('app.pac_app_env') && $sendCircularUser->server_flg == config('app.pac_contract_server')){
    ...
    private function formIssuanceIndex($circular_id, $mst_company_id, $table_name)
    {
        ...
        foreach($input_datas as $input_data) {
            ...
            if($input_data['data'] !== "" && $input_data['data'] !== null) {
                ...
                if($data_type === LongtermIndexUtils::DATE_TYPE) {
                ...
                }else if($data_type === LongtermIndexUtils::NUMERIC_TYPE){
                    $lonterm_exists = DB::table('mst_longterm_index')
                        ->where('index_name', $input_data['name'])
                        ->where('template_flg',2)
                        ->where('mst_company_id',$mst_company_id)
    ...
    private function copyCurrentCircularAllAttachment($intCircularID, $finishedDate,$mst_company_id)
    {
        try {
            ...
            ;
            Log::info("------------------   START    ------------------------");
            $folderPathEnv = config('app.pac_app_env') ? config('app.k5_storage_attachment_root_folder') : config('app.s3_storage_attachment_root_folder');
            $folderPath = config('filesystems.prefix_path') . '/' . $folderPathEnv . '/' . config("app.long_term_back_attachment_folder_pre");
            $arrRealPath = [];
            if ($objCircularUser->edition_flg == config('app.pac_contract_app') && ($objCircularUser->env_flg != config('app.pac_app_env') || $objCircularUser->server_flg != config('app.pac_contract_server'))) {
            ...
            $server_url = sprintf("%s/%s/%s/%s", $folderPath, $objCurrentCompanyCircularUser->edition_flg . $objCurrentCompanyCircularUser->env_flg . $objCurrentCompanyCircularUser->server_flg, $mst_company_id, $intCircularID);
            Log::info("server_url---------------------------- $server_url");
            Log::info("map start ---------------------------- ");

            $isDirectory = Storage::disk(config('app.pac_app_env') == CircularAttachmentUtils::ENV_FLG_K5 ? 'k5':'s3')->exists($server_url);
            Log::info("exists : $isDirectory ");
            if(!$isDirectory){
                Storage::disk(config('app.pac_app_env') == CircularAttachmentUtils::ENV_FLG_K5 ? 'k5':'s3')->makeDirectory($server_url);
            }
            foreach ($arrAllAttachment as $item) {
                ...
                if(isset($item->file_data)){
                    ...
                    $file_data = chunk_split(base64_encode(Storage::disk(config('app.pac_app_env') == CircularAttachmentUtils::ENV_FLG_K5 ? 'k5':'s3')->get($item->server_url)));
                }
                // decode
                $file_data = base64_decode($file_data);
                // K5 SERVER
                if (config('app.pac_app_env') == CircularAttachmentUtils::ENV_FLG_K5) {
