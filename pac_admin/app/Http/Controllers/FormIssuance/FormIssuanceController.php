...
class FormIssuanceController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        $arrOrder = ['user' => 'user_name', 'email' => 'U.email', 'frmSrvUserFlg' => 'U.frm_srv_user_flg',
        ...
        $arrHistory = DB::table('mst_user as U')
            ...
            ->select(DB::raw('U.id, U.frm_srv_user_flg, CONCAT(U.family_name, U.given_name) as user_name,U.email, mst_department_id,D.department_name, mst_position_id, P.position_name'))
        ...
        if (($request->get('frmSrvUserFlg') || ($request->get('frmSrvUserFlg') == '0'))) {
            $arrHistory->where('U.frm_srv_user_flg', $request->get('frmSrvUserFlg'));
        }
        ...
        return $this->render('FormIssuance.index');
    }
            ...
            try {
                if ($type == 1) {
                    DB::table('mst_user')
                        ->where('mst_company_id',$user->mst_company_id)
                        ->whereIn('id', $ids)
                        ->update(['frm_srv_user_flg' => '1']);
                } else {
                    DB::table('mst_user')
                        ->where('mst_company_id',$user->mst_company_id)
                        ->whereIn('id', $ids)
                        ->update(['frm_srv_user_flg' => '0']);
                }
                DB::commit();
                if ($type == 1) {
    ...
    public function show($id)
    {
        ...
        $item = DB::table('mst_user as U')
            ...
            ->leftJoin('mst_department as D', 'I.mst_department_id','D.id')
            ->leftJoin('mst_position as P', 'I.mst_position_id','P.id')
            ->leftJoin('mst_frm_srv_user_info as F', 'U.id','F.mst_user_id')
            ->select(DB::raw('U.id, U.frm_srv_user_flg, CONCAT(U.family_name, U.given_name) as user_name,U.email,D.department_name, P.position_name, F.id as frm_id'))
            ->where('U.id', $id)
            ->where('U.mst_company_id',$user->mst_company_id)
            ->first();
    ...
    public function update($id, Request $request)
    {
        ...
        $frm_srv_user_flg = $request->get('frm_srv_user_flg');
        ...
        try{
            DB::table('mst_user')
                ->where('mst_company_id',$user->mst_company_id)
                ->where('id', $id)
                ->update(['frm_srv_user_flg' => $frm_srv_user_flg]);

            $user_info = DB::table('mst_frm_srv_user_info')->where('mst_user_id', $id)->first();
            if (!isset($user_info)) {
                $frm_id = DB::table('mst_frm_srv_user_info')
                    ->insertGetId([
                        'mst_user_id' => $id,
                ...
                DB::table('mst_frm_srv_user_info')
                    ->where('mst_user_id', $id)
                    ->update([
            ...
            Log::error($e->getMessage().$e->getTraceAsString());
            if (!isset($user_info)) {
                return response()->json(['status' => false, 'frm_id' => $frm_id, 'message' => [__('message.false.register_single_form_issuance')]]);
            } else {
                return response()->json(['status' => false, 'frm_id' => $frm_id, 'message' => [__('message.false.update_single_form_issuance')]]);
            }
        }
        if (!isset($user_info)) {
            return response()->json(['status' => true, 'frm_id' => $frm_id, 'message' => [__('message.success.register_single_form_issuance')]]);
        } else {
            return response()->json(['status' => true, 'frm_id' => $frm_id, 'message' => [__('message.success.update_single_form_issuance')]]);
        }
    }
}
