...
class DownloadUtils
{
     ...
     * @throws \Exception
     */
    public static function downloadRequest(CompanyAdmin $user, $class_path, $function_name, $file_name, ...$param){
        try{
            // ダウンロード処理可能か確認
     ...
     * @throws \Exception
     */
    public static function downloadRequestSelectableSanitizing(CompanyAdmin $user, $class_path, $function_name, $file_name, $is_sanitizing, ...$param){
		try{
            // ダウンロード処理可能か確認
            self::checkEnableDownload($user, $class_path, $function_name);
     ...
     * @throws \Exception
     */
    public static function reDownloadRequest(CompanyAdmin $user, $request_id){
        try{
     ...
     * @throws \Exception
     */
    public static function reDownloadRequestSelectableSanitizing(CompanyAdmin $user, $request_id, $class_path, $function_name, $file_name, $is_sanitizing, ...$param){
		try{
            $dl_request = DownloadRequest::find($request_id);
     ...
     * @param string $function_name 関数名
     */
    public static function checkEnableDownload(CompanyAdmin $user, $class_path, $function_name)
    {
        if(!$user){
     ...
     * @throws \Exception
     */
    public static function removeExpiredRequestData()
    {
        $period_request = DownloadRequest::where('download_period', '<=', Carbon::now())
        ...
        try {
            ...
            // データ量多すぎを防ぐため、分割
            $ids = array_chunk($download_req_ids,30);
            foreach ($ids as $request_ids){
                DownloadProcWaitData::wherein('download_request_id', $request_ids)
                    ->update([
                        'document_data' => null,
                        'file_size' => 0,
                    ]);

                DownloadWaitData::wherein('download_request_id', $request_ids)
                    ->update([
                        'data' => null,
                        'update_at' => Carbon::now(),
                        'file_size' => 0,
     ...
     * @throws \Exception
     */
    public static function removeRequestAnHourAgoAndDeleteState()
    {
        try {
            ...
            foreach ($ids as $request_ids){
                // ダウンロード要求
                DownloadRequest::wherein('id', $request_ids)
                    ->delete();
