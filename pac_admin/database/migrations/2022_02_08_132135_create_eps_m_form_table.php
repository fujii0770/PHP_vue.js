<?php

use Illuminate\Database\Migrations\Migration;
use Illuminate\Database\Schema\Blueprint;
use Illuminate\Support\Facades\Schema;

class CreateEpsMFormTable extends Migration
{
    /**
     * Run the migrations.
     *
     * @return void
     */
    public function up()
    {
        Schema::create('eps_m_form', function (Blueprint $table) {
            $table->unsignedBigInteger('mst_company_id');
            $table->string('form_code',20);
            $table->string('form_name',200)->nullable();
            $table->unsignedInteger('form_type')->default(0);
            $table->string('form_describe',256)->nullable();
            $table->unsignedInteger('total_amt_min')->nullable();
            $table->unsignedInteger('total_amt_max')->nullable();
            $table->unsignedInteger('items_max')->default(0);
            $table->date('validity_period_from');
            $table->date('validity_period_to')->nullable();
            $table->unsignedInteger('depend_form_type')->default(0);
            $table->string('s3_path',256)->nullable();
            $table->string('s3_file_name',256)->nullable();
            $table->string('origin_file_name',256)->nullable();
            $table->string('remarks',2000)->nullable();
            $table->integer('display_order')->default(0);
            $table->dateTime('deleted_at')->nullable();
            $table->dateTime('create_at')->default(DB::raw('CURRENT_TIMESTAMP'));
            $table->string('create_user',128);
            $table->dateTime('update_at')->default(DB::raw('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'));
            $table->string('update_user',128);
            $table->bigInteger('version')->unsigned()->default(0);
            //PK
            $table->primary(['mst_company_id','form_code'],'primary_column');
            //FK
            $table->foreign('mst_company_id')->references('id')->on('mst_company');
        });
    }

    /**
     * Reverse the migrations.
     *
     * @return void
     */
    public function down()
    {
        Schema::dropIfExists('eps_m_form');
    }
}
