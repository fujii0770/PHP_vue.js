        ...
        <div class="form-search form-vertical">
            ...
            <div class="row">
                <div class="col-lg-2 form-group">
                    ...
                    {!! \App\Http\Utils\CommonUtils::buildSelect(\App\Http\Utils\AppUtils::EXPENSE_FLG, 'expense_flg', Request::get('expense_flg', ''),'',['class'=> 'form-control']) !!}
                </div>
                    ...
                    <table class="tablesaw-list tablesaw table-bordered adminlist margin-top-5" data-tablesaw-mode="swipe">
                        ...
                        <tr>
                            ...
                            <th class="title sort" scope="col">
                                {!! \App\Http\Utils\CommonUtils::showSortColumn('経費精算利用', 'expense_flg', $orderBy, $orderDir) !!}
                            </th>
                        ...
                        @if ($arrHistory)
                            @foreach ($arrHistory as $i => $item)
                                <tr class="row-{{ $item->id }} row-edit" ng-class="{ edit: id == {{ $item->id }} }">
                                    ...
                                    <td ng-click="detailsRecord({{ $item->id}})" class="title">{{ \App\Http\Utils\AppUtils::EXPENSE_FLG[$item->expense_flg] }}</td>
                                    <td ng-click="detailsRecord({{ $item->id}})" class="title">{{ $item->user_name.'('.$item->email.')' }}</td>
                                    <td ng-click="detailsRecord({{ $item->id}})" class="title">{{ $item->department_name }}</td>
                                    <td ng-click="detailsRecord({{ $item->id}})" class="title">{{ $item->position_name }}</td>
                                </tr>
                            @endforeach
                        @endif
                        </tbody>
            ...
            appPacAdmin.controller('ExpenseUserRegisterController', function($scope, $rootScope, $http){
                ...
                $scope.detaildisplay = function () {
                    ...
                    if ($('.row-edit input:checkbox:checked').length != 1) {
                        ...
                        $rootScope.$emit("openUserDetailsExpense",{id:id});
                    }
                };
