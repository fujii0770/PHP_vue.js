<div ng-controller="ExpenseUserRegisterController">
    <form action="{{\App\Http\Utils\AppUtils::forceSecureUrl(Request::url())}}" name="adminForm" method="GET">
        ...
        <div class="form-search form-vertical">
            <div class="row">
                <div class="col-lg-3">
                    {!! \App\Http\Utils\CommonUtils::showFormFieldVertical('wtsm_name','名称',Request::get('wtsm_name', ''),'text', false,
                    [ 'placeholder' =>'', 'id'=>'wtsm_name' ]) !!}
                </div>
                <div class="col-lg-3">
                    {!! \App\Http\Utils\CommonUtils::showFormFieldVertical('wtsm_describe','説明',Request::get('wtsm_describe', ''),'text', false,
                    [ 'placeholder' =>'', 'id'=>'wtsm_describe' ]) !!}
                </div>
                <div class="col-lg-3">
                    ...
                    <table class="tablesaw-list tablesaw table-bordered adminlist margin-top-5" data-tablesaw-mode="swipe">
                        ...
                        <tr>
                            ...
                            <th scope="col"  class="sort" style="width: 20%">
                                {!! \App\Http\Utils\CommonUtils::showSortColumn('名称', 'wtsm_name', $orderBy, $orderDir) !!}
                            </th>
                        ...
                        @if ($arrHistory)
                            @foreach ($arrHistory as $i => $item)
                               <tr class="row-'{{ $item->wtsm_name }}' row-edit" ng-class="{ edit: wtsm_name == '{{ $item->wtsm_name }}' }">
                                    <td class="title">
                                        <input type="checkbox" value="{{ $item->wtsm_name }}"
                                        name="cids[]" class="cid" onClick="isChecked(this.checked)"
                                        ng-click="toogleCheck('{{ $item->wtsm_name }}')"/> 
                                    </td>
                                    <td ng-click="detailsRecord('{{ $item->wtsm_name}}')" class="title">{{ $item->wtsm_name}}</td>
                                    <td ng-click="detailsRecord('{{ $item->wtsm_name}}')" class="title">{{ $item->wtsm_describe }}</td>
                                    <td ng-click="detailsRecord('{{ $item->wtsm_name}}')" class="title">{{ $item->remarks }}</td>
                                </tr>
                            @endforeach
                        @endif
                        </tbody>
    ...
    <script>
        if(appPacAdmin){
            appPacAdmin.controller('ExpenseUserRegisterController', function($scope, $rootScope, $http){
                ...
                $scope.cids = {!! json_encode($arrHistory->pluck('wtsm_name')) !!};
                ...
                $scope.addPurpose = function (wtsm_name) {
                    $rootScope.$emit("openUserDetailsExpMWtsmAdd",{wtsm_name:wtsm_name});
                };

                $scope.detailsRecord = function (wtsm_name) {
                    $rootScope.$emit("openUserDetailsExpMWtsm",{wtsm_name:wtsm_name});
                };
