                        ...
                        <div class="form-group">
                            <div class="row">
                                ...
                                <div class="col-lg-6 text-right">
                                    ...
                                    @endcanany
                                    @canany([PermissionUtils::PERMISSION_MASTER_EXPENSE_SETTING_CREATE])
                                        <div class="btn btn-success  mb-1" ng-click="searchPurpose(0)"><i class="fas fa-plus-circle" ></i> 関連様式設定</div>
                                    @endcanany
                                </div>
                            </div>
                        </div>
        ...
        if(appPacAdmin){
            appPacAdmin.controller('ExpenseUserRegisterController', function($scope, $rootScope, $http){
				...
				$scope.downloadCsv = function(){
                    $rootScope.$emit("showMocalConfirm",
                        {
                            title:'精算申請様式データを出力します。<br/>実行しますか？',
                            btnSuccess:'はい',
                            size:"default",
                            callSuccess: function(){
                                $rootScope.$emit("showLoading");
                ...
                $scope.searchPurpose = function (form_code) {
                    $rootScope.$emit("openUserDetailsExpMPurposeSearch",{form_code:form_code});
