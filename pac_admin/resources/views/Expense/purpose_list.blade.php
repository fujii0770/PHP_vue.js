<div ng-controller="ExpenseUserRegisterController">
    <form action="{{\App\Http\Utils\AppUtils::forceSecureUrl(Request::url())}}" name="adminForm" method="GET">
        ...
        <div class="form-search form-vertical">
            <div class="row">
                <div class="col-lg-3">
                    {!! \App\Http\Utils\CommonUtils::showFormFieldVertical('purpose_name','名称',Request::get('purpose_name', ''),'text', false,
                    [ 'placeholder' =>'', 'id'=>'purpose_name' ]) !!}
                </div>
                <div class="col-lg-3">
                    {!! \App\Http\Utils\CommonUtils::showFormFieldVertical('describe','説明',Request::get('describe', ''),'text', false,
                    [ 'placeholder' =>'', 'id'=>'describe' ]) !!}
                </div>
                <div class="col-lg-3">
                    ...
                    <table class="tablesaw-list tablesaw table-bordered adminlist margin-top-5" data-tablesaw-mode="swipe">
                        ...
                        <tr>
                            ...
                            <th scope="col"  class="sort" style="width: 20%">
                                {!! \App\Http\Utils\CommonUtils::showSortColumn('名称', 'purpose_name', $orderBy, $orderDir) !!}
                            </th>
                            <th scope="col"  class="sort" style="width: 40%">
                                {!! \App\Http\Utils\CommonUtils::showSortColumn('説明', 'describe', $orderBy, $orderDir) !!}
                            </th>
                            <th scope="col"  class="sort" style="width: 40%">
                        ...
                        @if ($arrHistory)
                            @foreach ($arrHistory as $i => $item)
                               <tr class="row-'{{ $item->purpose_name }}' row-edit" ng-class="{ edit: purpose_name == '{{ $item->purpose_name }}' }">
                                    <td class="title">
                                        <input type="checkbox" value="{{ $item->purpose_name }}"
                                        name="cids[]" class="cid" onClick="isChecked(this.checked)"
                                        ng-click="toogleCheck('{{ $item->purpose_name }}')"/> 
                                    </td>
                                    <td ng-click="detailsRecord('{{ $item->purpose_name}}')" class="title">{{ $item->purpose_name}}</td>
                                    <td ng-click="detailsRecord('{{ $item->purpose_name}}')" class="title">{{ $item->describe }}</td>
                                    <td ng-click="detailsRecord('{{ $item->purpose_name}}')" class="title">{{ $item->remarks }}</td>
                                </tr>
                            @endforeach
                        @endif
                        </tbody>
    ...
    <script>
        if(appPacAdmin){
            appPacAdmin.controller('ExpenseUserRegisterController', function($scope, $rootScope, $http){
                ...
                $scope.cids = {!! json_encode($arrHistory->pluck('purpose_name')) !!};
                ...
                $scope.addPurpose = function (purpose_name) {
                    // purpose_name = '';
                    $rootScope.$emit("openUserDetailsExpMPurposeAdd",{purpose_name:purpose_name});
                };

                $scope.detailsRecord = function (purpose_name) {
                    $rootScope.$emit("openUserDetailsExpMPurpose",{purpose_name:purpose_name});
                };
