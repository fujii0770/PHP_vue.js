<div ng-controller="DetailLongTermIndexTemplateController">
    <form class="form_edit" action="" method="" onsubmit="return false;">
        <div class="modal modal-detail" id="modalDetailTemplateItem" data-backdrop="static" data-keyboard="false">
            <div class="modal-dialog">
                <div class="modal-content">
            
                <!-- Modal Header -->
                <div class="modal-header">
                    <h4 class="modal-title" ng-show="!info.id">テンプレートインデックス情報登録</h4>
                    <h4 class="modal-title" ng-show="info.id">テンプレートインデックス情報更新</h4>
                ...
                <div class="modal-body form-horizontal" ng-show="!info.id">
                    ...
                    <div class="form-group">
                        <div class="row">
                            ...
                            {!! \App\Http\Utils\CommonUtils::buildSelect($longTermIndexName, 'longTermIndexName','','',
                                    ['class'=> 'form-control', 'ng-model' =>'info.temp', 'ng-readonly'=>"readonly",'id'=> 'form-temp']) !!}
                            </div>
                        </div>
                    </div>
...
@push('scripts')
    <script>
        if(appPacAdmin){
            appPacAdmin.controller('DetailLongTermIndexTemplateController', function($scope, $rootScope, $http) {
                $scope.save = function(){
                    if($(".form_edit")[0].checkValidity()) {
                        hideMessages();
                        // $rootScope.$on("openNewTemplateIndex");
                        var saveSuccess = function(event) {
                            ...
                            if(event.data.status == false){
                                ...
                                $("#modalDetailTemplateItem .message").append(showMessages(event.data.message, 'success', 10000));
                                hasChange = true;
                            }
                        }
                ...
                $rootScope.$on("openNewTemplateIndex", function(event){
                    $rootScope.id = 0;
                        ...
                        $rootScope.$emit("showMocalConfirm",
                            {
                                ...
                                callDanger: function(id){
                                    ...
                                    $http.post(link_ajax + "/templateRelease/" + id, { })
                                        .then(function(event) {
                                            $rootScope.$emit("hideLoading");
                                            if(event.data.status == false){
                                                $("#modalDetailTemplateItem .message").append(showMessages(event.data.message, 'danger', 10000));
                                            }else{
                                                $("#modalDetailTemplateItem .message").append(showMessages(event.data.message, 'warning', 10000));
                                                location.reload();
                                            }
                                        });
                                }
                ...
                $scope.tempsave = function(){
                    if($(".form_edit")[0].checkValidity()) {
                        ...
                        $http.post(link_ajax + "/templateValid/" + $form.value, { })
                                        .then(function(event) {
                                            $rootScope.$emit("hideLoading");
                                            if(event.data.status == false){
                                                $("#modalDetailTemplateItem .message").append(showMessages(event.data.message, 'danger', 10000));
