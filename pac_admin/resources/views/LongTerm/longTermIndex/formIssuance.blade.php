<div ng-controller="ListLongTermIndexFormIssuanceController">
    ...
    <form action="" name="adminForm">
        <div class="card mt-3">
            <div class="card-header">明細インデックス一覧</div>
            <div class="card-body">
                ...
                <table class="tablesaw-list tablesaw table-bordered adminlist mt-1" data-tablesaw-mode="swipe">
                    ...
                    <tbody>
                        @foreach ($frm_longTermStorage as $i => $item)
                        <tr class="">
                            <td class="title">
                ...
                @include('layouts.table_footer',['data' => $frm_longTermStorage])
            </div>
...
@push('scripts')
    <script>
        if(appPacAdmin){
            appPacAdmin.controller('ListLongTermIndexFormIssuanceController', function($scope, $rootScope, $http){
                $rootScope.info = {};
                $rootScope.id = 0;
                ...
                $scope.tempaddNew = function(){
                    $rootScope.$emit("openNewFormIssuanceIndex");
                };
        ...
        $("#modalDetailFormIssuanceItem").on('hide.bs.modal', function () {
             if(hasChange){
                 location.reload();
             }
        });
