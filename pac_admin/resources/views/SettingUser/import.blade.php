        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">            
                ...
                <div class="modal-header">
                    <h4 class="modal-title" ng-if="!showResult&&!without_email_import_flg">CSV取込(利用者登録)</h4>
                    <h4 class="modal-title" ng-if="!showResult&&without_email_import_flg">無ユーザーCSV取込(利用者登録)</h4>
                ...
                <div class="modal-body form-horizontal">
                    ...
                    <div class="sapo">
                        <div ng-if="!showResult&&!without_email_import_flg">
                            ...
                            <br>
                            <div>CSV形式:</div>
                            <div>　メールアドレス(xxx@domain.co.jp形式),姓,名,部署,役職,郵便番号,住所,電話番号,FAX番号,ホームページ,印面設定(※),印面文字,
                                有効化(1:有効にする),日付印の日付変更(0:変更不可),APIの使用(0:許可しない),多要素認証(0:無効,1:メール,2:QRコード),
                                認証コード送信先(0:登録メールアドレス,1:その他),認証コード送信先メールアドレス(xxx@domain.co.jp形式),テンプレート機能(0:無効,1:有効),
                                おじぎ印(0:無効,1:有効)</div>
                            <br>
                            <div>※印面設定 0:なし、1:氏名印[楷書]、2:氏名印[古印]、3:氏名印[行書]、4:日付印[楷書]、5:日付印[古印]、6:日付印[行書]</div>
                            <div>　0を設定した場合は、印面文字も空(null)にしてください。</div>
                            <div>　1～6を設定した場合は、既に印面が登録されている場合でも、新たに印面が追加されます。</div>
                            ...
                            <div class="text-center">
                                ...
                                <label><button type="button" class="btn btn-primary" onclick="location.href='import-history'"><span class="fas fa-file-import"></span>csv取込履歴一覧</button></label>
                            </div>
                        </div>
                        <div ng-if="!showResult&&without_email_import_flg">
                            <div>追加する利用者情報を記載したCSVファイルを指定してください。</div>
                            ...
                            <div>CSV形式:</div>
                            <div>　メールアドレス(xxx@domain.co.jp形式),姓,名,部署,役職,郵便番号,住所,電話番号,FAX番号,ホームページ,印面設定(※),印面文字,
                                有効化(1:有効にする),日付印の日付変更(0:変更不可),APIの使用(0:許可しない),多要素認証(0:無効,2:QRコード),
                                認証コード送信先(0:固定),認証コード送信先メールアドレス(空:固定),テンプレート機能(0:無効,1:有効),
                                おじぎ印(0:無効,1:有効)</div>
                            <br>
                            <div>※印面設定 0:なし、1:氏名印[楷書]、2:氏名印[古印]、3:氏名印[行書]、4:日付印[楷書]、5:日付印[古印]、6:日付印[行書]</div>
                            <div>　0を設定した場合は、印面文字も空(null)にしてください。</div>
