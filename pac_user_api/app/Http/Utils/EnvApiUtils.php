...
class EnvApiUtils
{
    ...
    public static function getAuthorizeClient($envId,$serverFlg)
    {
        $accessToken = null;
    ...
    public static function getEditionAuthorizeClient($envId, $circular, $currentCircularUser)
    {
        ...
        if($result->getStatusCode() == StatusCodeUtils::HTTP_OK || $result->getStatusCode() == StatusCodeUtils::HTTP_CREATED) {
            $response = json_decode((string) $result->getBody());
            if ($response->status == StatusCodeUtils::HTTP_OK || $response->status == StatusCodeUtils::HTTP_CREATED){
