<?php
/* This file is just a duplicate of pac_user/app/Services/OffeceConverterService. */
...
class OfficeConverterService extends OfficeConverter
{
    /** @var string */
    private $file;
    /** @var string */
     ...
     */
    public function __construct($filename, $tempPath = null, $default_user_suffix = "", $bin = 'libreoffice', $prefixExecWithExportHome = true)
    {
        if ($this->open($filename)) {
            $this->setup($tempPath, $bin, $prefixExecWithExportHome);
            $this->user = config('app.libreoffice_location') . $default_user_suffix;
        }
    }
    ...
    protected function makeCommand($outputDirectory, $outputExtension)
    {
        ...
        return "{$this->bin} --nologo --headless --nofirststartwizard -env:UserInstallation=file://$this->user --convert-to {$outputExtension} {$oriFile} --outdir {$outputDirectory}";
    }

    /**
     * @return false|string
     */
    public function lockFreeProfile()
    {
        $path = storage_path("app/libreoffice");
        if (!File::exists($path)) {
            File::makeDirectory($path, 0777, true);
        }

        for ($i = 0; $i <= config('app.libreoffice_user_count'); $i++) {
            $user_path = "$path/user" . ($i ? $i : "");
            if (!is_file($user_path)) {
                $fileStream = fopen($user_path, "w+");
                fclose($fileStream);
                $this->tmp = $user_path;
                return $this->user = config('app.libreoffice_location') . ($i ? $i : "");
            }
        }
        return false;
    }
