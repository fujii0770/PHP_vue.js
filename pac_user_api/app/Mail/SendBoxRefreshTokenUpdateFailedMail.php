...
class SendBoxRefreshTokenUpdateFailedMail extends Mailable
{
    ...
    public function build()
    {
        ...
        if ($this->isPlain) {
            return $this->from(config('mail.from.address'), config('mail.from.name'))
                ...
                ->text('email_plain_template.box_refresh_token_update_failed_template', $this->data);
        } else {
            return $this->from(config('mail.from.address'), config('mail.from.name'))
                ->subject($this->data['mail_subject'])
                ->view('email_template.box_refresh_token_update_failed_template', $this->data);
        }
    }
