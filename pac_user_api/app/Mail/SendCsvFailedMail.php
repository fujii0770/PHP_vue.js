...
class SendCsvFailedMail extends Mailable
{
    ...
    public function build()
    {
        ...
        if($this->isPlain){
            return $this->from(config('mail.from.address'),config('mail.from.name'))
                ...
                ->text('email_plain_template.pe_send_csv_failed_template',$this->data);
        } else {
            return $this->from(config('mail.from.address'),config('mail.from.name'))
                ->subject($this->data['mail_subject'])
                ->view('email_template.pe_send_csv_failed_template',$this->data);
        }
    }
}
