...
class SendTrialSuccessMail extends Mailable
{
    ...
    public function build()
    {
        ...
        if($this->isPlain){
            return $this->from(config('mail.from.address'),config('mail.from.name'))
                ->bcc(config('mail.HomepageReceiverEmail'))
                ->subject($this->data['mail_subject'])
                ->text('email_plain_template.trialSuccessMail',$this->data);
        } else{
            return $this->from(config('mail.from.address'),config('mail.from.name'))
                ->bcc(config('mail.HomepageReceiverEmail'))
                ->subject($this->data['mail_subject'])
                ->view('email_template.trialSuccessMail',$this->data);
        }
    }
