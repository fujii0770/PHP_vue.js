...
class SendDiskFileAccessCodeMail extends Mailable
{
	...
	public function build()
	{
		...
		if($this->isPlain){
			return $this->from(config('mail.from.address'), config('mail.from.name'))
				...
				->text('email_plain_template.SendDiskFileAccessCodeMail', $this->data);
		}
		else{
			return $this->from(config('mail.from.address'), config('mail.from.name'))
					->subject($this->data['mail_subject'])
					->view('email_template.SendDiskFileAccessCodeMail', $this->data);
		}
	}
}
