...
class TokenGuard
{
    ...
    protected function authenticateViaBearerToken($request)
    {
        ...
        if ($oauth_scopes){
            if (is_array($oauth_scopes) && count($oauth_scopes) > 0){
                if ($oauth_scopes[0] == AppUtils::ACCOUNT_TYPE_AUDIT){
                    $authModel = AuditUser::class;
                }
            }else{
                if ($oauth_scopes == AppUtils::ACCOUNT_TYPE_AUDIT){
                    $authModel = AuditUser::class;
                }
            }
        }
    //    Log::debug('authenticateViaBearerToken - authModel: '.$authModel);
        $this->provider->setModel($authModel);
